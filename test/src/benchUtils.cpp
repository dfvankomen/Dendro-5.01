...
int main(int argc, char** argv) {
    ...
    if (!rank) std::cout << "unzip valid scatter: " << valid << std::endl;

    mesh->unzipDG_scatter(v_dg, v_unzip, 1);
    valid = ot::test::isUnzipValid(mesh, v_unzip, fr, 1e-3);
    if (!rank) std::cout << "unzip valid dg scatter: " << valid << std::endl;

    t_unzip_cg.start();
    for (unsigned int i = 0; i < iter; i++) mesh->unzip_scatter(u, u_unzip, 1);
    t_unzip_cg.stop();
    ...
    if (!rank) std::cout << "unzip valid gather: " << valid << std::endl;

    mesh->unzipDG(v_dg, v_unzip, 1);
    valid = ot::test::isUnzipValid(mesh, v_unzip, fr, 1e-3);
    if (!rank) std::cout << "unzip valid dg gather: " << valid << std::endl;

    t_unzip_cg.start();
    for (unsigned int i = 0; i < iter; i++) mesh->unzip(u, u_unzip, 1);
    t_unzip_cg.stop();

    t_unzip_dg.start();
    for (unsigned int i = 0; i < iter; i++) mesh->unzipDG(v_dg, v_unzip, 1);
    t_unzip_dg.stop();
