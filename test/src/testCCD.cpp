/**
 * testCCD — correctness gate for the combined compact difference operators.
 *
 ...
 * Three checks:
 *
 *  1. Oracle match. scripts/ccd_operators.py builds the same operators by an
 ...
 *     a singular CCD system returns plausible garbage rather than failing, so
 *     the rcond check in the builder is load-bearing and must itself be tested.
 *
 * Run:
 *     cmake --build build_avx2 --target testCCD -j && ./build_avx2/testCCD
 */
...
// cond(A) ~ 1e6 for a healthy CCD6 system, so ~1e-10 is the honest floor for
...
static std::unique_ptr<Derivs> make_ccd6(unsigned int order) {
    auto &reg = (order == 1) ? get_first_order_registry()
                             : get_second_order_registry();
    auto it   = reg.find("CCD6");
...
static void check_oracle() {
    std::cout << "\n[1] oracle match vs scripts/ccd_operators.py "
              << "(all four bflag variants)\n";

    for (unsigned int order = 1; order <= 2; order++) {
        auto d = make_ccd6(order);
        if (!d) {
            report("CCD6 registry lookup", false,
                   "not registered for order " + std::to_string(order));
            return;
        }
        for (unsigned int e = 0; e < CCD_GOLDEN_COUNT; e++) {
            const auto &g = CCD_GOLDEN[e];
...
static void check_convergence() {
    ...
    for (unsigned int order = 1; order <= 2; order++) {
        ...
        std::snprintf(buf, sizeof(buf),
                      ...
                      monotone ? "yes" : "no");
        // one-sided: overshoot is fine (preasymptotic, and CCD's coupling
...
static void check_degenerate_rejected() {
    ...
    // Take CCD6 and make the two boundary rows linearly dependent on purpose,
    ...
    std::unique_ptr<CCDDiagonalEntries> bad(createCCD6Diagonals());
    const double k = 6.0 / 49.0;
    bad->eq1.GBoundary = bad->eq2.GBoundary;
    bad->eq1.WBoundary = bad->eq2.WBoundary;
    bad->eq1.FBoundary = bad->eq2.FBoundary;
    for (auto &row : bad->eq1.GBoundary)
        for (auto &v : row) v *= k;
    for (auto &row : bad->eq1.WBoundary)
        for (auto &v : row) v *= k;
    for (auto &row : bad->eq1.FBoundary)
        for (auto &v : row) v *= k;
    bad->eq1.GBoundaryLower = bad->eq1.GBoundary;
    bad->eq1.WBoundaryLower = bad->eq1.WBoundary;
    bad->eq1.FBoundaryLower = bad->eq1.FBoundary;

    bool threw = false;
    std::string msg;
    try {
        auto st = createCCDMatrixSystemForSingleSize<1>(PW, 17, bad.get(),
                                                        false);
    ...
    bool ok = true;
    try {
        std::unique_ptr<CCDDiagonalEntries> good(createCCD6Diagonals());
        auto st = createCCDMatrixSystemForSingleSize<1>(PW, 17, good.get(),
                                                        false);
    } catch (const std::exception &e) {
        ok  = false;
        msg = e.what();
    }
    report("healthy CCD6 still builds", ok, ok ? "" : msg);
}

int main(int argc, char **argv) {
    std::cout << "================================================\n"
              << "testCCD — combined compact difference operators\n"
    ...
    std::cout << "\n------------------------------------------------\n";
    if (g_failures == 0) {
        std::cout << "PASS — all CCD checks green\n";
    } else {
