// Bit-exactness gate for Mesh::zip().
//
// Compares the optimized Mesh::zip() against Mesh::zip_ref() (the verbatim
// pre-optimization baseline kept in include/mesh.tcc) BYTE FOR BYTE over the
// full CG vector, on a multi-refinement-level mesh.
//
// zip is a pure gather (no FP arithmetic, no reduction), so there is no
// reordering-tolerance argument: the only acceptable result is memcmp()==0.
//
// The gate is designed to be able to FAIL. Build with -DZIP_GATE_SABOTAGE=<n>
// to inject a deliberate defect and confirm the gate catches it:
//   1 = perturb one output value      (single-word data corruption)
//   2 = skip the last block           (missing writes)
//   3 = flip the ownership filter     (wrong-node writes / wrong owner)
// A gate that has never failed is not evidence.
...
int main(int argc, char** argv) {
    ...
    // Run the full bit-exact comparison against a given mesh. Called once for
    // the original mesh and once for a REMESHED one -- the remesh round is
    // what proves a cached, mesh-derived zip plan is actually invalidated
    // rather than silently reused against a mesh it no longer describes.
    ...
    if (mesh->isActive()) {
        ...
        // Several independent field patterns: an analytic field, plus
        // adversarial patterns (index-valued, random) that would expose an
        // index bug the smooth field could mask by coincidence.
        for (unsigned int f = 0; f < n_fields; f++) {
            ...
            // poison both outputs identically so untouched entries match
            // trivially and only zip's own writes are under test
