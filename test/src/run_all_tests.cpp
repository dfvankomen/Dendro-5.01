 ...
 *@brief Contains collection of test cases, to check the all mesh, interp, ghost
 *sync functionality.
 */
...
int main(int argc, char** argv) {
    ...
    // std::function<double(double,double,double)> func =[d_min,d_max](const
    // double x,const double y,const double z){ return
    ...
    /*std::function<double(double,double,double)> func =[](const double x,const
    double  y,const double z){ return (x*x+y*y+z*z);};
    std::function<double(double,double,double)> dx_func =[](const double x,const
    double  y,const double z){ return (2*x);};
    std::function<double(double,double,double)> dy_func =[](const double x,const
    double  y,const double z){ return (2*y);};
    std::function<double(double,double,double)> dz_func =[](const double x,const
    double  y,const double z){ return (2*z);};*/
    // std::function<double(double,double,double)> func =[](const double x,const
    // double  y,const double z){ return (x+y+z);};
    ...
    if (!rank)
        std::cout << GRN << " # f20. octants: " << globalSz << NRM << std::endl;
    ...
    const int p_npes_prev =
        binOp::getPrevHighestPowerOfTwo(std::max((globalSz / grainSz), 1LL));
    const int p_npes_next =
        binOp::getNextHighestPowerOfTwo(std::max((globalSz / grainSz), 1LL));
    ...
    shrinkOrExpandOctree(tmpNodes, DENDRO_DEFAULT_LB_TOL, DENDRO_DEFAULT_SF_K,
    ...
    if (isActive) {
        ...
        SFC::parSort::SFC_treeSort(tmpNodes, tmpVec, tmpVec, tmpVec,
                                   DENDRO_DEFAULT_LB_TOL, m_uiMaxDepth, root,
                                   ROOT_ROTATION, 1, TS_REMOVE_DUPLICATES,
                                   DENDRO_DEFAULT_SF_K, commActive);
    ...
    // this is explicit refinement , for the given function above it sometimes
    // generate regular grid by wavelets, this below code is to enforce
    // artificial refinement, so that, the unzip and other routines can be
    // tested on adaptive grid.
    ...
    do {
        ...
        std::function<double(double, double, double)> waveletTolFunc =
            [wavelet_tol](double x, double y, double z) { return wavelet_tol; };
        bool includebdy = false;
        isRefine1       = wavelet::compute_wavelet_remesh_flags(
            mesh, refine_flag, refvar, varids, 1, waveletTolFunc, 0.01,
            includebdy);
        // mesh->isReMeshUnzip((const double **)vars ,(const unsigned int
        // *)vIDs,1,w_tol_coarsen);
        //  if(!rank)
        //      std::cout<<"is refine triggered for coarsening tolerance :
        //      "<<isRefine<<std::endl;
        ...
        //     (pNodes[ele].getParent() == pNodes[ele +
        //     NUM_CHILDREN-1].getParent()) )
        //     // {
    ...
    ot::test::isElementalNodalValuesValid(mesh, &(*(funcVal.begin())), func,
                                          1e-3);
    ...
    if (!rank) {
        std::cout << YLW
                  << "========================================================="
                     "======================================="
                  << NRM << std::endl;
        std::cout << "        Nodal to Hanging Computation Check Begin     "
                  << std::endl;
        std::cout << YLW
                  << "========================================================="
                     "======================================="
    ...
    if (!rank) {
        std::cout << YLW
                  << "========================================================="
                     "======================================="
                  << NRM << std::endl;
        std::cout << "        Nodal to Hanging Computation Check End     "
                  << std::endl;
        std::cout << YLW
                  << "========================================================="
                     "======================================="
    ...
    if (!rank) {
        std::cout << YLW
                  << "========================================================="
                     "======================================="
                  << NRM << std::endl;
        std::cout << "        UnZip Test Check Begin  (unzip from a DG vec)    "
                  << std::endl;
        std::cout << YLW
                  << "========================================================="
                     "======================================="
...

    {
        ...
        for (unsigned int ele = mesh->getElementLocalBegin();
            ...
            mesh->getElementNodalValues(funcVal.data(), nvec.data(), ele,
                                        false);
    ...
    if (!rank) {
        std::cout << YLW
                  << "========================================================="
                     "======================================="
                  << NRM << std::endl;
        std::cout << "        UnZip Test Check End  (unzip from a DG vec)    "
                  << std::endl;
        std::cout << YLW
                  << "========================================================="
                     "======================================="
...

    {
        ...
        for (unsigned int blk = 0; blk < blkList.size(); blk++) {
            ...
            for (unsigned int ele = blkList[blk].getLocalElementBegin();
                ...
                mesh->getUnzipElementalNodalValues(funcValUnZip.data(), blk,
                                                   ele, blkIn.data(), true);
                ...
                // if(l_max > 1e-2)
                // {
                //     std::cout<<"rank: "<<mesh->getMPIRank()<<" blk :
                //     "<<blk<<" ele ID: "<<ele<<" wavelets(max) :
                //     "<<l_max<<std::endl; for(unsigned int k=0; k <
                //     wCout.size(); k++)
    ...
    if (!rank) {
        std::cout << YLW
                  << "========================================================="
                     "======================================="
                  << NRM << std::endl;
        std::cout << "        interpolation to sphere check begin    "
                  << std::endl;
        std::cout << YLW
                  << "========================================================="
                     "======================================="
    ...
    if (!rank) {
        std::cout << YLW
                  << "========================================================="
                     "======================================="
                  << NRM << std::endl;
        std::cout << "        interpolation to sphere check end     "
                  << std::endl;
        std::cout << YLW
                  << "========================================================="
                     "======================================="
...

    {
        ...
        for (unsigned int ele = mesh->getElementPreGhostBegin();
             ele < mesh->getElementPreGhostEnd(); ele++) {
            if (!(cellVec[ele] == 0 || cellVec[ele] == pNodes[ele].getLevel()))
                std::cout << "rank: " << mesh->getMPIRank()
                          << " elemental ghost sync error in pre ghost "
                          << std::endl;
        }
        ...
        for (unsigned int ele = mesh->getElementLocalBegin();
            ...
            if (!(cellVec[ele] == pNodes[ele].getLevel()))
                std::cout << "rank: " << mesh->getMPIRank()
                          << " elemental ghost sync currupts local values "
                          << std::endl;
        }

        for (unsigned int ele = mesh->getElementPostGhostBegin();
             ele < mesh->getElementPostGhostEnd(); ele++) {
            if (!(cellVec[ele] == 0 || cellVec[ele] == pNodes[ele].getLevel()))
                std::cout
                    << "rank: " << mesh->getMPIRank()
                    << " elemental ghost sync error in post ghost cell val "
                    << cellVec[ele] << " nodel lev: " << pNodes[ele].getLevel()
                    << std::endl;
        }
...

    {
                ...
                for (unsigned int k = 0; k < eOrder + 1; k++)
                    for (unsigned int j = 0; j < eOrder + 1; j++)
                        for (unsigned int i = 0; i < eOrder + 1; i++) {
                            double x = pNodes[ele].minX() +
                                       i * ((1u << m_uiMaxDepth -
                                                       pNodes[ele].getLevel()) /
                                            eOrder);
                            double y = pNodes[ele].minY() +
                                       j * ((1u << m_uiMaxDepth -
                                                       pNodes[ele].getLevel()) /
                                            eOrder);
                            double z = pNodes[ele].minZ() +
                                       k * ((1u << m_uiMaxDepth -
                                                       pNodes[ele].getLevel()) /
                                            eOrder);
        ...
        if (npes > 1 && mesh->isActive()) {
            const std::vector<unsigned int> recvEleOffset =
                mesh->getElementRecvOffsets();
            const std::vector<unsigned int> recvEleCounts =
                mesh->getElementRecvCounts();
            const std::vector<unsigned int> recvESM = mesh->getRecvElementSM();

            for (unsigned int v = 0; v < 2; v++) {
                for (unsigned int p = 0; p < mesh->getMPICommSize(); p++) {
                    for (unsigned int ee = recvEleOffset[p];
                         ee < (recvEleOffset[p] + recvEleCounts[p]); ee++) {
...

    {
        ...
        Stencil<double, 5, 2> D1_Order4StencilCentered_x(
            fd::D1_ORDER_4_CENTERED, 5, StencilDirection::STENCIL_DIR_X);
        Stencil<double, 5, 4> D1_Order4StencilBackward_x(
            fd::D1_ORDER_4_BACKWARD, 5, StencilDirection::STENCIL_DIR_X);
        Stencil<double, 5, 0> D1_Order4StencilForward_x(
            fd::D1_ORDER_4_FORWARD, 5, StencilDirection::STENCIL_DIR_X);

        Stencil<double, 5, 1> D1_Order4StencilUpWind_x(
            fd::D1_ORDER_4_UPWIND, 5, StencilDirection::STENCIL_DIR_X);
        Stencil<double, 5, 3> D1_Order4StencilDownWind_x(
            fd::D1_ORDER_4_DOWNWIND, 5, StencilDirection::STENCIL_DIR_X);

        Stencil<double, 5, 2> D1_Order4StencilCentered_y(
            fd::D1_ORDER_4_CENTERED, 5, StencilDirection::STENCIL_DIR_Y);
        Stencil<double, 5, 4> D1_Order4StencilBackward_y(
            fd::D1_ORDER_4_BACKWARD, 5, StencilDirection::STENCIL_DIR_Y);
        Stencil<double, 5, 0> D1_Order4StencilForward_y(
            fd::D1_ORDER_4_FORWARD, 5, StencilDirection::STENCIL_DIR_Y);

        Stencil<double, 5, 1> D1_Order4StencilUpWind_y(
            fd::D1_ORDER_4_UPWIND, 5, StencilDirection::STENCIL_DIR_Y);
        Stencil<double, 5, 3> D1_Order4StencilDownWind_y(
            fd::D1_ORDER_4_DOWNWIND, 5, StencilDirection::STENCIL_DIR_Y);

        Stencil<double, 5, 2> D1_Order4StencilCentered_z(
            fd::D1_ORDER_4_CENTERED, 5, StencilDirection::STENCIL_DIR_Z);
        Stencil<double, 5, 4> D1_Order4StencilBackward_z(
            fd::D1_ORDER_4_BACKWARD, 5, StencilDirection::STENCIL_DIR_Z);
        Stencil<double, 5, 0> D1_Order4StencilForward_z(
            fd::D1_ORDER_4_FORWARD, 5, StencilDirection::STENCIL_DIR_Z);

        Stencil<double, 5, 1> D1_Order4StencilUpWind_z(
            fd::D1_ORDER_4_UPWIND, 5, StencilDirection::STENCIL_DIR_Z);
        Stencil<double, 5, 3> D1_Order4StencilDownWind_z(
            fd::D1_ORDER_4_DOWNWIND, 5, StencilDirection::STENCIL_DIR_Z);
        ...
        //@milinda : If you are using grad functions defined in rkTransport
        utills, make sure to use the correct domain paramerters when computing
        the h. mesh.createUnZippedVector(unZipIn,0.0);
        ...
        //std::cout<<"zip vec: "<<funcVal.size()<<" unZipVec:
        "<<unZipIn.size()<<std::endl;
        ...
        mesh->applyStencil(funcVal, dx_funcVal, D1_Order4StencilCentered_x,
                           D1_Order4StencilBackward_x,
                           D1_Order4StencilForward_x);
        mesh->applyStencil(funcVal, dy_funcVal, D1_Order4StencilCentered_y,
                           D1_Order4StencilBackward_y,
                           D1_Order4StencilForward_y);
        mesh->applyStencil(funcVal, dz_funcVal, D1_Order4StencilCentered_z,
                           D1_Order4StencilBackward_z,
                           D1_Order4StencilForward_z);
        ...
        if (mesh->isActive()) {
            ...
            for (unsigned int ele = mesh->getElementLocalBegin();
                 ele < mesh->getElementLocalEnd(); ele++)
                cVec[mesh->getAllElements().size() + ele] =
                    mesh->getAllElements()[ele].getLevel();
        }

        unsigned int numPVars   = 3;
        const char* pVarNames[] = {"F(x)", "dxF(x)", "SxF(x)"};
        double* pVarVal[] = {&(*(funcVal.begin())), &(*(dx_funcVal1.begin())),
                             &(*(dx_funcVal.begin()))};
        ...
        unsigned int s_val[]    = {1u << (m_uiMaxDepth - 1),
                                   1u << (m_uiMaxDepth - 1),
                                   1u << (m_uiMaxDepth - 1)};
        unsigned int s_normal[] = {0, 0, 1};
        mesh->setDomainBounds(pt_min, pt_max);
        io::vtk::mesh2vtu_slice(mesh, s_val, s_normal, "f_val_slice", 2,
                                (const char**)fVarNames, (const double*)fVarVal,
                                numPVars, (const char**)pVarNames,
                                (const double**)pVarVal);
        io::vtk::mesh2vtuFine(
            mesh, "f_val", 2, (const char**)fVarNames, (const double*)fVarVal,
            numPVars, (const char**)pVarNames, (const double**)pVarVal,
            numCVars, cvarNames, (const double**)cVarVal, false);
        ...
        if (mesh->isActive()) {
            ...
            double l2_x =
                normL2(&(*(dx_funcVal1.begin() + nLocalBegin)),
                       &(*(dx_funcVal.begin() + nLocalBegin)),
                       (nLocalEnd - nLocalBegin), mesh->getMPICommunicator());
            double l_inf_x = normLInfty(&(*(dx_funcVal1.begin() + nLocalBegin)),
                                        &(*(dx_funcVal.begin() + nLocalBegin)),
                                        (nLocalEnd - nLocalBegin),
                                        mesh->getMPICommunicator());

            if (!mesh->getMPIRank())
                std::cout << "l2 norm (x derivative): " << l2_x << std::endl;
            if (!mesh->getMPIRank())
                std::cout << "l_inf norm (x derivative): " << l_inf_x
                          << std::endl;

            double l2_y =
                normL2(&(*(dy_funcVal1.begin() + nLocalBegin)),
                       &(*(dy_funcVal.begin() + nLocalBegin)),
                       (nLocalEnd - nLocalBegin), mesh->getMPICommunicator());
            double l_inf_y = normLInfty(&(*(dy_funcVal1.begin() + nLocalBegin)),
                                        &(*(dy_funcVal.begin() + nLocalBegin)),
                                        (nLocalEnd - nLocalBegin),
                                        mesh->getMPICommunicator());

            if (!mesh->getMPIRank())
                std::cout << "l2 norm (y derivative): " << l2_y << std::endl;
            if (!mesh->getMPIRank())
                std::cout << "l_inf norm (y derivative): " << l_inf_y
                          << std::endl;

            double l2_z =
                normL2(&(*(dz_funcVal1.begin() + nLocalBegin)),
                       &(*(dz_funcVal.begin() + nLocalBegin)),
                       (nLocalEnd - nLocalBegin), mesh->getMPICommunicator());
            double l_inf_z = normLInfty(&(*(dz_funcVal1.begin() + nLocalBegin)),
                                        &(*(dz_funcVal.begin() + nLocalBegin)),
                                        (nLocalEnd - nLocalBegin),
                                        mesh->getMPICommunicator());

            if (!mesh->getMPIRank())
                std::cout << "l2 norm (z derivative): " << l2_z << std::endl;
            if (!mesh->getMPIRank())
                std::cout << "l_inf norm (z derivative): " << l_inf_z
                          << std::endl;
        }
    }
...

    {
        ...
        std::function<double(double, double, double)> w_tol_coarsen =
            [wavelet_tol](double x, double y, double z) {
                return wavelet_tol * 1e8;
            };
        std::function<double(double, double, double)> w_tol_refine =
            [wavelet_tol](double x, double y, double z) { return wavelet_tol; };

        // Note: change this to false to preserve the grid generated by the
        // wavelets.
        bool isRefine = true;
        // mesh->isReMeshUnzip((const double **)vars ,(const unsigned int
        // *)vIDs,1,w_tol_coarsen);
        ...
        for (unsigned int v = 0; v < dof; v++)
            std::memcpy(fv2 + v * mesh->getDegOfFreedom(), funcVal.data(),
                        sizeof(double) * mesh->getDegOfFreedom());
        ...
        if (isRefine) {
            ...
            mesh->interGridTransfer(funcVal, newMesh,
                                    ot::INTERGRID_TRANSFER_MODE::INJECTION);

            // igt for dof=2
            mesh->interGridTransfer(
                fv2, newMesh, ot::INTERGRID_TRANSFER_MODE::INJECTION, dof);
        ...
        std::memcpy(funcVal.data(), fv2 + 1 * (mesh->getDegOfFreedom()),
                    sizeof(double) * mesh->getDegOfFreedom());
        ...
        if (isRefine) {
            if (!rank) {
                std::cout << YLW
                          ...
                          << NRM << std::endl;
                std::cout
                    << "        UnZip Test Check (after remesh) Begin     "
                    << std::endl;
                std::cout << YLW
                          << "================================================="
                             "==============================================="
            ...
            ot::test::isUnzipValid(mesh, &(*(funcValUnZip.begin())), func,
                                   1e-3);
            ...
            if (!rank) {
                std::cout << YLW
                          << "================================================="
                             "==============================================="
                          << NRM << std::endl;
                std::cout << "        UnZip Test Check (after remesh) End     "
                          << std::endl;
                std::cout << YLW
                          << "================================================="
                             "==============================================="
        ...
        // unsigned int s_val[] ={1u<<(m_uiMaxDepth-1), 1u<<(m_uiMaxDepth-1),
        // 1u<<(m_uiMaxDepth-1)}; unsigned int s_normal[]={0,0,1};
        mesh->setDomainBounds(pt_min, pt_max);
        // io::vtk::mesh2vtu_slice(mesh, s_val,s_normal, "f_val_slice",2,(const
        // char **)fVarNames,(const double *)fVarVal,numPVars,(const char**
        // )pVarNames,(const double **)pVarVal);
        io::vtk::mesh2vtuFine(mesh, "f_val_rmesh", 0, NULL, NULL, numPVars1,
                              (const char**)pVarNames1,
                              (const double**)pVarVal1);
    }
