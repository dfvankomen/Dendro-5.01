...
bool isSorted(const std::vector<T>& nodes) {
    for (unsigned int i = 1; i < nodes.size(); i++) {
        if (nodes[i] < nodes[i - 1]) {
            std::cout << "\n Local Sort Check failed for: " << nodes[i]
                      << " and " << nodes[i - 1] << std::endl
                      << std::endl;
            return false;
        }
    }
...
bool isSorted_all_pairs(const std::vector<T>& nodes) {
    for (unsigned int i = 0; i < nodes.size(); i++) {
        for (unsigned int j = i + 1; j < nodes.size(); j++) {
            if (nodes[i] > nodes[j]) {
                std::cout << "\n Local Sort_pair Check failed for: " << nodes[i]
                          << " and " << nodes[j] << std::endl
                          << std::endl;
                return false;
            }
        }
    ...
    for (unsigned int i = 1; i < sz; i++) {
        if (nodes[i] < nodes[i - 1]) {
            std::cout << "\n Local Sort Check failed for: " << nodes[i]
                      << " and " << nodes[i - 1] << std::endl
                      << std::endl;
            std::cout << "nodes[i] < nodes[i-1]:" << (nodes[i] < nodes[i - 1])
                      << std::endl;
            std::cout << "nodes[i] > nodes[i-1]:" << (nodes[i] > nodes[i - 1])
                      << std::endl;
            return false;
...
bool isBlockNAN(const T* in, const unsigned int* sz, unsigned int flag) {
    if (flag == 0) {  // implies this is an internal block.
...
bool isUniqueAndSorted(const std::vector<T>& nodes) {
    for (unsigned int i = 1; i < nodes.size(); i++) {
        if (nodes[i] <= nodes[i - 1]) {
            std::cout << "\n Local Sort+Unique Check failed for: " << nodes[i]
                      << " and " << nodes[i - 1] << std::endl;
            std::cout << "nodes[i] <= nodes[i-1]:" << (nodes[i] <= nodes[i - 1])
                      << std::endl;
            std::cout << "nodes[i] >= nodes[i-1]:" << (nodes[i] >= nodes[i - 1])
...
bool containsAncestor(const std::vector<T>& nodes) {
    for (unsigned int i = 1; i < nodes.size(); i++) {
        if (nodes[i - 1].isAncestor(nodes[i])) {
            std::cout << "\n Local contains ancestors failed for: " << nodes[i]
                      << " and " << nodes[i - 1] << std::endl;
            std::cout << "nodes[i] <= nodes[i-1]:" << (nodes[i] <= nodes[i - 1])
                      << std::endl;
            std::cout << "nodes[i] >= nodes[i-1]:" << (nodes[i] >= nodes[i - 1])
...
bool isComplete(const std::vector<T>& nodes) {
    ...
    //  Note: Current implementation of isComplete works only with maxDepth <21
    //  due to capacity of DendroIntL.
    ...
    if (nodes.size() != 0) {
        ...
        for (DendroIntL i = 0; i < nodes.size(); i++) {
            ...
            vol = vol + vol1;
            // if(vol1<0)
            // std::cout<<"vol: "<<vol<<" vol1: "<<vol1 <<" len:  "<<(maxDepth -
            // nodes[i].getLevel())<<" node : "<<nodes[i]<<std::endl;
        }

        bool state = (vol == volMaxDepth);

        if (!state) {
            std::cout << "Volume based on MaxDepth: " << volMaxDepth
                      << std::endl;
            std::cout << "Computed Volume based on OCtants: " << vol
                      << std::endl;
...
template <typename T>
bool checkE2EMapping(const std::vector<unsigned int>& E2EMap,
                     const std::vector<T>& allNodes, unsigned int localBegin,
                     unsigned int localEnd, unsigned int k_s,
        ...
        for (unsigned int k = 1; k <= k_s; k++) {
            if ((myX + k * mySz) < domain_max) {
                ...
                if (lookUPIndex != UINT_MAX) {
                    ...
                    if (!(tmp.isAncestor(lookUp)) && (tmp != lookUp) &&
                        (!lookUp.isAncestor(tmp))) {
                        std::cout
                            << "rank: " << rank << "owner: " << inPtr[i]
                            << " key right: " << tmp << " lookup: " << lookUp
                            << " lookupIndex: " << lookUPIndex << std::endl;
                        return false;
                    }
                }
            }
            ...
            if ((myY + k * mySz) < domain_max) {
                tmp = ot::TreeNode(myX, (myY + k * mySz), myZ,
                                   (OCT_KEY_UP | m_uiMaxDepth), m_uiDim,
                                   m_uiMaxDepth);
                lookUPIndex =
                    E2EMap[i * k_s * numDirections + OCT_DIR_UP * k_s + k - 1];
                // assert(lookUPIndex!=UINT_MAX);
                if (lookUPIndex != UINT_MAX) {
                    assert(lookUPIndex < allNodes.size());
                    lookUp = allNodes[lookUPIndex];
                ...
                if (lookUPIndex != UINT_MAX) {
                    ...
                    if (!(tmp.isAncestor(lookUp)) && (tmp != lookUp) &&
                        ...
                        if (!rank)
                            std::cout
                                << "rank: " << rank
                                << "element ID: " << (i - localBegin)
                                << " element : " << inPtr[i]
                                << " key down: " << tmp << " lookup: " << lookUp
                                << " lookupIndex: " << lookUPIndex << std::endl;
                        return false;
                    }
                }
...
template <typename T>
bool checkE2NMapping(const std::vector<unsigned int>& E2EMap,
                     const std::vector<unsigned int>& E2NMap,
                     const std::vector<T>& allNodes, unsigned int numDirections,
    ...
    // perform iteration over local nodes and their neighbours, to see whether
    // we have any invalid nodes.

    for (unsigned int e = 0; e < allNodes.size(); e++) {
        if (E2NMap[e * numNpE] !=
            UINT_MAX) {  // This is because we invalidate some ghost element
                         // nodes who doesn't have any neighbours.
            for (unsigned int k = 0; k < numNpE; k++) {
                nodeIndex = E2NMap[e * numNpE + k];
                assert(nodeIndex !=
                       UINT_MAX);  // This is to check whether we have partial
                                   // invalidations. Partial invalidations are
                                   // not allowed.
                /*Note : This is true, because we can grantee that
                 * (k/(d+1)+j/(d+1)^2 +i/(d+1)^3 < 1)*/
                elementIndex =
                    nodeIndex /
                    ((elementOrder + 1) * (elementOrder + 1) *
                     (elementOrder + 1));  //(elementOrder + 1) / (elementOrder
                                           //+ 1) / (elementOrder + 1);
                assert(allNodes[elementIndex].getLevel() <=
                       allNodes[e].getLevel());
                if (e != elementIndex) {
                    neighState = false;
                    ...
                    /* if(!neighState)
                     {
                        std::cout<<"e1: "<<e<<" octant:
                     "<<allNodes[e]<<std::endl; for(unsigned int
                     dir=0;dir<numDirections;dir++)
                         {
                             std::cout<<" "<<E2EMap[e*numDirections+dir];
                         }
                         std::cout<<std::endl;
                         std::cout<<"e2: "<<elementIndex<<" octant:
                     "<<allNodes[elementIndex]<<std::endl; for(unsigned int
                     dir=0;dir<numDirections;dir++)
                         {
                             std::cout<<"
                     "<<E2EMap[elementIndex*numDirections+dir];
                         }
                         std::cout<<std::endl;

                     }*/

                    // Note: This is due to the fact that, if DG index of a node
                    // doesn't belong to that element those elements should have
                    // a commom boundary between smaller octant and larger
                    // octant.
                    assert(neighState);
    ...
    if (allLocalsPassed) {
        ...
        if (!nodes.empty()) {
            ...
            if (rank) {
                par::Mpi_Recv<T>(&prev, 1, rank - 1, 0, new_comm, &status);
                if ((prev >= me)) {
                    std::cout << rank << " from isUnique test : prev: " << prev
                              << " me: " << me << std::endl;
                    failedParCheck = true;
                }
            }
        ...
        if (!nodes.empty()) {
            ...
            if (rank) {
                ...
                if ((prev.isAncestor(me))) {
                    std::cout
                        << rank
                        << " par::test::containsAncestor failed: prev: " << prev
                        << " me: " << me << std::endl;
                    failedParCheck = true;
                }
            }
...
bool isComplete(const std::vector<T>& nodes, MPI_Comm comm) {
    ...
    if (nodes.size() != 0) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            if (i < (nodes.size() - 1) && (nodes[i].isAncestor(nodes[i + 1]))) {
                std::cout << "Rank:" << rank << " contains duplicate nodes."
                          << std::endl;
                return false;
            }
            vol += len * len * len;
    ...
    if (!rank) {
        ...
        if (g_vol == (max_len * max_len * max_len)) {
            ...
            std::cout << "Volume of the Complete Octree:"
                      << (max_len * max_len * max_len) << std::endl;
...
bool isBalancedInternal(unsigned int dim, unsigned int maxDepth,
    ...
    for (unsigned int i = 0; i < nodes.size(); i++) {
        ...
        // cout<<RED<<"R"<<NRM<<endl;
        {
            ...
            if (holder.isAncestor(it)) {
                bool found = seq::maxLowerBound(nodes, it.getDFDMorton(),
                                                retIdx, NULL, NULL);
                if (found) {
                    unsigned int retLev = nodes[retIdx].getLevel();
                    unsigned int myLev  = nodes[i].getLevel();
                    if ((it.getAnchor() != nodes[retIdx].getAnchor()) &&
                        (!(nodes[retIdx].isAncestor(it)))) {
                        found = false;
                    } else if ((retLev < myLev) &&
                               (myLev - retLev) > maxLevDiff) {
                ...
                if (!found) {
                    yesBalanced = false;
                    std::cout << nodes[i] << ": (R) ->" << nodes[retIdx]
                              << std::endl;
        ...
        // cout<<RED<<"L"<<NRM<<endl;
        {
            ...
            if (holder.isAncestor(it)) {
                ...
                if (!found) {
                    yesBalanced = false;
                    std::cout << nodes[i] << ": (L) ->" << nodes[retIdx]
                              << std::endl;
                    failedFaces.push_back(nodes[i]);
                    // break;
                }
        ...
        // cout<<RED<<"T"<<NRM<<endl;
        {
            ...
            if (holder.isAncestor(it)) {
                ...
                if (!found) {
                    yesBalanced = false;
                    std::cout << nodes[i] << ": (T) ->" << nodes[retIdx]
                              << std::endl;
                    failedFaces.push_back(nodes[i]);
                    // break;
                }
        ...
        // cout<<RED<<"Bo"<<NRM<<endl;
        {
            ...
            if (holder.isAncestor(it)) {
                ...
                if (!found) {
                    yesBalanced = false;
                    std::cout << nodes[i] << ": (Bo) ->" << nodes[retIdx]
                              << std::endl;
                    failedFaces.push_back(nodes[i]);
                    // break;
                }
        ...
        // cout<<RED<<"F"<<NRM<<endl;
        {
            ...
            if (holder.isAncestor(it)) {
                ...
                if (!found) {
                    yesBalanced = false;
                    std::cout << nodes[i] << ": (F) ->" << nodes[retIdx]
                              << std::endl;
                    failedFaces.push_back(nodes[i]);
                    // break;
                }
        ...
        // cout<<RED<<"Bk"<<NRM<<endl;
        {
            ...
            if (holder.isAncestor(it)) {
                ...
                if (!found) {
                    yesBalanced = false;
                    std::cout << nodes[i] << ": (Bk) ->" << nodes[retIdx]
                              << std::endl;
                    failedFaces.push_back(nodes[i]);
                    // break;
                }
        ...
        if (dim == 3 || incCorn) {
            // cout<<RED<<"TR"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (TR) ->" << nodes[retIdx]
                                  << std::endl;
            ...
            // cout<<RED<<"TL"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (TL) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"BoL"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (BoL) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"BoR"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (BoR) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"RBk"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (RBk) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"TBk"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (TBk) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"LF"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (LF) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"LBk"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (LBk) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"RF"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (RF) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"TF"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (TF) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"BoBk"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (BoBk) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"BoF"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (BoF) ->" << nodes[retIdx]
                                  << std::endl;
                        failedEdges.push_back(nodes[i]);
                        // break;
                    }
        ...
        if (incCorn) {
            // cout<<RED<<"TRBk"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (TRBk) ->" << nodes[retIdx]
                                  << std::endl;
            ...
            // cout<<RED<<"TLF"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (TLF) ->" << nodes[retIdx]
                                  << std::endl;
                        failedCorners.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"BoRBk"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (BoRBk) ->" << nodes[retIdx]
                                  << std::endl;
                        failedCorners.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"TLBk"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (TLBk) ->" << nodes[retIdx]
                                  << std::endl;
                        failedCorners.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"BoRF"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (BoRF) ->" << nodes[retIdx]
                                  << std::endl;
                        failedCorners.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"TRF"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (TRF) ->" << nodes[retIdx]
                                  << std::endl;
                        failedCorners.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"BoLBk"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (BoLBk) ->" << nodes[retIdx]
                                  << std::endl;
                        failedCorners.push_back(nodes[i]);
                        // break;
                    }
            ...
            // cout<<RED<<"BoLF"<<NRM<<endl;
            {
                ...
                if (holder.isAncestor(it)) {
                    ...
                    if (!found) {
                        yesBalanced = false;
                        std::cout << nodes[i] << ": (BoLF) ->" << nodes[retIdx]
                                  << std::endl;
                        failedCorners.push_back(nodes[i]);
                        // break;
                    }
    ...
    if (!yesBalanced) {
        ...
        std::cout << YLW << "Failed Face Cases:" << failedFaces.size() << NRM
                  << std::endl;
        std::cout << YLW << "Failed Edges Cases:" << failedEdges.size() << NRM
                  << std::endl;
        std::cout << YLW << "Failed Corners Cases:" << failedCorners.size()
                  << NRM << std::endl;
        std::cout << BLU
                  << "===============================================" << NRM
                  << std::endl;
...
bool isBalanced(unsigned int dim, unsigned int maxDepth, char* failFileName,
                const std::vector<T>& nodes, bool incCorn,
                unsigned int maxLevDiff) {
    TreeNode root(dim, maxDepth);
    return isBalancedInternal(dim, maxDepth, failFileName, nodes, root, incCorn,
                              maxLevDiff);
}

template <typename T, typename B>
bool isBlockListValid(const std::vector<T>& pNodes, std::vector<B>& blockList,
    ...
    for (unsigned int lev = d_min; lev <= d_max; lev++) {
        ...
        if (numOctLevBlock != numOctLevOctree) {
            ...
            std::cout << "rank: " << rank
                      << " num oct mismatch for lev: " << lev
                      << " numOctLevBlock: " << numOctLevBlock
                      << " numOctLevOctree: " << numOctLevOctree << std::endl;
