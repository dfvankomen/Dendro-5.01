...
std::vector<std::string> inline split(const std::string &str) {
    ...
    while (std::getline(ss, key, ',')) {
        if (!key.empty() && key.back() == '\0')
            key.pop_back();  // last variables contain a '\0', which is
                             // unexpected when comparing to raw string, e.g.
                             // value == "test" will fail when the last
                             // character is '\0'. Therefore we can remove it
...
template <typename T>
std::string to_lower(
    const T &str_) {  // both std::string and std::basic_string_view<char> (for
                      // magic_enum) are using to_lower
...
template <typename T>
inline T get(const std::string &v) {  // remaining types
    ...
    } else if constexpr (std::is_enum<T>::value) {  // case-insensitive enum
                                                    // conversion
...
struct ConvertType : public ConvertBase {
    ...
    void set_default(const std::unique_ptr<ConvertBase> &default_value,
        ...
        if (this->get_type_id() ==
            default_value
                ->get_type_id())  // When the types do not match exactly. resort
...
struct Entry {
    ...
    // Allow both string inputs and direct-type inputs. Where a string-input
    // will be converted like it would when using the commandline, and the
    // direct approach is to simply use the value provided.
    template <typename T>
    ...
    template <typename T>
    operator T &() {
        // Automatically set the default to nullptr for pointer types and empty
        // for optional types
        if constexpr (is_optional<T>::value || std::is_pointer<T>::value ||
            ...
            if (!default_str_.has_value()) {
                ...
                if constexpr (is_optional<T>::value) {
                    data_default =
                        std::make_unique<ConvertType<T>>(T{std::nullopt});
                } else {
                    data_default =
                        std::make_unique<ConvertType<T>>((T) nullptr);
                }
            }
        }
    ...
    template <typename T>
    operator T() {
    }  // When you get here  because you received an error, make sure all
   ...
   private:
    ...
    void _convert(const std::string &value) {
        try {
            ...
            error = "Invalid argument, could not convert \"" + value +
                    "\" for " + _get_keys() + " (" + help + ")";
    ...
    void _apply_default() {
        ...
        if (data_default != nullptr) {
            ...
            datap->set_default(data_default, *default_str_);
                       ...
                       .has_value()) {  // in cases where a string is provided
                                        // to the `set_default` function
    ...
    [[nodiscard]] std::string info() const {
        ...
        const std::string implicit_value =
            implicit_value_.has_value()
                ? "implicit: \"" + *implicit_value_ + "\", "
                : "";
...
class Args {
   private:
    ...
    bool has_options() {
        return std::find_if(all_entries.begin(), all_entries.end(), [](auto e) {
                   return e->type != Entry::ARG;
               }) != all_entries.end();
    };
   ...
   public:
    ...
    /* Add a *named* positional argument, the order in which it is defined
     * equals the order in which they are being read. key : The name of the
     * argument, otherwise arg_<position> will be used help : Description of the
     * variable
     *
     * Returns a reference to the Entry, which will collapse into the requested
     * type in `Entry::operator T()`
    ...
    Entry &arg(const std::string &key, const std::string &help) {
        ...
        // Increasing _arg_idx, so that arg2 will be arg_2, irregardless of
        // whether it is preceded by other positional arguments
        _arg_idx++;
     ...
     * key : A comma-separated string, e.g. "k,key", which denotes the short
     * (-k) and long(--key) keys_ help : Description of the variable
    ...
    /* Add a a subcommand
     * command : name of the subcommand, e.g. 'commit', if you wish to implement
     * a function like 'git commit'
     *
     * Returns a reference to the Entry, which will collapse into the requested
     * type in `Entry::operator T()` Expected type *Must* be an std::shared_ptr
     * of derivative of the argparse::Args class
     */
    ...
    virtual void welcome() {}  // Allow to overwrite the `welcome` function to
                               // add a welcome-message to the help output
    virtual void help() {
        welcome();
    ...
    /* parse all parameters and also check for the help_flag which was set in
     * this constructor Upon error, it will print the error and exit immediately
     * if validation_action is ValidationAction::EXIT_ON_ERROR
     */
    void parse(int argc, const char *const *argv, const bool &raise_on_error) {
        ...
        argc = parse_subcommands();  // argc_ is the number of arguments that
                                     // should be parsed after the subcommand
        ...
        auto is_value = [&](const size_t &i) -> bool {
            return params.size() > i &&
                   (params[i][0] != '-' ||
                    (params[i].size() > 1 &&
                     std::isdigit(
                         params[i][1])));  // check for number to not
                                           // accidentally mark negative numbers
                                           // as non-parameter
        };
        auto parse_param = [&](size_t &i, const std::string &key,
                               const bool is_short,
            ...
            if (itt != kwarg_entries.end()) {
                ...
                if (equal_value.has_value()) {
                ...
                } else if (!is_short) {  // short values are not allowed to look
                    ...
                    if (is_value(i + 1)) {
                        ...
                        entry->_convert("");  // for multiargument parameters,
                                              // return an empty vector when not
                                              // passing any more values
                    } else {
        ...
        auto add_param = [&](size_t &i, const size_t &start) {
            size_t eq_idx = params[i].find(
                '=');  // check if value was passed using the '=' sign
        ...
        for (size_t i = 0; i < params.size(); i++) {
            if (!is_value(i)) {
                if (params[i].size() > 1 && params[i][1] == '-') {  // long --
                    ...
                    for (size_t j = 1; j < j_end;
                         j++) {  // add possible other flags
        ...
        // Parse all the positional arguments, making sure multi_argument
        // positional arguments are processed last to enable arguments
        // afterwards
        size_t arg_i = 0;
        for (; arg_i < arg_entries.size() &&
               !arg_entries[arg_i]->_is_multi_argument;
             arg_i++) {  // iterate over positional arguments until a
                         // multi-argument is found
        ...
        for (size_t j_end = arg_entries.size() - arg_i; arg_j <= j_end;
             arg_j++) {  // iterate from back to front, to ensure
                         // non-multi-arguments in the front and back are given
                         // preference
            size_t flat_idx = arguments_flat.size() - arg_j;
            if (flat_idx < arguments_flat.size() && flat_idx >= arg_i) {
                if (arg_entries[arg_entries.size() - arg_j]
                        ->_is_multi_argument) {
    ...
    void print() const {
        for (const auto &entry : all_entries) {
            std::string snip = entry->type == Entry::ARG
                                   ? "(" +
                                         (entry->help.size() > 10
                                              ? entry->help.substr(0, 7) + "..."
                                              : entry->help) +
                                         ")"
                                   : "";
            cout << setw(21) << entry->_get_keys() + snip << " : "
                 << (entry->is_set_by_user
                         ? bold(entry->value_.value_or("null"))
