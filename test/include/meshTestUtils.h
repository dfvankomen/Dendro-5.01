 ...
 * */
template <typename T>
bool isElementalNodalValuesValid(ot::Mesh* pMesh, T* vec,
                                 std::function<T(T, T, T)> func, double tol);

/**
 * @brief check whether the element to hanging node contribution is computed
 * correctly. Test: Accumilation of the varible vector is equal to the integral
 * of that. Hence we compute the analytical integration and compare it to the
 * computed one.
 * @param[in] pMesh: mesh
 ...
 * */
template <typename T>
bool isElementalContributionValid(ot::Mesh* pMesh,
                                  std::function<T(T, T, T)> func,
                                  std::function<T(T, T, T)> Ifunc, double tol);

/**
 *@brief check whether the unzipped values are correct (Note: Need to unzip
 *after performing the ghost exchange.)
 * @param[in] pMesh : pMesh
 ...
 * */
template <typename T>
bool isUnzipValid(ot::Mesh* pMesh, T* unzipVec, std::function<T(T, T, T)> fn,
                  T tol);
...
/**
 * @breif : Test whether a known function func, can be correctly interpolated to
 * the surface of a sphere.
 * @param[in] mesh : input mesh
 * @paran[in] zipIn: mesh representation of the function func
 * @param[in] func: testing function
 * @param[in] r: radius of the sphere from
 * (1u<<(maxDepth-1),1u<<(maxDepth-1),1u<<(maxDepth-1))
 * @param[in] tolerance: tolerance for interpolation check.
 *
 * */

template <typename T>
bool isInterpToSphereValid(const ot::Mesh* mesh, const T* zipIn,
                           std::function<T(T, T, T)> func, double r,
                           const double* dMin, const double* dMax,
                           double tolerance);

template <typename T>
bool isSphereInterpValid(ot::Mesh* pMesh, T* vec,
                         std::function<double(double, double, double)> func,
                         double r, double tol, Point d_min, Point d_max);
 ...
 */
template <typename T>
bool isSubScatterMapValid(ot::Mesh* const pMesh, ot::SubScatterMap* const subSM,
                          T* const vec);
...
template <typename T>
bool ot::test::isElementalNodalValuesValid(ot::Mesh* pMesh, T* vec,
                                           std::function<T(T, T, T)> func,
                                           double tol) {
        ...
        for (unsigned int k = 0; k < (eleOrder + 1); k++)
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    ...
                    if ((i > 1 && i < eleOrder) && (j > 1 && j < eleOrder) &&
                        (k > 1 && k < eleOrder) &&
                        fabs(nodalValues[k * (eleOrder + 1) * (eleOrder + 1) +
                                         j * (eleOrder + 1) + i] -
                             func(x, y, z)) > tol) {
                        isValid = false;
                        pMesh->dg2eijk(
                        ...
                        std::cout
                            << " nvalue: "
                            << nodalValues[k * (eleOrder + 1) * (eleOrder + 1) +
                                           j * (eleOrder + 1) + i]
                            << " actual: " << func(x, y, z) << " ele: " << ele
                            << " (i,j,k) : (" << i << "," << j << "," << k
                            << ") : element : " << pNodes[ele] << " (x,y,z): ("
                            << x << "," << y << "," << z << ")"
                            << " owner: " << pNodes[owner] << " (i,j,k): ("
                            << ix << "," << jy << "," << kz << " )"
                            << std::endl;
                    }

                    if ((i == 0) && (j == 0) && (k >= 0 && k <= eleOrder) &&
                        fabs(nodalValues[k * (eleOrder + 1) * (eleOrder + 1) +
                                         j * (eleOrder + 1) + i] -
                             func(x, y, z)) > tol) {
                        isValid = false;
...
template <typename T>
bool ot::test::isUnzipValid(ot::Mesh* pMesh, T* unzipVec,
                            std::function<T(T, T, T)> fn, T tol) {
    if (!(pMesh->isActive())) return true;
    ...
    for (unsigned int blk = 0; blk < blkList.size(); blk++) {
        ...
        // to check if the specific non-boundary block has non at any unzip
        // point. below we specically check the each boundary case accordingly.
        if (!bflag)
            for (unsigned int k = 0; k < lz; k++)
                for (unsigned int j = 0; j < ly; j++)
                    for (unsigned int i = 0; i < lx; i++) {
                        ...
                        if (fabs(unzipVec[offset + k * ly * lx + j * lx + i] -
                                 fn(x, y, z)) > tol) {
                            valid = false;
                            std::cout
                                << "rank: " << rank << "blk: " << blkNode
                                << " block unzip error : unzip value:  "
                                << unzipVec[offset + k * ly * lx + j * lx + i]
                                << "\t func(x,y,z): " << fn(x, y, z)
                                << " (i,j,k): (" << i << ", " << j << "," << k
                                << " )"
                                << "sz: " << lx << std::endl;
            ...
            for (unsigned int j = jb; j < je; j++)
                for (unsigned int i = ib; i < ie; i++) {
                    ...
                    if (fabs(unzipVec[offset + k * ly * lx + j * lx + i] -
                        ...
                        std::cout
                            << "rank: " << rank << "blk: " << blkNode
                            << " block[internal] unzip error : unzip value:  "
                            << unzipVec[offset + k * ly * lx + j * lx + i]
                            << "\t func(x,y,z): " << fn(x, y, z)
                            << " (i,j,k): (" << i << ", " << j << "," << k
                            << " )"
                ...
                for (unsigned int j = jb; j < je; j++)
                    for (unsigned int i = ib; i < ie; i++) {
                        ...
                        if (fabs(unzipVec[offset + k * ly * lx + j * lx + i] -
                            ...
                            std::cout
                                << "rank: " << rank << "blk: " << blkNode
                                << " block[LEFT] unzip error : unzip value:  "
                                << unzipVec[offset + k * ly * lx + j * lx + i]
                                << "\t func(x,y,z): " << fn(x, y, z)
                                << " (i,j,k): (" << i << ", " << j << "," << k
                                << " )"
                ...
                for (unsigned int k = kb; k < ke; k++)
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (fabs(unzipVec[offset + k * ly * lx + j * lx +
                                              i] -
                                     fn(x, y, z)) > tol) {
                                valid = false;
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << " block[LEFT_DOWN] unzip error : unzip "
                                       "value:  "
                    ...
                    for (unsigned int k = kb; k < ke; k++)
                        for (unsigned int j = jb; j < je; j++)
                            for (unsigned int i = ib; i < ie; i++) {
                                ...
                                if (fabs(unzipVec[offset + k * ly * lx +
                                                  j * lx + i] -
                                         fn(x, y, z)) > tol) {
                                    valid = false;
                                    std::cout
                                        << "rank: " << rank
                                        << "blk: " << blkNode
                                        << " block[LEFT_DOWN_BACK] unzip error "
                                           ": unzip value:  "
                    ...
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (fabs(unzipVec[offset + k * ly * lx + j * lx +
                                ...
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << " block[LEFT_UP] unzip error : unzip "
                                       "value:  "
                                    << unzipVec[offset + k * ly * lx + j * lx +
                                                i]
                                    << "\t func(x,y,z): " << fn(x, y, z)
                            ...
                            for (unsigned int i = ib; i < ie; i++) {
                                ...
                                if (fabs(unzipVec[offset + k * ly * lx +
                                    ...
                                    std::cout
                                        << "rank: " << rank
                                        << "blk: " << blkNode
                                        << " block[LEFT_UP_BACK] unzip error : "
                                           "unzip value:  "
                                        << unzipVec[offset + k * ly * lx +
                                                    j * lx + i]
                                        << "\t func(x,y,z): " << fn(x, y, z)
                            ...
                            for (unsigned int i = ib; i < ie; i++) {
                                ...
                                if (fabs(unzipVec[offset + k * ly * lx +
                                    ...
                                    std::cout
                                        << "rank: " << rank
                                        << "blk: " << blkNode
                                        << " block[LEFT_UP_FRONT] unzip error "
                                           ": unzip value:  "
                                        << unzipVec[offset + k * ly * lx +
                                                    j * lx + i]
                                        << "\t func(x,y,z): " << fn(x, y, z)
                    ...
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (fabs(unzipVec[offset + k * ly * lx + j * lx +
                                ...
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << " block[LEFT_BACK] unzip error : unzip "
                                       "value:  "
                                    << unzipVec[offset + k * ly * lx + j * lx +
                                                i]
                                    << "\t func(x,y,z): " << fn(x, y, z)
                ...
                for (unsigned int j = jb; j < je; j++)
                    for (unsigned int i = ib; i < ie; i++) {
                        ...
                        if (fabs(unzipVec[offset + k * ly * lx + j * lx + i] -
                            ...
                            std::cout
                                << "rank: " << rank << "blk: " << blkNode
                                << " block[RIGHT] unzip error : unzip value:  "
                                << unzipVec[offset + k * ly * lx + j * lx + i]
                                << "\t func(x,y,z): " << fn(x, y, z)
                                << " (i,j,k): (" << i << ", " << j << "," << k
                                << " )"
                    ...
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (fabs(unzipVec[offset + k * ly * lx + j * lx +
                                ...
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << "  block[RIGHT_UP] unzip error : unzip "
                                       "value:  "
                                    << unzipVec[offset + k * ly * lx + j * lx +
                                                i]
                                    << "\t func(x,y,z): " << fn(x, y, z)
                            ...
                            for (unsigned int i = ib; i < ie; i++) {
                                ...
                                if (fabs(unzipVec[offset + k * ly * lx +
                                    ...
                                    std::cout
                                        << "rank: " << rank
                                        << "blk: " << blkNode
                                        << "  block[RIGHT_UP_BACK] unzip error "
                                           ": unzip value:  "
                                        << unzipVec[offset + k * ly * lx +
                                                    j * lx + i]
                                        << "\t func(x,y,z): " << fn(x, y, z)
                    ...
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (fabs(unzipVec[offset + k * ly * lx + j * lx +
                                ...
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << " block[RIGHT_BACK] unzip error : unzip "
                                       "value:  "
                                    << unzipVec[offset + k * ly * lx + j * lx +
                                                i]
                                    << "\t func(x,y,z): " << fn(x, y, z)
                ...
                for (unsigned int j = jb; j < je; j++)
                    for (unsigned int i = ib; i < ie; i++) {
                        ...
                        if (fabs(unzipVec[offset + k * ly * lx + j * lx + i] -
                            ...
                            std::cout
                                << "rank: " << rank << "blk: " << blkNode
                                << " block[DOWN] unzip error : unzip value:  "
                                << unzipVec[offset + k * ly * lx + j * lx + i]
                                << "\t func(x,y,z): " << fn(x, y, z)
                                << " (i,j,k): (" << i << ", " << j << "," << k
                                << " )"
                    ...
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (fabs(unzipVec[offset + k * ly * lx + j * lx +
                                ...
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << " block[DOWN_BACK] unzip error : unzip "
                                       "value:  "
                                    << unzipVec[offset + k * ly * lx + j * lx +
                                                i]
                                    << "\t func(x,y,z): " << fn(x, y, z)
                    ...
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (fabs(unzipVec[offset + k * ly * lx + j * lx +
                                ...
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << " block[DOWN_FRONT] unzip error : unzip "
                                       "value:  "
                                    << unzipVec[offset + k * ly * lx + j * lx +
                                                i]
                                    << "\t func(x,y,z): " << fn(x, y, z)
                ...
                for (unsigned int j = jb; j < je; j++)
                    for (unsigned int i = ib; i < ie; i++) {
                        ...
                        if (fabs(unzipVec[offset + k * ly * lx + j * lx + i] -
                            ...
                            std::cout
                                << "rank: " << rank << "blk: " << blkNode
                                << " block[UP] unzip error : unzip value:  "
                                << unzipVec[offset + k * ly * lx + j * lx + i]
                                << "\t func(x,y,z): " << fn(x, y, z)
                                << " (i,j,k): (" << i << ", " << j << "," << k
                                << " )"
                    ...
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (fabs(unzipVec[offset + k * ly * lx + j * lx +
                                ...
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << " block[UP_BACK] unzip error : unzip "
                                       "value:  "
                                    << unzipVec[offset + k * ly * lx + j * lx +
                                                i]
                                    << "\t func(x,y,z): " << fn(x, y, z)
                    ...
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (fabs(unzipVec[offset + k * ly * lx + j * lx +
                                ...
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << " block[UP_FRONT] unzip error : unzip "
                                       "value:  "
                                    << unzipVec[offset + k * ly * lx + j * lx +
                                                i]
                                    << "\t func(x,y,z): " << fn(x, y, z)
                ...
                for (unsigned int j = jb; j < je; j++)
                    for (unsigned int i = ib; i < ie; i++) {
                        ...
                        if (fabs(unzipVec[offset + k * ly * lx + j * lx + i] -
                            ...
                            std::cout
                                << "rank: " << rank << "blk: " << blkNode
                                << " block[BACK] unzip error : unzip value:  "
                                << unzipVec[offset + k * ly * lx + j * lx + i]
                                << "\t func(x,y,z): " << fn(x, y, z)
                                << " (i,j,k): (" << i << ", " << j << "," << k
                                << " )"
                ...
                for (unsigned int j = jb; j < je; j++)
                    for (unsigned int i = ib; i < ie; i++) {
                        ...
                        if (fabs(unzipVec[offset + k * ly * lx + j * lx + i] -
                            ...
                            std::cout
                                << "rank: " << rank << "blk: " << blkNode
                                << " block[FRONT] unzip error : unzip value:  "
                                << unzipVec[offset + k * ly * lx + j * lx + i]
                                << "\t func(x,y,z): " << fn(x, y, z)
                                << " (i,j,k): (" << i << ", " << j << "," << k
                                << " )"
                ...
                for (unsigned int i = ib; i < ie; i++) {
                    ...
                    if (std::isnan(
                        ...
                        std::cout << "rank: " << rank << "blk: " << blkNode
                                  << " block[internal] unzip error : unzip "
                                     "value: NAN "
                                  << " (i,j,k): (" << i << ", " << j << "," << k
                                  << " )"
                                  << "sz: " << lx << std::endl;
                ...
                for (unsigned int k = kb; k < ke; k++)
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (std::isnan(unzipVec[offset + k * ly * lx +
                                                    j * lx + i])) {
                                valid = false;
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                        ...
                        for (unsigned int j = jb; j < je; j++)
                            for (unsigned int i = ib; i < ie; i++) {
                                ...
                                if (std::isnan(unzipVec[offset + k * ly * lx +
                                    ...
                                    std::cout << "rank: " << rank
                                              << "blk: " << blkNode
                                              << " block[left_down_back] unzip "
                                                 "error : unzip value: NAN "
                                              << " (i,j,k): (" << i << ", " << j
                                              << "," << k << " )"
                                              << "sz: " << lx << std::endl;
                                }
                    ...
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (std::isnan(unzipVec[offset + k * ly * lx +
                                ...
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << " block[left_front] unzip error : unzip "
                                       "value: NAN "
                                    << " (i,j,k): (" << i << ", " << j << ","
                                    << k << " )"
                                    << "sz: " << lx << std::endl;
                    ...
                    for (unsigned int j = jb; j < je; j++)
                        for (unsigned int i = ib; i < ie; i++) {
                            ...
                            if (std::isnan(unzipVec[offset + k * ly * lx +
                                ...
                                std::cout
                                    << "rank: " << rank << "blk: " << blkNode
                                    << " block[right_down] unzip error : unzip "
                                       "value: NAN "
                                    << " (i,j,k): (" << i << ", " << j << ","
                                    << k << " )"
                                    << "sz: " << lx << std::endl;
...
bool ot::test::isZipNAN(ot::Mesh* pMesh, T* zipVec) {
    ...
    for (unsigned int i = nodeLocalBegin; i < nodeLocalEnd; i++) {
        if (std::isnan(zipVec[i])) {
            std::cout << "rank: " << activeRank << " node local: " << i
                      << " is nan" << std::endl;
            assert(false);
        }
    }
...
template <typename T>
bool ot::test::isElementalContributionValid(ot::Mesh* pMesh,
                                            std::function<T(T, T, T)> func,
                                            std::function<T(T, T, T)> Ifunc,
                                            double tol) {
    if (pMesh->isActive()) {
        ...
        par::Mpi_Reduce(&integralVal, &integralVal_g, 1, MPI_SUM, 0,
                        commActive);
        /*char fPrefix[256];
...
template <typename T>
bool ot::test::isInterpToSphereValid(const ot::Mesh* mesh, const T* zipIn,
                                     std::function<T(T, T, T)> func, double r,
                                     const double* dMin, const double* dMax,
                                     double tolerance) {
    ...
    if (mesh->isActive()) {
        ...
        for (unsigned int pts = 0; pts < numPts; pts++) {
            coords[3 * pts + 0] =
                (r * sin(LEBEDEV_025_THETA[pts]) * cos(LEBEDEV_025_PHI[pts]) -
                 dMin[0]) *
                (1u << m_uiMaxDepth) / (dMax[0] - dMin[0]);
            coords[3 * pts + 1] =
                (r * sin(LEBEDEV_025_THETA[pts]) * sin(LEBEDEV_025_PHI[pts]) -
                 dMin[1]) *
                (1u << m_uiMaxDepth) / (dMax[1] - dMin[1]);
            coords[3 * pts + 2] = (r * cos(LEBEDEV_025_THETA[pts]) - dMin[2]) *
                                  (1u << m_uiMaxDepth) / (dMax[2] - dMin[2]);

            // std::cout<<"rank: "<<rankActive<<" x: "<<coords[3*pts + 0]<<" y:
            // "<<coords[3*pts + 1]<<" z: "<<coords[3*pts + 2]<<std::endl;
        }
        ...
        ot::da::interpolateToCoords(mesh, zipIn, &(*(coords.begin())),
                                    coords.size(), &(*(interpValues.begin())),
                                    validIndex);
 ...
 */
template <typename T>
bool ot::test::isSphereInterpValid(
    ot::Mesh* pMesh, T* vec, std::function<double(double, double, double)> func,
    double r, double tol, Point d_min, Point d_max) {
    ...
    ot::da::interpolateToCoords(pMesh, vec, &(*(coords.begin())), coords.size(),
                                ...
                                &(*(interpVal.begin())), validIndex);
    const double gridRangeX =
        grid_limit[1].x() - grid_limit[0].x();  // x range of the grid.
    const double gridRangeY =
        grid_limit[1].y() - grid_limit[0].y();  // y range of the grid.
    const double gridRangeZ =
        grid_limit[1].z() - grid_limit[0].z();  // z range of the grid

    const double domainRangeX =
        domain_limit[1].x() - domain_limit[0].x();  // x range of the domain.
    const double domainRangeY =
        domain_limit[1].y() - domain_limit[0].y();  // y range of the domain.
    const double domainRangeZ =
        domain_limit[1].z() - domain_limit[0].z();  // z range of the domain.
    ...
    if (pMesh->isActive()) {
        for (unsigned int i = 0; i < numPts_theta; i++)
            for (unsigned int j = 0; j < numPts_phi; j++) {
                if (count < validIndex.size() &&
                    validIndex[count] == (i * numPts_phi + j)) {
                    double x =
                        grid_limit[0].x() +
                        ((coords[3 * validIndex[count] + 0] - d_min.x()) *
                         (gridRangeX / domainRangeX));
                    double y =
                        grid_limit[0].y() +
                        ((coords[3 * validIndex[count] + 1] - d_min.y()) *
                         (gridRangeY / domainRangeY));
                    double z =
                        grid_limit[0].z() +
                        ((coords[3 * validIndex[count] + 2] - d_min.z()) *
                         (gridRangeZ / domainRangeZ));

                    if (fabs(interpVal[validIndex[count]] - func(x, y, z)) >
                        tol) {
                        std::cout
                            << "rank: " << pMesh->getMPIRank()
                            << " intepVal: " << interpVal[validIndex[count]]
                            << " actual val : " << func(x, y, z) << " diff : "
                            << fabs(interpVal[validIndex[count]] -
                                    func(x, y, z))
                            << std::endl;
                    }
                    ...
                    sum += (r * r) * sin(i * theta_res) * sin(i * theta_res) *
                           theta_res * phi_res;
                }
            }
    }
    ...
    double sum_g = 0;
    par::Mpi_Reduce(&sum, &sum_g, 1, MPI_SUM, 0,
                    pMesh->getMPIGlobalCommunicator());
    if (!(pMesh->getMPIRankGlobal())) {
        std::cout << " integration value: " << sum_g
...
template <typename T>
bool ot::test::isSubScatterMapValid(ot::Mesh* const pMesh,
                                    ot::SubScatterMap* const subSM,
                                    T* const vec) {
    // todo (milinda): This is not a strong test this is a weak test to check
    // the accuracy of the level wise ghost sync
    ...
    if (pMesh->isActive()) {
        for (unsigned int n = pMesh->getNodeLocalBegin();
             n < pMesh->getNodeLocalEnd(); n++)
            uvec[n] = vec[n];
    ...
    par::Mpi_Allreduce(&state, &state_g, 1, par::Mpi_datatype<bool>::LAND(),
                       pMesh->getMPIGlobalCommunicator());
...
bool isDGGhostValid(ot::Mesh* pMesh, T* vec,
    ...
    if (pMesh->getMPICommSizeGlobal() > 1 && pMesh->isActive()) {
        const std::vector<unsigned int> recvEleOffset =
            pMesh->getElementRecvOffsets();
        const std::vector<unsigned int> recvEleCounts =
            pMesh->getElementRecvCounts();
