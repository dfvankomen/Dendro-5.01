 ...
 *@brief The purpose of this file is to check the E2N mapping , is correct. We
 *generate a complete octree and compute number of tasks using p number of
 *processors, then number of actual variables should be equal to the number of
 *actual variables in the sequential case.
 */
//
...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        if (!rank)
            std::cout << "Usage :" << argv[0] << " inp  numPts "
                      << " dim "
                      << " maxDepth "
                      << " tol  "
                      << " distribution (default =0) (0- Normal 1- Uniform 2- "
                         "LogarithmicNormal) genPts(default=1) SplitterFixK "
                         "(default =2)"
                      << std::endl;
        return -1;
    }
    ...
    if (argc > 6) distribution = atoi(argv[6]);

    if (argc > 7) genPts = (bool)atoi(argv[7]);

    if (argc > 8) sf_k = atoi(argv[8]);

    if (!rank) std::cout << "sf parameter: " << sf_k << std::endl;

    _InitializeHcurve(dim);
    if (!rank)
        std::cout << "Initialized H-Curves for dimension " << dim << std::endl;
    ...
    if (npes == 1) {
        ...
        // Read pts from files
        if (!rank) {
            std::cout << RED " Reading  " << ptsFile << NRM
                      << std::endl;  // Point size
        }
        IO::readPtsFromFile(pFile, ptsFromFile);

        if (!rank) {
            std::cout << GRN " Finished reading  " << ptsFile << NRM
                      << std::endl;  // Point size
        }
        pts2Octants(tmpNodes, &(*(ptsFromFile.begin())), totPts, dim,
                    maxDepth);  // generate octants from the points.
        ptsFromFile.clear();
        ...
        if (!rank)
            std::cout << "This test is intended to run in parallel. "
                      << std::endl;
        ...
        std::vector<ot::TreeNode> tmpNodes;
        m_uiMaxDepth  = maxDepth;  // this is needed because we increase this
                                   // when we embedded the tree.
        ...
        if (!rank)
            std::cout << RED << "Remove duplicates begin" << NRM << std::endl;
        auto t1 = MPI_Wtime();
        SFC::parSort::SFC_treeSort(
            tmpNodes, pNodesSorted, pNodesConstructed, pNodesBalanced, tol,
            maxDepth, root, ROOT_ROT_ID, 1, TS_REMOVE_DUPLICATES, sf_k, comm);
        auto t2 = MPI_Wtime();
        t_rd    = t2 - t1;
        if (!rank)
            std::cout << RED << "Remove duplicates end" << NRM << std::endl;
        tmpNodes.clear();
        ...
        if (!rank) {
            std::cout << YLW << "Number of unique octants: " << numUniqueOct
                      << NRM << std::endl;
            std::cout << YLW << "Remove duplicates time(max): " << t_rd_g[2]
                      << NRM << std::endl;
        }
        ...
        if (!rank)
            std::cout << RED << "Tree construction begin" << NRM << std::endl;
        t1 = MPI_Wtime();
        SFC::parSort::SFC_treeSort(
            pNodesSorted, pNodesSorted, pNodesConstructed, pNodesBalanced, tol,
            maxDepth, root, ROOT_ROT_ID, 1, TS_CONSTRUCT_OCTREE, sf_k, comm);
        t2     = MPI_Wtime();
        t_cons = t2 - t1;
        if (!rank)
            std::cout << RED << "Tree construction end" << NRM << std::endl;
        pNodesSorted.clear();
        ...
        numConsOct = globalSz;
        if (!rank) {
            std::cout << YLW << "Number of unbalanced octants: " << numConsOct
                      << NRM << std::endl;
            std::cout << YLW << "Tree construction time(max): " << t_cons_g[2]
                      << NRM << std::endl;
        }
        ...
        if (!rank) {
            std::cout << YLW << "Number of balanced octants: " << numBalOct
                      << NRM << std::endl;
            std::cout << YLW << " Tree balancing time (max): " << t_bal_g[2]
                      << NRM << std::endl;
        }
        MPI_Barrier(comm);
        ...
        /*  std::vector<ot::Block> blockList;
          ...
          //std::cout<<"rank: "<<rank<<" block list size: "<<blockList.size()<<"
        ...
        MPI_Allgatherv(&(*(pNodesBalanced.begin())),localBalSz,MPI_BYTE,&(*(pNodesBalanced_global.begin())),(int
        *)recvCount,(int *)recvCountOffset,MPI_BYTE,comm);
        ...
        if (!rank)
            std::cout << RED << "mesh generation begin" << NRM << std::endl;
        t1 = MPI_Wtime();  // std::chrono::high_resolution_clock::now();
        ...
        if (!rank) {
            std::cout << YLW << "Mesh generation time (max): " << t_mesh_g[2]
                      << NRM << std::endl;
        }
        ...
        if (!rank)
            std::cout << "Staring sequential run to see whether we get the "
                         "same number of local nodes "
                      << std::endl;
        ...
        unsigned int* nodeNeigh =
            new unsigned int[mesh.getNumNodesPerElement()];
        unsigned int* nodeNeigh1 =
            new unsigned int[mesh.getNumNodesPerElement()];
        ...
        while (mesh1.nextAvailable<ot::WaveletDA::INDEPENDENT>() &&
               (elePtr[mesh.currentIndex()] != elePtr1[mesh1.currentIndex()])) {
        ...
        while (mesh.nextAvailable<ot::WaveletDA::INDEPENDENT>() &&
            ...
            // std::cout<<"rank : "<<rank<<" checking: "<<mesh.currentIndex()<<"
            // and "<<mesh1.currentIndex()<<std::endl;
            assert(elePtr[mesh.currentIndex()] ==
                   elePtr1[mesh1.currentIndex()]);
            ...
            for (unsigned int dir = 0; dir < mesh.getNumDirections(); dir++) {
                if (eleNeig[dir] == UINT_MAX) {
                    ...
                    if (elePtr[eleNeig[dir]] != elePtr1[eleNeig1[dir]]) {
                        std::cout
                            << "rank: " << rank << " E2E mismatch: " << dir
                            << " element: " << elePtr[mesh.currentIndex()]
                            << " & element: " << elePtr1[mesh1.currentIndex()]
                            << " neighbour: " << elePtr[eleNeig[dir]] << " & "
                            << elePtr1[eleNeig1[dir]] << std::endl;
                    }
                    assert(elePtr[eleNeig[dir]] == elePtr1[eleNeig1[dir]]);
                }
            }

            // NOTE: !!! To use the following test E2N mapping should be in DG
            // indexing after removing duplicates
            /* for(unsigned int node=0;node<mesh.getNumNodesPerElement();node++)
             {
                 ...
                 if(((x1+i1*sz1)!=(x+i*sz)) || ((y1+j1*sz1)!=(y+j*sz))||
             ((z1+k1*sz1)!=(z+k*sz))){
                     currentEle.push_back(elePtr[mesh.currentIndex()]);
                     owner.push_back(elePtr[e]);
                     owner1.push_back(elePtr1[e1]);
                     std::cout<<"[E2N Mapping Failed: ] rank "<<rank <<"E2N
             failed for :" <<elePtr[mesh.currentIndex()]<<" node : "<<node<<"
             element e1: "<<e1<<" : "<<elePtr1[e1] <<" element e: "<<e<<" :
             "<<elePtr[e]<<" x1: "<<(x1+i1*sz1)<<" x: "<<(x+i*sz)<< " y1:
             "<<(y1+i1*sz1)<<" y: "<<(y+i*sz)<< " z1: "<<(z1+i1*sz1)<<" z:
             "<<(z+i*sz)<<std::endl;

                     std::cout<<"E2E mapping for failed element:
             "<<elePtr[mesh.currentIndex()]<<": "; for (unsigned int dir = 0;
             dir < mesh.getNumDirections(); dir++)
                     {
                         std::cout<<" ("<<elePtr[eleNeig[dir]]<<",
             "<<elePtr1[eleNeig1[dir]]<<" ) ";
        ...
        // summation of number of local nodes in each processor should be equal
        // to the number of local nodes in the sequential case.
        if (!rank)
            std::cout << "seq: mesh local Nodes: "
                      << mesh1.getNumLocalMeshNodes()
                      << " par: mesh local nodes: " << localNodes_g
                      << std::endl;
