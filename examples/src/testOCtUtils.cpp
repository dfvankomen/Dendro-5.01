...
int readPtsFromFile(char* filename, std::vector<double>& pts) {
    ...
    // std::cout << __func__ << ": size " << temp << ", " << pts.size() <<
    // std::endl;
    return 1;
}  // end function
...
int main(int argc, char** argv) {
    ...
    if (!rank)
        std::cout << "Initialized Hcurves for dimention " << dim << std::endl;
    ...
    if (!rank)
        std::cout << "finished generating " << dim << " dimensional points"
                  << std::endl;

    // Read pts from files
    /*if (!rank) {
        std::cout << RED " Reading  " << argv[1] << NRM << std::endl; // Point
    size
    }
    ...
    if (!rank) {
        std::cout << GRN " Finished reading  " << argv[1] << NRM << std::endl;
    // Point size
    }
    ...
    for (DendroIntL i = 0; i < totPts; i += 3) {
        if ((pts[i] > 0.0) &&
            (pts[i + 1] > 0.0)
            && (pts[i + 2] > 0.0) &&
            (((unsigned int) (pts[i] * ((double) (1u << maxDepth)))) < (1u <<
    maxDepth)) &&
            (((unsigned int) (pts[i + 1] * ((double) (1u << maxDepth)))) < (1u
    << maxDepth)) &&
            (((unsigned int) (pts[i + 2] * ((double) (1u << maxDepth)))) < (1u
    << maxDepth))) {

            tmpNodes.push_back(ot::TreeNode((unsigned int) (pts[i] * (double)
    (1u << maxDepth)), (unsigned int) (pts[i + 1] * (double) (1u << maxDepth)),
                                            (unsigned int) (pts[i + 2] *
    (double) (1u << maxDepth)), maxDepth, dim, maxDepth));
        }
    }*/
    ...
    if (npes == 1) {
        ...
        SFC::seqSort::SFC_treeSort(&(*(tmpNodes.begin())), tmpNodes.size(),
                                   ...
                                   pNodesBalanced, maxDepth, maxDepth, root, 0,
                                   1, options);
        ...
        if(isUniqueSorted)
        {
            if(!rank) std::cout<<"seq::test::isUniqueAndSorted .............
        PASSED "<<std::endl;
        ...
        {
            if(!rank) std::cout<<"seq::test::isCompleteOctree and
        seq::test::isUniqueAndSorted ............. PASSED "<<std::endl; }else
        {
            if(!rank) std::cout<<"seq::test::isCompleteOctree and
        seq::test::isUniqueAndSorted ............. FAILED "<<std::endl;
        }
        ...
        if (!rank)
            std::cout
                << RED
                << "           Executing the PARALLEL TREESORT with options : "
                << options << NRM << std::endl;
        if (!rank)
            std::cout << RED
                      << "====================================================="
        ...
        SFC::parSort::SFC_treeSort(tmpNodes, pNodesSorted, pNodesConstructed,
                                   pNodesBalanced, tol, maxDepth, root, 0, 1,
                                   options, 2, MPI_COMM_WORLD);
    ...
    if (!rank)
        std::cout << YLW
                  << " Total number of sorted & unique points: " << totalSz
                  << NRM << std::endl;
    if (!rank)
        std::cout << YLW
                  << "========================================================="
    ...
    if (!rank)
        std::cout << YLW
                  << " Total number of octants in Complete octree: " << totalSz
                  << NRM << std::endl;
    if (!rank)
        std::cout << YLW
                  << "========================================================="
    ...
    if (!rank)
        std::cout << YLW
                  ...
                  << NRM << std::endl;
    if (!rank)
        std::cout << YLW << " Total number of balanced octants : " << totalSz
                  << NRM << std::endl;
    if (!rank)
        std::cout << YLW
                  << "========================================================="
