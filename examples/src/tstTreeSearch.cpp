 ...
 * @brief generate an octree balance it, and perform comparison study between
 *binary search and sfc search.
 *
 **/
...
int main(int argc, char** argv) {
    ...
    // if (!rank) std::cout << "Initialized Hcurves for dimention " << dim <<
    // std::endl;
    ...
    if (!rank)
        std::cout << YLW
                  ...
                  << NRM << std::endl;
    if (!rank)
        std::cout << YLW << " Total number of Points: " << totalSz << NRM
                  << std::endl;
    if (!rank)
        std::cout << YLW
                  << "========================================================="
    ...
    if (npes > 2) {
        std::cout
            << "This is a sequential test, do not run with more than 1 process."
            << std::endl;
        MPI_Finalize();
        return 0;
    }
    ...
    if (npes == 1) {
        ...
        if (!rank)
            std::cout << RED
                      << "           Executing the SEQUENTIAL TREESORT with "
                         "options : "
                      << NRM << std::endl;
        if (!rank)
            std::cout << RED
        ...
        SFC::seqSort::SFC_treeSort(&(*(tmpNodes.begin())), tmpNodes.size(),
                                   pSorted, pConstructed, pBalanced, maxDepth,
                                   maxDepth, root, 0, 1, TS_CONSTRUCT_OCTREE);
        t2 = MPI_Wtime();

        if (!rank)
            std::cout << "construction time(s) : " << (t2 - t1) << std::endl;
        if (!rank)
            std::cout << "number of octs(construction): " << pConstructed.size()
                      << std::endl;

        t1 = MPI_Wtime();
        SFC::seqSort::SFC_treeSort(&(*(pConstructed.begin())),
                                   pConstructed.size(), pSorted, pConstructed,
                                   pBalanced, maxDepth, maxDepth, root, 0, 1,
                                   TS_BALANCE_OCTREE);
        t2 = MPI_Wtime();

        if (!rank)
            std::cout << "balancing time(s) : " << (t2 - t1) << std::endl;
        if (!rank)
            std::cout << "number of octs(balOcts): " << pBalanced.size()
                      << std::endl;
        ...
        SFC::seqSort::SFC_treeSort(
            &(*(tmpKeys.begin())), tmpKeys.size(), tmpNodes, tmpNodes, tmpNodes,
            m_uiMaxDepth, m_uiMaxDepth, root, ROOT_ROTATION, 1, TS_SORT_ONLY);
        ...
        for (unsigned int iter = 0; iter < NUM_ITERARTIONS; iter++) {
            ...
            t1 = MPI_Wtime();
            // SFC::seqSearch::SFC_treeSearch(&(*(tmpKeys.begin())),&(*(pBalanced.begin())),0,tmpKeys.size(),0,pBalanced.size(),m_uiMaxDepth,m_uiMaxDepth,ROOT_ROTATION);
            SFC::seqSort::SFC_treeSort(&(*(tmpKeys.begin())), tmpKeys.size(),
                                       tmpNodes, tmpNodes, tmpNodes,
                                       m_uiMaxDepth, m_uiMaxDepth, root,
        ...
        if (!rank)
            std::cout << "sfcSearch(s) : " << sfc_time << "\t averge over "
                      << NUM_ITERARTIONS
                      << "(s): " << (sfc_time / NUM_ITERARTIONS) << std::endl;
        if (!rank)
            std::cout << "number of keys: " << tmpKeys.size() << std::endl;
        // if(!rank)std::cout<<"last search found keys:
        // "<<tmpKeyResults.size()<<std::endl;
        ...
        for (unsigned int i = 0; i < tmpKeys.size(); i++) {
            ot::TreeNode* found = (ot::TreeNode*)std::bsearch(
                (ot::TreeNode*)&tmpKeys[i], &(*(pBalanced.begin())),
                pBalanced.size(), sizeof(ot::TreeNode),
                SFC::compare<ot::TreeNode>);
            if (found != NULL)
        ...
        if (!rank)
            std::cout << "bsearch(s) : " << bsearch_time << "\t averge over "
                      << NUM_ITERARTIONS
                      << "(s): " << (bsearch_time / NUM_ITERARTIONS)
                      << std::endl;
        if (!rank)
            std::cout << "number of keys: " << tmpKeys.size() << std::endl;
