 ...
 *
 * @breif Sequential program to evaluate the performance of the local sort and
 * SFC::seq::bucketing function.
 * @details This was written to debug the weak sacalability issues with
 * SFC::treeSort and bucketing function.
 */
...
int main(int argc, char** argv) {
    ...
    if (argc > 4) distribution = atoi(argv[4]);
    ...
    SFC::seqSort::SFC_treeSort((&(*(tmpNodes.begin()))), tmpNodes.size(),
                               tmpSorted, tmpConstruct, tmpBalance, maxDepth,
                               maxDepth, root, 0, 1, 1);
    double local_sort = std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    if (!rank) {
        ...
        std::cout << npes << "\t" << stat_property[0] << "\t"
                  << stat_property[1] << "\t" << stat_property[2] << std::endl;
    }
    ...
    SFC::seqSort::SFC_bucketing(&(*(tmpNodes.begin())), 0, maxDepth, 0, begin,
                                end, splitterTemp);
    double bucket_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                             std::chrono::high_resolution_clock::now() - t1)
                             .count();  // MPI_Wtime()-t1;

    par::Mpi_Reduce(&bucket_time, &stat_property[0], 1, MPI_MIN, 0,
                    GLOBAL_COMM);
    par::Mpi_Reduce(&bucket_time, &stat_property[1], 1, MPI_SUM, 0,
                    GLOBAL_COMM);
    par::Mpi_Reduce(&bucket_time, &stat_property[2], 1, MPI_MAX, 0,
                    GLOBAL_COMM);
    ...
    tmpNodes          = tmpNodes_cpy;
    t1                = std::chrono::high_resolution_clock::now();
    SFC::parSort::SFC_treeSort(tmpNodes, tmpSorted, tmpConstruct, tmpBalance,
                               0.1, maxDepth, root, 0, 1, 1, sf_k, GLOBAL_COMM);
    double ts_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                         std::chrono::high_resolution_clock::now() - t1)
                         .count();  // MPI_Wtime()-t1;
