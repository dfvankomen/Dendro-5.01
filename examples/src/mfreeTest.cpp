...
void ptBucketRoofline(const Point* ptList, unsigned int numPts,
                      unsigned int lev, unsigned int pMaxDepth) {
    ...
    if (lev == pMaxDepth) {
        for (unsigned int l = 0; l < pMaxDepth; l++) tLev[l].snapreset();
    }
...
#pragma omp critical
    ...
    for (unsigned int child = 0; child < NUM_CHILDREN; child++) {
        (counts[child] > 0) ? ptChild[child] = new Point[counts[child]]
                            : ptChild[child] = NULL;
    }

    for (unsigned int i = 0; i < NUM_CHILDREN; i++) counts[i] = 0;
...
#pragma omp critical
    ...
    if (lev == pMaxDepth) {
        for (unsigned int l = 0; l < pMaxDepth; l++)
            std::cout << "lev: " << (l) << " time (s) : " << tLev[l].snap
                      << std::endl;
    }
...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        if (!rank)
            std::cout << "Usage: " << argv[0] << " maxDepth numpts eleOrder"
                      << std::endl;
        return 0;
    }
    ...
    SFC::parSort::SFC_treeSort(tmpNodes, tmpVec, tmpVec, tmpVec, partition_tol,
                               m_uiMaxDepth, root, ROOT_ROTATION, 1,
                               TS_REMOVE_DUPLICATES, DENDRO_DEFAULT_SF_K, comm);
    std::swap(tmpNodes, tmpVec);
    tmpVec.clear();

    SFC::parSort::SFC_treeSort(tmpNodes, tmpVec, tmpVec, tmpVec, partition_tol,
                               m_uiMaxDepth, root, ROOT_ROTATION, 1,
                               TS_CONSTRUCT_OCTREE, DENDRO_DEFAULT_SF_K, comm);
    std::swap(tmpNodes, tmpVec);
    tmpVec.clear();
    ...
    SFC::parSort::SFC_treeSort(tmpNodes, balOct, balOct, balOct, partition_tol,
                               m_uiMaxDepth, root, ROOT_ROTATION, 1,
                               TS_BALANCE_OCTREE, DENDRO_DEFAULT_SF_K, comm);
    tmpNodes.clear();
    ...
    mfree::ptBucketRoofline(&(*(ptList.begin())), ptList.size(), m_uiMaxDepth,
                            m_uiMaxDepth);

    if (!rank)
        std::cout << YLW << "Total number of cg points :  " << globalSz
                  << std::endl;
    profiler_t tBucket;
    tBucket.start();
    ...
    if (!rank)
        std::cout << "SFC bucket time: " << tBucket.snap
                  << " mpi: " << (t2 - t1) << std::endl;
...
#pragma omp parallel for
    ...
    if (!rank)
        std::cout << "Indirect Memory Access time: " << tIndirectMemAccess.snap
                  << std::endl;
