...
void meshBenchMark(char* ptsFile, bool genPts, unsigned int numPts,
                   unsigned int dim, unsigned int maxDepth,
                   unsigned int distribution, double tol, unsigned int sf_k,
                   unsigned int options, char* prefix, MPI_Comm comm) {
    int rank, npes;
    ...
    m_uiMaxDepth = maxDepth;  // this is needed because we increase this when we
                              // embedded the tree.
    ...
    if (npes >= 2) {
        ...
        if (!rank) {
            std::cout << YLW << "Mesh generation time (max): " << t_mesh_g[2]
                      << NRM << std::endl;
            std::cout << "\t" << YLW << " e2e (min,mean,max): "
                      << "( " << t_e2e_g[0] << "\t" << t_e2e_g[1] << "\t"
                      << t_e2e_g[2] << " )" << NRM << std::endl;
            std::cout << "\t" << YLW << " e2n (min,mean,max): "
                      << "( " << t_e2n_g[0] << "\t" << t_e2n_g[1] << "\t"
                      << t_e2n_g[2] << " )" << NRM << std::endl;
            std::cout << "\t" << YLW << " sm (min,mean,max): "
                      << "( " << t_sm_g[0] << "\t" << t_sm_g[1] << "\t"
                      << t_sm_g[2] << " )" << NRM << std::endl;
            std::cout << "\t" << YLW << " blk (min,mean,max): "
                      << "( " << t_blk_g[0] << "\t" << t_blk_g[1] << "\t"
        ...
        if (!rank)
            std::cout << RED << "Remove duplicates begin" << NRM << std::endl;
        auto t1 = MPI_Wtime();
        SFC::seqSort::SFC_treeSort(&(*(tmpNodes.begin())), tmpNodes.size(),
                                   pNodesSorted, pNodesConstructed,
                                   pNodesBalanced, maxDepth, maxDepth, root,
                                   ROOT_ROT_ID, 1, TS_REMOVE_DUPLICATES);
        auto t2 = MPI_Wtime();
        t_rd    = t2 - t1;
        if (!rank)
            std::cout << RED << "Remove duplicates end" << NRM << std::endl;
        ...
        SFC::seqSort::SFC_treeSort(
            &(*(pNodesSorted.begin())), pNodesSorted.size(), pNodesSorted,
            pNodesConstructed, pNodesBalanced, maxDepth, maxDepth, root,
            ROOT_ROT_ID, 1, TS_CONSTRUCT_OCTREE);
        t2     = MPI_Wtime();
        t_cons = t2 - t1;
        if (!rank)
            std::cout << RED << "Tree construction end" << NRM << std::endl;
    ...
    if (!rank) {
        ...
        if (npes == 2) {
            statFile
                << "npes\tgrainSz\tdim\tmaxDepth\ttolerance\tsf_"
                   "k\tstencilSz\teleOrder\tt_rm_min\tt_rm_mean\tt_rm_max\tt_"
                   "cons_min\tt_cons_mean\tt_cons_max\tt_bal_min\tt_bal_"
                   "mean\tt_bal_max\tt_mesh_min\tt_mesh_mean\tt_mesh_max\tnum_"
                   "oct\tnum_oct_cons\tnum_oct_bal\tcg_nodes"
                << std::endl;
        }

        statFile << npes << "\t" << numPts << "\t" << dim << "\t" << maxDepth
                 << "\t" << tol << "\t" << sf_k << "\t" << stencilSz << "\t"
                 << eleOrder << "\t" << t_rd_g[0] << "\t" << t_rd_g[1] << "\t"
                 << t_rd_g[2] << "\t" << t_cons_g[0] << "\t" << t_cons_g[1]
                 << "\t" << t_cons_g[2] << "\t" << t_bal_g[0] << "\t"
                 << t_bal_g[1] << "\t" << t_bal_g[2] << "\t" << t_mesh_g[0]
                 << "\t" << t_mesh_g[1] << "\t" << t_mesh_g[2] << "\t"
                 << numUniqueOct << "\t" << numConsOct << "\t" << numBalOct
                 << "\t" << cg_sz << std::endl;
        statFile.close();
    }
}

void weakScalingDriver(char* ptsFile, bool genPts, DendroIntL numPts,
                       unsigned int dim, unsigned int maxDepth,
                       double tolerance, int distribution, unsigned int k,
                       unsigned int options, unsigned int sf_k, char* prefix,
                       MPI_Comm comm) {
    int rank, npes;
    MPI_Comm_rank(comm, &rank);
    MPI_Comm_size(comm, &npes);
    ...
    meshBenchMark(ptsFile, genPts, numPts, dim, maxDepth, distribution,
                  tolerance, sf_k, options, prefix, comm);
    ...
    MPI_Barrier(comm_ws);
    meshBenchMark(ptsFile, genPts, numPts, dim, maxDepth, distribution,
                  tolerance, sf_k, options, prefix, comm_ws);
...
int main(int argc, char** argv) {
    ...
    weakScalingDriver(argv[1], genPts, numPts, dim, maxDepth, tol, distribution,
                      k_perOct, options, sf_k, (char*)prefix.c_str(),
                      MPI_COMM_WORLD);
    dollar::text(std::cout);
    MPI_Finalize();
