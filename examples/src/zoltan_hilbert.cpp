...
void user_sizes_node(void *data, int num_gid_entries, int num_lid_entries,
...
void user_pack_node(void *data, int num_gid_entries, int num_lid_entries,
                    int nids, ZOLTAN_ID_PTR global_id, ZOLTAN_ID_PTR local_id,
                    int *dest, int *size, int *index, char *buf, int *ierr) {
    *ierr           = ZOLTAN_OK;
    MESH_DATA *mesh = (MESH_DATA *)data;
...
void user_unpack_node(void *data, int gidSize, int num_ids,
                      ZOLTAN_ID_PTR global_id, int *size, int *idx, char *buf,
                      int *ierr) {
    *ierr           = ZOLTAN_OK;
    MESH_DATA *mesh = (MESH_DATA *)data;
    ...
    mesh->globalIds = (ZOLTAN_ID_TYPE *)realloc(
        mesh->globalIds, sizeof(ZOLTAN_ID_TYPE) * num_ids);
    mesh->localIds = (ZOLTAN_ID_TYPE *)realloc(
        mesh->localIds, sizeof(ZOLTAN_ID_TYPE) * num_ids);

    mesh->x =
        (ZOLTAN_ID_TYPE *)realloc(mesh->x, sizeof(ZOLTAN_ID_TYPE) * num_ids);
    mesh->y =
        (ZOLTAN_ID_TYPE *)realloc(mesh->x, sizeof(ZOLTAN_ID_TYPE) * num_ids);
    mesh->z =
        (ZOLTAN_ID_TYPE *)realloc(mesh->x, sizeof(ZOLTAN_ID_TYPE) * num_ids);
...
void ZoltanHibertLBandSort(DendroIntL grainSz, unsigned int dim,
                           unsigned int maxDepth, double tol,
    ...
    for (DendroIntL i = 0; i < grainSz; i++) {
        if ((pts[3 * i] > 0.0) && (pts[3 * i + 1] > 0.0) &&
            (pts[3 * i + 2] > 0.0) &&
            (((unsigned int)(pts[3 * i] * ((double)(1u << maxDepth)))) <
             (1u << maxDepth)) &&
            (((unsigned int)(pts[3 * i + 1] * ((double)(1u << maxDepth)))) <
             (1u << maxDepth)) &&
            (((unsigned int)(pts[3 * i + 2] * ((double)(1u << maxDepth)))) <
             (1u << maxDepth))) {
    ...
    Zoltan_Set_Param(zz, "NUM_GID_ENTRIES",
                     std::to_string(NUM_GID_SIZE).c_str());
    Zoltan_Set_Param(zz, "NUM_LID_ENTRIES",
                     std::to_string(NUM_LID_SIZE).c_str());
    ...
    ******************************************************************/
    ...
    Zoltan_Set_Param(zz, "KEEP_CUTS",
                     "1");  // KEEP_CUTS = 0 means don't keep cuts, KEEP_CUTS =
                            // 1 means keep cuts,

    // REDUCE_DIMENSIONS: When a 3 dimensional geometry is almost flat, it may
    // make more sense to treat it as a 2 dimensional geometry when applying the
    // HSFC algorithm. (Coordinate values in the omitted direction are ignored
    // for the purposes of partitioning.) If this parameter is set to 1, a 3
    // dimensional geometry will be treated as 2 dimensional if is very flat, or
    // 1 dimensional if it very thin. And a 2 dimensional geometry will be
    // treated as 1 dimensional if it is very thin. Turning this parameter on
    // removes the possibility that disconnected parts will appear on the
    // surface of a flat 3 dimensional object.
    // zz->Set_Param( "REDUCE_DIMENSIONS", "0");

    // DEGENERATE_RATIO:
    // If the REDUCE_DIMENSIONS parameter is set, then this parameter determines
    // when a geometry is considered to be flat. A bounding box which is
    // oriented to the geometry is constructed, and the lengths of its sides are
    // tested against a ratio of 1
    ...
    int changes = 0, numGidEntries = 1, numLidEntries = 1, numImport = 0,
        numExport                  = 0;
    ZOLTAN_ID_PTR importGlobalGids = NULL, importLocalGids = NULL,
                  exportGlobalGids = NULL, exportLocalGids = NULL;
    int *importProcs = NULL, *importToPart = NULL, *exportProcs = NULL,
        *exportToPart = NULL;
    ...
    /*if(rc==ZOLTAN_OK)
    {
        if(!rank)
    std::cout<<"=================================================================="<<std::endl;
        if(!rank) std::cout<<"                     Zoltan LB PARTIION FINISHED
    "<<std::endl; if(!rank)
    std::cout<<"=================================================================="<<std::endl;
    }*/
    ...
    // std::cout << "rank : " << rank << " numPts: " << mesh.numLocalPts << "
    // sendCnt : " << numExport << " recvCnt: " << numImport << std::endl;
    ...
    for (unsigned int i = 0; i < numExport; i++) {
        //        if(!rank) std::cout <<" Exp Proc:
        //        "<<exportProcs[i]<<std::endl;
        sendCnts[exportProcs[i]]++;
    }

    for (unsigned int i = 0; i < numImport; i++) recvCnts[importProcs[i]]++;
    ...
    for (unsigned int i = 0; i < numExport; i++) {
        //        std::cout<<" Index :
        //        "<<(senCntsOffset[exportProcs[i]]+Cnts[exportProcs[i]])<<std::endl;
    ...
    /*  MPI_Alltoallv(&sendData.front(), (int *) sendCnts, (int *)
       senCntsOffset, MPI_NODETYPE, &recvData.front(), (int *) recvCnts, (int *)
       recvCntsOffset, MPI_NODETYPE, comm);*/

    // par::Mpi_Alltoallv(&sendData.front(),(int *) sendCnts,(int *)
    // senCntsOffset,&recvData.front(), (int *) recvCnts, (int *)
    // recvCntsOffset,comm);

    par::Mpi_Alltoallv_Kway(&sendData.front(), (int *)sendCnts,
                            (int *)senCntsOffset, &recvData.front(),
                            (int *)recvCnts, (int *)recvCntsOffset, comm);
    ...
    /*   Zoltan_Set_Obj_Size_Multi_Fn(zz, user_sizes_node, &mesh);
       ...
       Zoltan_Migrate(zz, numImport, importGlobalGids, importLocalGids,
       importProcs, importToPart, numExport, exportGlobalGids, exportLocalGids,
    ...
    for(unsigned int i=0;i<localPts;i++)
    {
        ...
        if( (permutedIds[i] - minRank) > localPts){
            std::cout<<"rank "<<rank<<" permute id:
    "<<permutedIds[i]-minRank<<std::endl; count++; continue;
        }
    ...
    MPI_Reduce(&zz_splitter, zz_splitter_g + 1, 1, MPI_DOUBLE, MPI_SUM, 0,
               comm);
    MPI_Reduce(&zz_splitter, zz_splitter_g + 2, 1, MPI_DOUBLE, MPI_MAX, 0,
               comm);
    ...
    if (!rank) {
        /*   std::cout << "zz_min\tzz_mean\tzz_max"<<std::endl;
           std::cout <<
           zz_time_g[0]<<"\t"<<zz_time_g[1]<<"\t"<<zz_time_g[2]<<std::endl;*/
...
int main(int argc, char *argv[]) {
    ...
    ******************************************************************/
    ...
    if (argc < 5) {
        if (rank == 0)
            std::cout << "Usage: mpirun -np x ./zoltan numPts dim maxDepth tol "
                         "dis (0- gauss 1-logNormal 2-Uniform)"
                      << std::endl;
        MPI_Finalize();
        exit(0);
    }
    ...
    if (!rank)
        std::cout << "========================================================="
                  ...
                  << std::endl;
    if (!rank)
        std::cout << "     Warm Up run  (par)                                "
                  << std::endl;
    if (!rank)
        std::cout << "========================================================="
                     "============="
                  << std::endl;
    ZoltanHibertLBandSort(grainSize, dim, maxDepth, tol, warm_up, distribution,
                          MPI_COMM_WORLD);
    ...
    if (!rank)
        std::cout << "========================================================="
                  ...
                  << std::endl;
    if (!rank)
        std::cout << "     Zoltan Hilbert Partitioning Run  (par)              "
                     "                  "
                  << std::endl;
    if (!rank)
        std::cout << "========================================================="
    ...
    ZoltanHibertLBandSort(grainSize, dim, maxDepth, tol, stats_zz, distribution,
                          MPI_COMM_WORLD);
    ...
    if (splitStatus == 0) {
        ...
        for (int i = 1; i < npes; i++) {
            ...
            gatherCountOfffset[i] =
                gatherCountOfffset[i - 1] + localSz_g[i - 1];
        }

        std::vector<double> stat_g_zz(gatherCountScan[npes - 1]);

        MPI_Gatherv(&stats_zz[0], localSz, MPI_DOUBLE, &stat_g_zz[0], localSz_g,
                    gatherCountOfffset, MPI_DOUBLE, 0, statComm);
        ...
        if (!rank) {
            ...
            std::cout << "npes\tzz_splitter_min\tzz_splitter_mean\tzz_splitter_"
                         "max\tzz_all2all_min\tzz_all2all_mean\tzz_all2all_"
                         "max\tzz_local_min\tzz_local_mean\tzz_local_max\tzz_"
                         "min\tzz_mean\tz_max"
                      << std::endl;
            std::cout << "Weak Scaling Results for : " << stat_g_zz[0]
                      << std::endl;
            ...
            for (int i = 17; i < stat_g_zz.size(); i = i + 17) {
                ...
                std::cout << stat_g_zz[i + 1] << "\t" << stat_g_zz[i + 5]
                          << "\t" << stat_g_zz[i + 6] << "\t"
                          << stat_g_zz[i + 7] << "\t" << stat_g_zz[i + 8]
                          << "\t" << stat_g_zz[i + 9] << "\t"
                          << stat_g_zz[i + 10] << "\t" << stat_g_zz[i + 11]
                          << "\t" << stat_g_zz[i + 12] << "\t"
                          << stat_g_zz[i + 13] << "\t" << stat_g_zz[i + 14]
                          << "\t" << stat_g_zz[i + 15] << "\t"
                          << stat_g_zz[i + 16] << std::endl;
            }
...
void get_geometry_list(void *data, int sizeGID, int sizeLID, int num_obj,
