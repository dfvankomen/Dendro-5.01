...
/* --------------------------------------------------------------------------*/
int
TCP_Index (int ivar, int i, int j, int k, int nvar, int n1, int n2, int n3)
{
  int i1 = i, j1 = j, k1 = k;
...
/* --------------------------------------------------------------------------*/
void
TCP_Derivatives_AB3 (int nvar, int n1, int n2, int n3, derivs v)
{
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    ...
    {				/* Calculation of Derivatives w.r.t. A-Dir. */
      ...
      {				/* (Chebyshev_Zeros)*/
        for (i = 0; i < n1; i++)
        {
          indx[i] = TCP_Index (ivar, i, j, k, nvar, n1, n2, n3);
          p[i] = v.d0[indx[i]];
        }
        chebft_Zeros (p, n1, 0);
        chder (p, dp, n1);
    ...
    {				/* Calculation of Derivatives w.r.t. B-Dir. */
      ...
      {				/* (Chebyshev_Zeros)*/
        for (j = 0; j < n2; j++)
        {
          indx[j] = TCP_Index (ivar, i, j, k, nvar, n1, n2, n3);
          p[j] = v.d0[indx[j]];
          q[j] = v.d1[indx[j]];
        }
        chebft_Zeros (p, n2, 0);
    ...
    {				/* Calculation of Derivatives w.r.t. phi-Dir. (Fourier)*/
      for (j = 0; j < n2; j++)
      {
        for (k = 0; k < n3; k++)
        {
          indx[k] = TCP_Index (ivar, i, j, k, nvar, n1, n2, n3);
          p[k] = v.d0[indx[k]];
          q[k] = v.d1[indx[k]];
          r[k] = v.d2[indx[k]];
        }
...
void TCP_F_of_v(int nvar, int n1, int n2, int n3, derivs v, CCTK_REAL *F, derivs u) {
 /*      Calculates the left hand sides of the non-linear equations F_m(v_n)=0*/
  ...
  TCP_allocate_derivs (&U, nvar);

  TCP_Derivatives_AB3 (nvar, n1, n2, n3, v);
  ...
  for (i = 0; i < n1; i++)
  {
    for (j = 0; j < n2; j++)
    {
      for (k = 0; k < n3; k++)
      {
        ...
        for (ivar = 0; ivar < nvar; ivar++)
        {
          indx = TCP_Index (ivar, i, j, k, nvar, n1, n2, n3);
        ...
        TCP_AB_To_XR (nvar, A, B, &X, &R, U);
        /* Calculation of (x,r) and*/
        /* (U, U_x, U_r, U_3, U_xx, U_xr, U_x3, U_rr, U_r3, U_33)*/
        TCP_C_To_c (nvar, X, R, &x, &r, U);
        /* Calculation of (y,z) and*/
        /* (U, U_x, U_y, U_z, U_xx, U_xy, U_xz, U_yy, U_yz, U_zz)*/
        TCP_rx3_To_xyz (nvar, x, r, phi, &y, &z, U);
        TCP_NonLinEquations (A, B, X, R, x, r, phi, y, z, U, values);
        for (ivar = 0; ivar < nvar; ivar++)
        {
          indx = TCP_Index (ivar, i, j, k, nvar, n1, n2, n3);
        ...
        if (debugfile && (k==0))
        {
          CCTK_REAL eta, varphi, kappa, kappa2, varphi2;

          r_plus = sqrt ((x - TPID::par_b) * (x - TPID::par_b) + y * y + z * z);
          r_minus = sqrt ((x + TPID::par_b) * (x + TPID::par_b) + y * y + z * z);

          eta    = 0.5 * TPID::par_m_plus / r_plus + 0.5 * TPID::par_m_minus / r_minus;
          varphi = 0.5 * TPID::par_q_plus / r_plus + 0.5 * TPID::par_q_minus / r_minus;
          kappa  = 1. + U.d0[0] + eta;

          kappa2  = kappa * kappa;
          varphi2 = varphi * varphi;

          psi = sqrt(kappa2 - varphi2);
          ...
          fprintf(debugfile,
                  ...
                  (double)(U.d11[0] +
                           U.d22[0] +
                           U.d33[0] +
/*                      0.125 * TCP_BY_KKofxyz (x, y, z) / psi7 +*/
                           (2.0 * Pi / psi2/psi * TCP_compute_rho_em(x, y, z)) * FAC),
                  (double)((U.d11[0] +
                            U.d22[0] +
                            U.d33[0])*FAC),
                  (double)(-(2.0 * Pi / psi2/psi * TCP_compute_rho_em(x, y, z)) * FAC),
                  (double)TCP_compute_rho_em(x, y, z)
                  /*(double)F[indx]*/
          );
        }
      }
    ...
    for (i = 0; i < n1; i++)
    {
      ...
      for (j = 0; j < n2; j++)
      {
        for (k = 0; k < n3; k++)
        {
          ...
          TCP_AB_To_XR (nvar, A, B, &X, &R, dU);
          /* Calculation of (x,r) and*/
          /* (dU, dU_x, dU_r, dU_3, dU_xx, dU_xr, dU_x3, dU_rr, dU_r3, dU_33)*/
          TCP_C_To_c (nvar, X, R, &x, &r, dU);
          /* Calculation of (y,z) and*/
          /* (dU, dU_x, dU_y, dU_z, dU_xx, dU_xy, dU_xz, dU_yy, dU_yz, dU_zz)*/
          TCP_rx3_To_xyz (nvar, x, r, phi, &y, &z, dU);
          TCP_LinEquations (A, B, X, R, x, r, phi, y, z, dU, U, values);
          for (ivar = 0; ivar < nvar; ivar++)
          {
            indx = TCP_Index (ivar, i, j, k, nvar, n1, n2, n3);
...
void TCP_JFD_times_dv(int i, int j, int k, int nvar, int n1, int n2, int n3,
  ...
  for (ivar = 0; ivar < nvar; ivar++)
  {
    int iccc = TCP_Index (ivar, i, j, k, nvar, n1, n2, n3),
      ipcc = TCP_Index (ivar, i + 1, j, k, nvar, n1, n2, n3),
      imcc = TCP_Index (ivar, i - 1, j, k, nvar, n1, n2, n3),
      icpc = TCP_Index (ivar, i, j + 1, k, nvar, n1, n2, n3),
      icmc = TCP_Index (ivar, i, j - 1, k, nvar, n1, n2, n3),
      iccp = TCP_Index (ivar, i, j, k + 1, nvar, n1, n2, n3),
      iccm = TCP_Index (ivar, i, j, k - 1, nvar, n1, n2, n3),
      icpp = TCP_Index (ivar, i, j + 1, k + 1, nvar, n1, n2, n3),
      icmp = TCP_Index (ivar, i, j - 1, k + 1, nvar, n1, n2, n3),
      icpm = TCP_Index (ivar, i, j + 1, k - 1, nvar, n1, n2, n3),
      icmm = TCP_Index (ivar, i, j - 1, k - 1, nvar, n1, n2, n3),
      ipcp = TCP_Index (ivar, i + 1, j, k + 1, nvar, n1, n2, n3),
      imcp = TCP_Index (ivar, i - 1, j, k + 1, nvar, n1, n2, n3),
      ipcm = TCP_Index (ivar, i + 1, j, k - 1, nvar, n1, n2, n3),
      imcm = TCP_Index (ivar, i - 1, j, k - 1, nvar, n1, n2, n3),
      ippc = TCP_Index (ivar, i + 1, j + 1, k, nvar, n1, n2, n3),
      impc = TCP_Index (ivar, i - 1, j + 1, k, nvar, n1, n2, n3),
      ipmc = TCP_Index (ivar, i + 1, j - 1, k, nvar, n1, n2, n3),
      immc = TCP_Index (ivar, i - 1, j - 1, k, nvar, n1, n2, n3);
...
void TCP_SetMatrix_JFD(int nvar, int n1, int n2, int n3, derivs u, int *ncols,
                    ...
                    for (i = 0; i < n1; i++)
                    {
                      for (j = 0; j < n2; j++)
                      {
                        for (k = 0; k < n3; k++)
                        {
                          for (ivar = 0; ivar < nvar; ivar++)
                          {
                            row = TCP_Index (ivar, i, j, k, nvar, n1, n2, n3);
                            ncols[row] = 0;
                            dv.d0[row] = 0;
                          }
                        }
                    ...
                    for (i = 0; i < n1; i++)
                    {
                      for (j = 0; j < n2; j++)
                      {
                        for (k = 0; k < n3; k++)
                        {
                          for (ivar = 0; ivar < nvar; ivar++)
                          {
                            column = TCP_Index (ivar, i, j, k, nvar, n1, n2, n3);
                            dv.d0[column] = 1;
                  ...
                  /*					i_0 = 0;
                            ...
                            for (i1 = i_0; i1 <= i_1; i1++)
                            {
                              for (j1 = j_0; j1 <= j_1; j1++)
                              {
                                ...
                                {
                                  TCP_JFD_times_dv (i1, j1, k1, nvar, n1, n2, n3,
                                                    dv, u, values);
                                  for (ivar1 = 0; ivar1 < nvar; ivar1++)
                                  {
                                    if (values[ivar1] != 0)
                                    {
                                      row = TCP_Index (ivar1, i1, j1, k1, nvar, n1, n2, n3);
                                      mcol = ncols[row];
                  ...
                  CCTK_REAL
                  TCP_PunctEvalAtArbitPosition (CCTK_REAL *v, int ivar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL phi,
...
/* --------------------------------------------------------------------------*/
void
TCP_calculate_derivs (int i, int j, int k, int ivar, int nvar, int n1, int n2,
                      int n3, derivs v, derivs vv)
{
  ...
  vv.d0[0] = v.d0[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)];
  vv.d1[0] = v.d1[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al;
  vv.d2[0] = v.d2[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_be;
  vv.d3[0] = v.d3[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)];
  vv.d11[0] = v.d11[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin2_al
    + v.d1[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * cos_al;
  vv.d12[0] =
    v.d12[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al * sin_be;
  vv.d13[0] = v.d13[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_al;
  vv.d22[0] = v.d22[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin2_be
    + v.d2[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * cos_be;
  vv.d23[0] = v.d23[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)] * sin_be;
  vv.d33[0] = v.d33[TCP_Index (ivar, i, j, k, nvar, n1, n2, n3)];
}

/* --------------------------------------------------------------------------*/
CCTK_REAL
TCP_interpol (CCTK_REAL a, CCTK_REAL b, CCTK_REAL c, derivs v)
{
  return v.d0[0]
...
CCTK_REAL
TCP_PunctTaylorExpandAtArbitPosition (int ivar, int nvar, int n1,
                                      ...
                                      CCTK_REAL z)
{
  ...
  TCP_calculate_derivs (i, j, k, ivar, nvar, n1, n2, n3, v, vv);
  result = TCP_interpol (a, b, c, vv);
  TCP_free_derivs (&vv, 1);
...
/* Calculates the value of v at an arbitrary position (x,y,z)*/
CCTK_REAL
TCP_PunctIntPolAtArbitPosition (int ivar, int nvar, int n1,
                                int n2, int n3, derivs v, CCTK_REAL x, CCTK_REAL y,
                                CCTK_REAL z)
{
  CCTK_REAL phi, A, B, result, Ui;

  TCP_xyz_To_AB3(x,y,z,n1,n2,n3,&A,&B,&phi);

  result = TCP_PunctEvalAtArbitPosition (v.d0, ivar, A, B, phi, nvar, n1, n2, n3);
...
CCTK_REAL 
TCP_PunctEvalAtArbitPositionFast (CCTK_REAL *cf_v, int ivar, CCTK_REAL A, CCTK_REAL B, CCTK_REAL phi, int nvar, int n1, int n2, int n3)
{
  int i, j, k, N;
  CCTK_REAL *p, *q, *values1, result;
  ...
  for (k = 0; k < n3; k++){
    for (j = 0; j < n2; j++){
      for (i = 0; i < n1; i++){
        p[i] = cf_v[ivar + nvar * (i + n1 * (j + n2 * k))];
      }
      q[j] = chebev (-1, 1, p, n1, A);
    }
    values1[k] = chebev (-1, 1, q, n2, B);
  }
...
/* --------------------------------------------------------------------------*/
CCTK_REAL
TCP_PunctIntPolAtArbitPositionFast (int ivar, int nvar, int n1,
                                    int n2, int n3, derivs cf_v, CCTK_REAL x, CCTK_REAL y,
                                    CCTK_REAL z)
{
  ...
  result = TCP_PunctEvalAtArbitPositionFast (cf_v.d0, ivar, A, B, phi, nvar, n1, n2, n3);
