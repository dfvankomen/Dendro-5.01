...
void write_coefficient_file(FILE* file, derivs w, int num_tot){
  ...
  /* We assume that the first two lines have already been written */
  /* The first line contains n1, n2, n3, the second mp, mm */
  /* Opening and closing file is up to the caller */
  for (int j = 0; j < num_tot; ++j)
    fprintf(file, "%.16e %.16e %.16e %.16e\n",
            (double)w.d0[j], (double)w.d1[j],
            (double)w.d2[j], (double)w.d3[j]);
}


static
void load_coefficient_file(FILE* file, derivs w, int num_tot){
  /* Load content of file into w */
  /* file has to be like the matrix w */

  /* We assume that the first two lines have already been read */
  /* The first line contains n1, n2, n3, the second mp, mm */
  /* Opening and closing file is up to the caller */
  for (int j = 0; j < num_tot; ++j)
    fscanf(file, "%le %le %le %le\n",
           &(w.d0[j]), &(w.d1[j]), &(w.d2[j]), &(w.d3[j]));
}

/* Swap two variables */
static inline
void swap(CCTK_REAL* a, CCTK_REAL* b)
{
    CCTK_REAL t = *a;
    *a = *b;
    *b = t;
...
//     return (M * (3. / 8 * pow(r, 4) / pow(TPID::TCP_Extend_Radius, 5) -
//                  5. / 4 * pow(r, 2) / pow(TPID::TCP_Extend_Radius, 3) +
//                  15. / 8 / TPID::TCP_Extend_Radius));
// }
    ...
    *mm = TPID::par_m_minus;
    ...
    switch (gsm) {
        case GSM_Taylor_expansion:
            U = TCP_PunctTaylorExpandAtArbitPosition(0, nvar, n1, n2, n3, v, xx, yy,
                                                 zz);
            break;
        case GSM_evaluation:
            U = TCP_PunctIntPolAtArbitPositionFast(0, nvar, n1, n2, n3, cf_v, xx,
                                               yy, zz);
            break;
        default:
            assert(0);
    }
            r_plus = pow (pow (r_plus, 4) + pow (TPID::TCP_epsilon, 4), 0.25);
        r_minus = pow (pow (r_minus, 4) + pow (TPID::TCP_epsilon, 4), 0.25);
        if (r_plus < TPID::TCP_Tiny)
            r_plus = TPID::TCP_Tiny;
        if (r_minus < TPID::TCP_Tiny)
            r_minus = TPID::TCP_Tiny;
        CCTK_REAL psi1 = sqrt(
          (1. + U + 0.5 * *mp / r_plus + 0.5 * *mm / r_minus) *
          (1. + U + 0.5 * *mp / r_plus + 0.5 * *mm / r_minus) -
          0.25 * (
            (TPID::par_q_plus / r_plus + TPID::par_q_minus / r_minus) *
            (TPID::par_q_plus / r_plus + TPID::par_q_minus / r_minus)
          )
        );
        if (r_plus < TPID::TCP_Extend_Radius) {
          psi1 = sqrt(
            (1. + U + 0.5 * EXTEND(*mp, r_plus) + 0.5 * *mm / r_minus) *
            (1. + U + 0.5 * EXTEND(*mp, r_plus) + 0.5 * *mm / r_minus) -
            0.25 * (
              (EXTEND(TPID::par_q_plus, r_plus) + TPID::par_q_minus / r_minus) *
              (EXTEND(TPID::par_q_plus, r_plus) + TPID::par_q_minus / r_minus)
            )
          );
        }
        if (r_minus < TPID::TCP_Extend_Radius) {
          psi1 = sqrt(
            (1. + U + 0.5 * EXTEND(*mm, r_minus) + 0.5 * *mp / r_plus) *
            (1. + U + 0.5 * EXTEND(*mm, r_minus) + 0.5 * *mp / r_plus) -
            0.25 * (
              (EXTEND(TPID::par_q_minus, r_minus) + TPID::par_q_plus / r_plus) *
              (EXTEND(TPID::par_q_minus, r_minus) + TPID::par_q_plus / r_plus)
            )
          );
        ...
        CCTK_REAL psi6 = psi1 * psi1 * psi1 * psi1 * psi1 * psi1;
    ...
    if ((conformal_state > 0) || (pmn_lapse) || (brownsville_lapse)) {
        ...
        rp = pow(pow(rp, 4) + pow(TPID::TCP_epsilon, 4), 0.25);
        if (rp < TPID::TCP_Tiny) rp = TPID::TCP_Tiny;

        ir = 1.0 / rp;

        if (rp < TPID::TCP_Extend_Radius) {
            ir = EXTEND(1., rp);
        }
...
#include "adm2emda.h"
    ...
    if (0) {
        ...
        TCP_free_derivs(&u, ntotal);
        TCP_free_derivs(&v, ntotal);
        TCP_free_derivs(&cf_v, ntotal);
    }
}
...
void TPStore(double *mp, double *mm, double *mp_adm, double *mm_adm, double *E,
    ...
    *mm = TPID::par_m_minus;
    ...
    if (!F) {
        ...
        TCP_allocate_derivs(&u, ntotal);
        TCP_allocate_derivs(&v, ntotal);
        TCP_allocate_derivs(&cf_v, ntotal);
...
#pragma omp parallel for num_threads(TP_OMP_THREADS)
        ...
        if (!(TPID::give_bare_mass)) {
            ...
            do {
                printf("Bare masses: mp=%.15g, mm=%.15g\n", (double)*mp,
                       (double)*mm);
                TCP_Newton(nvar, n1, n2, n3, v, TPID::Newton_tol, 2);

                TCP_F_of_v(nvar, n1, n2, n3, v, F, u);

                up = TCP_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v,
                                                TPID::par_b, 0., 0.);
                um = TCP_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v,
                                                -TPID::par_b, 0., 0.);
        ...
        TCP_Newton(nvar, n1, n2, n3, v, TPID::Newton_tol, TPID::Newton_maxit);

        TCP_F_of_v(nvar, n1, n2, n3, v, F, u);

        TCP_SpecCoef(n1, n2, n3, 0, v.d0, cf_v.d0);

        printf("The two puncture masses are mp=%.17g and mm=%.17g\n", *mp, *mm);

        up = TCP_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v, TPID::par_b, 0.,
                                        0.);
        um = TCP_PunctIntPolAtArbitPosition(0, nvar, n1, n2, n3, v, -TPID::par_b,
                                        0., 0.);
        ...
        *mm_adm = (1 + um) * *mm + *mp * *mm / (4. * TPID::par_b);
        ...
        admMass =
            (*mp + *mm -
             4 * TPID::par_b *
                 TCP_PunctEvalAtArbitPosition(v.d0, 0, 1, 0, 0, nvar, n1, n2, n3));
