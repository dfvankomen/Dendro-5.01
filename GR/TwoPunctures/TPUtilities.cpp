...
/* -------------------------------------------------------------------------*/
/* -------------------------------------------------------------------------*/
/* It is critical not to optimize this function!!! */
#pragma optimize("", off)
#pragma GCC push_options
#pragma GCC optimize("O0")
__float128 __attribute__((optimize("O0")))
Klein_sum (__float128 *input, int num_terms){

  /* We define everything as volatile to scare off the compiler from touching the code */
  volatile __float128 sum = 0.0Q;
  volatile __float128 cs = 0.0Q;
  volatile __float128 ccs = 0.0Q;
  volatile __float128 c;
  volatile __float128 cc;

  for (int i = 0; i < num_terms; i++){
    volatile __float128 t = sum + input[i];
    if (fabsl((__float128)sum) >= fabsl(input[i])) {
    ...
    sum = t;
    t = cs + c;
    if (fabsl((__float128)cs) >= fabsl((__float128)c)) {
      cc = (cs - t) + c;
    }else{
      cc = (c - t) + cs;
    }
