/* TwoChargedPunctures:  File  "TCP_Newton.c"*/
...
static void LineRelax_al(CCTK_REAL *const dv, int const j, int const k,
    ...
    for (ivar = 0; ivar < nvar; ivar++) {
        ...
        for (i = 0; i < n1; i++) {
            Ip = TCP_Index(ivar, i + 1, j, k, nvar, n1, n2, n3);
            Ic = TCP_Index(ivar, i, j, k, nvar, n1, n2, n3);
            Im = TCP_Index(ivar, i - 1, j, k, nvar, n1, n2, n3);
...
static void LineRelax_be(CCTK_REAL *const dv, int const i, int const k,
    ...
    for (ivar = 0; ivar < nvar; ivar++) {
        ...
        gsl_vector_set_zero(f);
        for (j = 0; j < n2; j++) {
            Ip = TCP_Index(ivar, i, j + 1, k, nvar, n1, n2, n3);
            Ic = TCP_Index(ivar, i, j, k, nvar, n1, n2, n3);
            Im = TCP_Index(ivar, i, j - 1, k, nvar, n1, n2, n3);
...
void TCP_TestRelax(int nvar, int n1, int n2, int n3, derivs v, CCTK_REAL *dv) {
    ...
    TCP_SetMatrix_JFD(nvar, n1, n2, n3, u, ncols, cols, JFD);
...
static int bicgstab(int const nvar, int const n1, int const n2, int const n3,
    ...
    TCP_J_times_dv(nvar, n1, n2, n3, dv, r, u);
#pragma omp parallel for num_threads(TP_OMP_THREADS)
...
#pragma omp parallel for num_threads(TP_OMP_THREADS)
        ...
        TCP_J_times_dv(nvar, n1, n2, n3, ph, vv, u); /* vv=J*ph*/
...
#pragma omp parallel for num_threads(TP_OMP_THREADS)
        ...
        TCP_J_times_dv(nvar, n1, n2, n3, sh, t, u); /* t=J*sh*/
...
/* -------------------------------------------------------------------*/
void TCP_Newton(int const nvar, int const n1, int const n2, int const n3, derivs v,
