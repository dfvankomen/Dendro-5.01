...
template <typename T>
inline int Mpi_Isend(T *buf, int count, int dest, int tag, MPI_Comm comm,
                     MPI_Request *request) {
    int error_code = MPI_Isend(buf, count, par::Mpi_datatype<T>::value(), dest,
                               tag, comm, request);
...
template <typename T>
inline int Mpi_Issend(T *buf, int count, int dest, int tag, MPI_Comm comm,
                      MPI_Request *request) {
    int error_code = MPI_Issend(buf, count, par::Mpi_datatype<T>::value(), dest,
                                tag, comm, request);
    __MPI_CHECK_ERROR__(error_code, comm);
    return error_code;
}
...
template <typename T>
inline int Mpi_Recv(T *buf, int count, int source, int tag, MPI_Comm comm,
                    MPI_Status *status) {
    int error_code = MPI_Recv(buf, count, par::Mpi_datatype<T>::value(), source,
                              tag, comm, status);
    __MPI_CHECK_ERROR__(error_code, comm);
    return error_code;
}
...
template <typename T>
inline int Mpi_Irecv(T *buf, int count, int source, int tag, MPI_Comm comm,
                     MPI_Request *request) {
    int error_code = MPI_Irecv(buf, count, par::Mpi_datatype<T>::value(),
...
inline int Mpi_Sendrecv(T *sendBuf, int sendCount, int dest, int sendTag,
    ...
    error_code =
        MPI_Sendrecv(sendBuf, sendCount, par::Mpi_datatype<T>::value(), dest,
                     sendTag, recvBuf, recvCount, par::Mpi_datatype<S>::value(),
...
template <typename T>
inline int Mpi_Scan(T *sendbuf, T *recvbuf, int count, MPI_Op op,
                    MPI_Comm comm) {
    ...
    error_code = MPI_Scan(sendbuf, recvbuf, count,
                          par::Mpi_datatype<T>::value(), op, comm);

    PROF_PAR_SCAN_END
}

template <typename T>
inline int Mpi_Allreduce(T *sendbuf, T *recvbuf, int count, MPI_Op op,
                         MPI_Comm comm) {
#ifdef __PROFILE_WITH_BARRIER__
    MPI_Barrier(comm);
#endif
    ...
    error_code = MPI_Allreduce(sendbuf, recvbuf, count,
                               par::Mpi_datatype<T>::value(), op, comm);
...
inline int Mpi_Alltoall(T *sendbuf, T *recvbuf, int count, MPI_Comm comm) {
    ...
    error_code =
        MPI_Alltoall(sendbuf, count, par::Mpi_datatype<T>::value(), recvbuf,
                     count, par::Mpi_datatype<T>::value(), comm);
...
inline int Mpi_Alltoallv(T *sendbuf, int *sendcnts, int *sdispls, T *recvbuf,
    ...
    int error_code = MPI_Alltoallv(
        sendbuf, sendcnts, sdispls, par::Mpi_datatype<T>::value(), recvbuf,
        recvcnts, rdispls, par::Mpi_datatype<T>::value(), comm);
...
template <typename T>
inline int Mpi_Gather(T *sendBuffer, T *recvBuffer, int count, int root,
                      MPI_Comm comm) {
#ifdef __PROFILE_WITH_BARRIER__
    MPI_Barrier(comm);
#endif
    ...
    error_code =
        MPI_Gather(sendBuffer, count, par::Mpi_datatype<T>::value(), recvBuffer,
                   count, par::Mpi_datatype<T>::value(), root, comm);

    PROF_PAR_GATHER_END
}

template <typename T>
inline int Mpi_Gatherv(T *sendBuffer, int sendcnts, T *recvBuffer,
                       int *recvcnts, int *rdispls, int root, MPI_Comm comm) {
#ifdef __PROFILE_WITH_BARRIER__
    MPI_Barrier(comm);
#endif
    PROF_PAR_GATHER_BEGIN

    error_code = MPI_Gatherv(
        sendBuffer, sendcnts, par::Mpi_datatype<T>::value(), recvBuffer,
        recvcnts, rdispls, par::Mpi_datatype<T>::value(), root, comm);
...
template <typename T>
inline int Mpi_Reduce(T *sendbuf, T *recvbuf, int count, MPI_Op op, int root,
                      MPI_Comm comm) {
#ifdef __PROFILE_WITH_BARRIER__
    MPI_Barrier(comm);
#endif
    ...
    error_code = MPI_Reduce(sendbuf, recvbuf, count,
                            par::Mpi_datatype<T>::value(), op, root, comm);
...
#pragma omp parallel for
    ...
    error_code = MPI_Allgatherv(
        sendBuf, sendCount, par::Mpi_datatype<T>::value(), recvBuf, recvCounts,
        displs, par::Mpi_datatype<T>::value(), comm);
...
#pragma omp parallel for
    ...
    error_code =
        MPI_Allgather(sendBuf, count, par::Mpi_datatype<T>::value(), recvBuf,
                      count, par::Mpi_datatype<T>::value(), comm);
...
template <typename T>
int Mpi_Alltoallv_sparse(T *sendbuf, int *sendcnts, int *sdispls, T *recvbuf,
                         int *recvcnts, int *rdispls, MPI_Comm comm) {
#ifdef __PROFILE_WITH_BARRIER__
    MPI_Barrier(comm);
#endif
    ...
    error_code = Mpi_Alltoallv(sendbuf, sendcnts, sdispls, recvbuf, recvcnts,
                               rdispls, comm);
#else
...
#pragma omp parallel for reduction(+ : commCnt)
    ...
    for (int i = 0; i < rank; i++) {
        if (recvcnts[i] > 0) {
            par::Mpi_Irecv<T>(&(recvbuf[rdispls[i]]), recvcnts[i], i, 1, comm,
                              &(requests[commCnt]));
            commCnt++;
        }
    }
    ...
    for (int i = 0; i < rank; i++) {
        if (sendcnts[i] > 0) {
            par::Mpi_Issend<T>(&(sendbuf[sdispls[i]]), sendcnts[i], i, 1, comm,
                               &(requests[commCnt]));
            commCnt++;
        }
    }
...
#pragma omp parallel for
    ...
    // 27/05/20(Milinda) : A2A_sparse was crashing on impi19 in frontera. I put
    // a barrier here since all2all v should be a blocking call, otherwise
    // repeated calls might cause problems.
    // MPI_Barrier(comm);
...
template <typename T>
int Mpi_Alltoallv_Kway(T *sbuff_, int *s_cnt_, int *sdisp_, T *rbuff_,
                       int *r_cnt_, int *rdisp_, MPI_Comm comm) {
#ifdef __PROFILE_WITH_BARRIER__
    MPI_Barrier(comm);
#endif
    PROF_PAR_ALL2ALLV_DENSE_BEGIN
#ifndef ALLTOALLV_FIX
    error_code =
        Mpi_Alltoallv(sbuff_, s_cnt_, sdisp_, rbuff_, r_cnt_, rdisp_, comm);
#else
...
#pragma omp parallel for
    for (int i = 0; i < np; i++) {
        ...
        memcpy(&sbuff[sdisp[i]] + 2 * sizeof(int), &sbuff_[sdisp_[i]],
               s_cnt[i] - 2 * sizeof(int));
    }
    ...
    while (range[1] - range[0] > 1) {
        iter_cnt++;
        if (kway > range[1] - range[0]) kway = range[1] - range[0];
        ...
        // Communication.
        {
            ...
            for (int i = 0; i < kway; i++) {
                ...
                assert((new_pid < cmp_np
                            ? true
                            : new_range[i] + new_pid ==
                                  new_range[i + 1]));  // Remove this.
                MPI_Sendrecv(&s_cnt[new_range[i] - new_range[0]], cmp_np,
                             MPI_INT, partner, 0, &r_cnt[new_np * i], new_np,
                ...
                if (new_pid == new_np - 1 && cmp_np > new_np) {
                    ...
                    MPI_Sendrecv(&s_cnt_ext[0], cmp_np, MPI_INT, partner, 0,
                                 &r_cnt_ext[new_np * i], new_np, MPI_INT,
            ...
            for (int i_ = 0; i_ <= kway / 2; i_++) {
                ...
                for (int j = 0; j < (i_ == 0 || i_ == kway / 2 ? 1 : 2); j++) {
                    ...
                    int send_dsp_last =
                        sdisp[new_range[i + 1] - new_range[0] - 1];
                    int send_cnt = s_cnt[new_range[i + 1] - new_range[0] - 1] +
                                   send_dsp_last - send_dsp;

                    MPI_Sendrecv(&sbuff[send_dsp], send_cnt, MPI_BYTE, partner,
                                 0, &rbuff[rdisp[new_np * i]],
                                 r_cnt[new_np * (i + 1) - 1] +
                                     rdisp[new_np * (i + 1) - 1] -
                                     rdisp[new_np * i],
                                 MPI_BYTE, partner, 0, comm, &status);
                    ...
                    if (pid == new_np - 1 && cmp_np > new_np) {
                        ...
                        MPI_Sendrecv(NULL, 0, MPI_BYTE, partner, 0,
                                     &rbuff[rdisp_ext[new_np * i]],
                                     r_cnt_ext[new_np * (i + 1) - 1] +
                                         rdisp_ext[new_np * (i + 1) - 1] -
                                         rdisp_ext[new_np * i],
                                     MPI_BYTE, partner, 0, comm, &status);
                    }
                }
            ...
            // Rearrange received data.
            {
                ...
                for (int i = 0; i < new_np; i++)
                    for (int j = 0; j < kway; j++) {
                        ...
                        cnt_new[(i * 2 + 1) * kway + j] =
                            r_cnt_ext[j * new_np + i];
                    }
...
#pragma omp parallel for
                for (int i = 0; i < new_np; i++)
                    for (int j = 0; j < kway; j++) {
                        memcpy(&sbuff[disp_new[(i * 2) * kway + j]],
                               &rbuff[rdisp[j * new_np + i]],
                               r_cnt[j * new_np + i]);
                        memcpy(&sbuff[disp_new[(i * 2 + 1) * kway + j]],
                               &rbuff_ext[rdisp_ext[j * new_np + i]],
                               r_cnt_ext[j * new_np + i]);
                    }
...
template <typename T>
int scatterValues(std::vector<T> &in, std::vector<T> &out, DendroIntL outSz,
                  MPI_Comm comm) {
#ifdef __PROFILE_WITH_BARRIER__
    MPI_Barrier(comm);
#endif
    ...
    // compute the partition offsets and sizes so that All2Allv can be
    // performed. initialize ...
    for (int i = 0; i < npes; i++) {
        sendSz[i] = 0;
    }
    ...
    par::Mpi_Alltoallv_sparse<T>(inPtr, sendSz, sendOff, outPtr, recvSz,
                                 recvOff, comm);
...
int concatenate(std::vector<T> &listA, std::vector<T> &listB, MPI_Comm comm) {
    ...
    par::Mpi_Allreduce<DendroIntL>(locAsz_locBsz, globAsz_globBsz, 2, MPI_SUM,
                                   comm);
    ...
    DendroIntL avgTotalSize =
        ((globAsz_globBsz[0] + globAsz_globBsz[1]) / npes);

    // since listA is not empty on any of the active procs,
    // globASz > npes so avgTotalSize >= 1

    DendroIntL remTotalSize =
        ((globAsz_globBsz[0] + globAsz_globBsz[1]) % npes);
    ...
    if (globAsz_globBsz[1] <= (numSmallProcs * avgTotalSize)) {
        ...
        DendroIntL numBusingAvgPlus1 =
            ((globAsz_globBsz[1]) / (avgTotalSize + 1));
        DendroIntL remBusingAvgPlus1 =
            ((globAsz_globBsz[1]) % (avgTotalSize + 1));
    ...
    assert((numAhighProcs + numAlowProcs + numBothProcs + numBhighProcs +
            numBlowProcs) == npes);
...
int maxLowerBound(const std::vector<T> &keys, const std::vector<T> &searchList,
    ...
    par::Mpi_Alltoallv_sparse<T>(sendKptr, numKeysSend, sendOffsets, recvKptr,
                                 numKeysRecv, recvOffsets, comm);
    ...
    for (unsigned int i = 0; i < totalKeys; i++) {
        unsigned int idx;
        bool found =
            par::maxLowerBound<T>(searchList, recvK[i], &idx, NULL, NULL);
        if (found) {
...
template <typename T>
int partitionW(std::vector<T> &nodeList, unsigned int (*getWeight)(const T *),
               MPI_Comm comm) {
#ifdef __PROFILE_WITH_BARRIER__
    MPI_Barrier(comm);
#endif
...
#pragma omp parallel for reduction(+ : nn)
    ...
    par::Mpi_Alltoallv_sparse<T>(nodeListPtr, sendSz, sendOff, newNodesPtr,
                                 recvSz, recvOff, comm);
...
template <typename T>
int sampleSort(std::vector<T> &arr, std::vector<T> &SortedElem,
               std::vector<double> &stats, MPI_Comm comm) {
// std::cout<<"Sample Sort Execution Begin"<<std::endl;
    ...
    // std::cout << rank << "Nodes Input : " << __func__ << arr.size() <<
    // std::endl;
    ...
    if (totSize < (FIVE * npesLong * npesLong)) {
        if (!myrank) {
            std::cout << " *Using bitonic sort since totSize < (5*(npes^2)). "
                         "totSize: "
        ...
        if (totSize < npesLong) {
            if (!myrank) {
                std::cout << " Input to sort is small. splittingComm: " << npes
                          << " -> " << totSize << std::endl;
            }
            par::splitCommUsingSplittingRank(static_cast<int>(totSize),
                                             &new_comm, comm);
        } else {
            new_comm = comm;
        }
...
#ifdef __DEBUG_PAR__
    if (!myrank) {
        std::cout << "Using sample sort to sort nodes. n/p^2 is fine."
                  << std::endl;
    }
#endif
    ...
    // if (!rank) std::cout << GRN "[samplesort] initial local sort" NRM <<
    // std::endl;
...
#pragma omp parallel for
    for (int i = 1; i < npes; i++) {
        ...
        // std::cout << BLU << "==============================================="
        // << NRM << std::endl;
        //  std::cout << RED " Splittersn in Sample Sort Rank:"<<rank<< NRM <<
        //  std::endl;
        // std::cout << BLU << "==============================================="
        // << NRM << std::endl;
        ...
        temp = ot::TreeNode(1, atoi(results[0].c_str()),
                            atoi(results[1].c_str()), atoi(results[2].c_str()),
    ...
    // std::cout << myrank << ": snedSplit " << sendSplits[0] << "  ||||  " <<
    // sendSplits[1] << std::endl; NOTE: @hari fails for npes=2
    // sort sendSplits using bitonic ...
    // if (!rank)
    // std::cout << rank << RED " [samplesort] bitonicsort " NRM <<
    // sendSplits.size() << std::endl;
    par::bitonicSort<T>(sendSplits, comm);
    // if (!rank)
    // std::cout << rank << GRN " [samplesort] done bitonicsort" NRM <<
    // std::endl;

    // treeNodesTovtk(treenode_spliters,rank,"SplitersafBitTonicSort");

    // std::cout << myrank << ": afterBitonic " << sendSplits[0] << "  ||||  "
    // <<  sendSplits[1] << std::endl;
    ...
    //       std::cout<<"Rank:"<<rank<<" :splittersPtr
    //       Size"<<sizeof(splittersPtr)<<std::endl;
...
#pragma omp parallel for
        for (int i = 1; i < omp_p; i++) {
            // proc_split[i] =
            // seq::BinSearch(&splittersPtr[0],&splittersPtr[npes-1],arr[i*nelem/omp_p],std::less<T>());
            proc_split[i] =
                std::upper_bound(&splittersPtr[0], &splittersPtr[npes - 1],
                                 arr[i * nelem / omp_p], std::less<T>()) -
                &splittersPtr[0];
            if (proc_split[i] < npes - 1) {
                // lst_split_indx[i]=seq::BinSearch(&arr[0],&arr[nelem],splittersPtr[proc_split[i]],std::less<T>());
                lst_split_indx[i] =
                    std::upper_bound(&arr[0], &arr[nelem],
                                     splittersPtr[proc_split[i]],
                                     std::less<T>()) -
                    &arr[0];
            } else {
                proc_split[i] = npes - 1;
...
#pragma omp parallel for
        for (int i = 0; i < omp_p; i++) {
            ...
            for (DendroIntL j = lst_split_indx[i]; j < lst_split_indx[i + 1];
                ...
                if (arr[j] <= splitters[k]) {
                    ...
                    sendcnts_ = 0;
                    k = seq::UpperBound<T>(npes - 1, splittersPtr, k + 1,
                                           arr[j]);
                    if (k == (npes - 1)) {
                        // could not find any splitter >= arr[j]
                        sendcnts_ = (nelem - j);
    ...
    DendroIntL totalRecv =
        rdispls[npes - 1] + recvcnts[npes - 1] - recvcnts[rank];
    DendroIntL totalSend =
        sdispls[npes - 1] + sendcnts[npes - 1] - sendcnts[rank];
    ...
    par::Mpi_Alltoallv_Kway<T>(arrPtr, sendcnts, sdispls, SortedElemPtr,
                               recvcnts, rdispls, comm);
    ...
    MPI_Reduce(&splittercal, splitterCalculation_g, 1, MPI_DOUBLE, MPI_MIN, 0,
               comm);
    MPI_Reduce(&splittercal, splitterCalculation_g + 1, 1, MPI_DOUBLE, MPI_SUM,
               0, comm);
    MPI_Reduce(&splittercal, splitterCalculation_g + 2, 1, MPI_DOUBLE, MPI_MAX,
               0, comm);
    ...
    MPI_Reduce(&totalSend, sendCounts_g + 1, 1, MPI_LONG_LONG, MPI_SUM, 0,
               comm);
    MPI_Reduce(&totalSend, sendCounts_g + 2, 1, MPI_LONG_LONG, MPI_MAX, 0,
               comm);
    sendCounts_g[1] = sendCounts_g[1] / (double)npes;

    MPI_Reduce(&totalRecv, recvCounts_g, 1, MPI_LONG_LONG, MPI_MIN, 0, comm);
    MPI_Reduce(&totalRecv, recvCounts_g + 1, 1, MPI_LONG_LONG, MPI_SUM, 0,
               comm);
    MPI_Reduce(&totalRecv, recvCounts_g + 2, 1, MPI_LONG_LONG, MPI_MAX, 0,
               comm);
    ...
    double ss_time = (std::chrono::duration_cast<std::chrono::milliseconds>(
                          (ss_end - ss_start))
                          .count()) /
                     (MILLISECOND_CONVERSION);
    double ss_time_g[3];
    ...
    if (!rank) {
        /*std::cout << "SS (min mean max) \t TS (min mean max)" << std::endl;
               std::cout << ss_time_g[0] << "\t" << ss_time_g[1] << "\t" <<
           ss_time_g[2] <<std::endl;*/
 ...
 */
template <typename T>
void MergeSplit(std::vector<T> &local_list, int which_keys, int partner,
                MPI_Comm comm) {
    MPI_Status status;
    ...
    par::Mpi_Sendrecv<int, int>(&send_size, 1, partner, 0, &recv_size, 1,
                                partner, 0, comm, &status);
    ...
    par::Mpi_Sendrecv<T, T>(local_listPtr, send_size, partner, 1, temp_listPtr,
                            recv_size, partner, 1, comm, &status);
...
template <typename T>
void Par_bitonic_sort_incr(std::vector<T> &local_list, int proc_set_size,
                           MPI_Comm comm) {
    int eor_bit;
    int proc_set_dim;
    int stage;
...
template <typename T>
void Par_bitonic_sort_decr(std::vector<T> &local_list, int proc_set_size,
                           MPI_Comm comm) {
    int eor_bit;
    int proc_set_dim;
    int stage;
...
template <typename T>
void Par_bitonic_merge_incr(std::vector<T> &local_list, int proc_set_size,
                            MPI_Comm comm) {
    int partner;
    int rank, npes;
...
void bitonicSort_binary(std::vector<T> &in, MPI_Comm comm) {
    ...
    for (proc_set_size = 2, and_bit = 2; proc_set_size <= npes;
...
void bitonicSort(std::vector<T> &in, MPI_Comm comm) {
    ...
    if (npes > 1) {
        ...
        if (isPower) {
            ...
            // Since npes is not a power of two, we shall split the problem in
            // two ...
            //
            // 1. Create 2 comm groups ... one for the 2^d portion and one for
            // the remainder.
            ...
            Par_bitonic_merge_incr(in, binOp::getNextHighestPowerOfTwo(npes),
                                   comm);
...
void MergeLists(std::vector<T> &listA, std::vector<T> &listB, int KEEP_WHAT) {
    ...
    for (size_t i = 0; i < list_size; i++) {
        ...
        if ((index1 < listA.size()) &&
            ((index2 >= listB.size()) || (listA[index1] <= listB[index2]))) {
    ...
    if (KEEP_WHAT == KEEP_LOW) {
        ...
        while (((scratch_list[ii] < _low) || (ii < (list_size / 2))) &&
        ...
        int ii = (list_size - 1);
        while (((ii >= (list_size / 2)) && (scratch_list[ii] >= _low)) ||
               (scratch_list[ii] > _high)) {
            ii--;
        }
        if (ii < (list_size - 1)) {
...
template <typename T>
void parallel_rank(const T *in, unsigned int sz, DendroIntL *out,
                   MPI_Comm comm) {
    int rank, npes;
    ...
    // for(unsigned int i=0;i<key_sorted.size();i++)
    // {
    //   std::cout<<"val: "<<key_sorted[i].val<<" p: "<<key_sorted[i].p<<" rank:
    //   "<<key_sorted[i].rank<<" idx : "<<key_sorted[i].idx<<std::endl;
    // }
    ...
    for (unsigned int i = 0; i < key_sorted.size(); i++) {
        sBuf[sOffset[key_sorted[i].p] + ccount[key_sorted[i].p]] =
            key_sorted[i];
    ...
    MPI_Alltoallv(&(*(sBuf.begin())), sCounts, sOffset, MPI_BYTE,
                  &(*(key.begin())), rCounts, rOffset, MPI_BYTE, comm);

    for (unsigned int i = 0; i < key.size(); i++) out[key[i].idx] = key[i].rank;
