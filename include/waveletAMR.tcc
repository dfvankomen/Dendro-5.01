...
template <typename T>
double compute_element_wavelet(const ot::Mesh* pMesh, const WaveletEl* wRefEl,
                               const T* eleUnzip, double wavelet_tol,
                               unsigned int dof, bool isBdyEle) {
    double wMax = 0.0;
        ...
        for (unsigned int v = 0; v < dof; v++) {
            ((WaveletEl*)wRefEl)
                ->compute_wavelets_3D((double*)(eleUnzip + v * sz_per_dof), isz,
                                      wCout, isBdyEle);
            const double l_max =
                (normL2(wCout.data(), wCout.size())) / sqrt(wCout.size());
...
template <typename T>
bool compute_wavelet_remesh_flags(
    ...
    if (pMesh->isActive()) {
        ...
        for (unsigned int blk = 0; blk < blkList.size(); blk++) {
            ...
            for (unsigned int ele = blkList[blk].getLocalElementBegin();
                ...
                eleWMax[ele - eleOfst] = compute_element_wavelet(
                    pMesh, wrefEl, blkIn.data(), tol_ele, numVars, isBdyOct);
