...
inline void kim_filter_cal_coeff(double* c, double kc) {
    double AF = 30.0 - 5.0 * cos(kc) + 10.0 * cos(2.0 * kc) - 3.0 * cos(3.0 * kc);
...
inline MatrixDiagonalEntries* create_Kim_filter_diagonals(
    ...
    // New layout:
    //   F_coeffs[0] -> sigma (filter strength)
    //   F_coeffs[1] -> kc_factor
    //   F_coeffs[2] -> eps
    const double sigma     = (F_coeffs.size() > 0) ? F_coeffs[0] : 1.0;
    const double kc_factor = (F_coeffs.size() > 1) ? F_coeffs[1] : 1.0;
    const double eps       = (F_coeffs.size() > 2) ? F_coeffs[2] : 0.0;

    const double kc  = kc_factor * pi;
    ...
    const double t2    = sin(M_PI / 2.0);
    const double t3    = sin(M_PI / 3.0);
    const double t6    = sin(M_PI / 6.0);

    const double kcd   = kc * (1.0 - eps * t6 * t6);
    const double kcdd  = kc * (1.0 - eps * t3 * t3);
    const double kcddd = kc * (1.0 - eps * t2 * t2);
    ...
    const double BF =
        ...
        (5.0 + 35.0 * betaFdd - 29.0 * betaFdd * betaFdd) * alphaFdd +
    ...
    // === NEW: bake sigma into S only ===
    if (sigma != 1.0) {
        for (double &v : SDiagInterior) {
            v *= sigma;
        }
        for (auto &row : SDiagBoundary) {
            for (double &v : row) {
                v *= sigma;
            }
        }
