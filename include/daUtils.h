...
/**
 * @brief interpolates input vector and a local element to the given coordinate
 * which lies in the specified octant
 ...
 * @param[in] elementID: element ID (note that the coord should lie in the
 * elementID)
...
/**
 * @brief Lagrange linear interp, function computes the linear order simplex
 * containting the pt, and then use the linear lagrange interp on the element to
 * interpolate.
 * @param[in] mesh : input mesh
 * @param[in] in : input vector (zipped) version defined on the mesh.
 * @param[in] domain_coord: coordinate in domain basis
 ...
 * @assumption: This routine assumes that, it transforms the octree cube domain
 * to the physical domain of cube.
 * @param[in] mesh : input mesh
 * @param[in] in : input vector (zipped) version defined on the mesh.
 * @param[in] domain_coords: pointer to the list of coordinates in the domain
 * reference.  (specified in the global coords, coords that are not in the local
 * parition will be ignored)
 * @param[in] length: input length of the coordinates
 * @param[in] grid_limit : size 2 array of points for min and max point on the
 * grid.
 * @param[in] domain_limit : size 2 array of points for min and max point on the
 * domain.
