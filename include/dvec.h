...
class DVector {
   ...
   public:
     ...
     * @param isUnzip : true if this is unzip vector format (block local vector
     * format)
     * @param isElemental : true if this is elemental vector
     * @param dof : number of degrees of freedoms.
     */
    void create_vector(const ot::Mesh* pMesh, DVEC_TYPE type, DVEC_LOC loc,
                       unsigned int dof = 1, bool allocate_ghost = true);
     ...
     */
    void set_vec_ptr(T*& ptr, const ot::Mesh* pMesh, DVEC_TYPE type,
                     DVEC_LOC loc, unsigned int dof, bool allocate_ghost);
     ...
     */
    bool operator==(DVector<T, I>& other) const {
    ...
    /**
     * @brief returns the 2D vector. Note that the v2d assumeed to allocated no
    ...
    static void grid_transfer(ot::Mesh* m_old, const ot::Mesh* m_new,
                              DVector<T, I>& v);
};
...
template <typename T, typename I>
void DVector<T, I>::create_vector(const ot::Mesh* pMesh, DVEC_TYPE type,
    ...
    if (m_vec_type == DVEC_TYPE::OCT_SHARED_NODES)
        (allocate_ghost) ? m_size = pMesh->getDegOfFreedom()* m_dof
                         : m_size = pMesh->getNumLocalMeshNodes() * m_dof;
    else if (m_vec_type == DVEC_TYPE::OCT_LOCAL_NODES)
        (allocate_ghost) ? m_size = pMesh->getDegOfFreedomDG()* m_dof
                         : m_size = pMesh->getNumLocalMeshElements() *
                                    pMesh->getNumNodesPerElement() * m_dof;
    else if (m_vec_type == DVEC_TYPE::OCT_LOCAL_WITH_PADDING)
        (allocate_ghost) ? m_size = pMesh->getDegOfFreedomUnZip()* m_dof
                         : m_size = pMesh->getDegOfFreedomUnZip() * m_dof;
    else if (m_vec_type == DVEC_TYPE::OCT_CELL_CENTERED)
        (allocate_ghost) ? m_size = pMesh->getAllElements().size()* m_dof
                         : m_size = pMesh->getNumLocalMeshElements() * m_dof;
    else {
        dendro_log(
            " unknown type in DVector, expected a value between 0 and 3, "
            "got: " +
            std::to_string(m_vec_type));
        MPI_Abort(m_comm, 0);
    }

    // NOTE: we don't want to allocate the memory if we're not active. But we
    // *must* have the vector initialized.
    if (!(pMesh->isActive())) return;
...
template <typename T, typename I>
void DVector<T, I>::axpy(const ot::Mesh* const pMesh, T a,
                         const DVector<T, I>& x, DVector<T, I>& y,
                         bool local_only) {
...
template <typename T, typename I>
void DVector<T, I>::axpby(const ot::Mesh* const pMesh, T a,
                          const DVector<T, I>& x, T b, DVector<T, I>& y,
                          bool local_only) {
    if (y.m_data_ptr == nullptr) return;
...
template <typename T, typename I>
void DVector<T, I>::grid_transfer(ot::Mesh* m_old, const ot::Mesh* m_new,
                                  DVector<T, I>& dvec) {
    ...
    const unsigned int sz_per_dof_new =
        (dof != 0) ? vec_tmp.get_size() / dof : 0;

    if (dvec.m_vec_type == DVEC_TYPE::OCT_SHARED_NODES)
        m_old->interGridTransfer(in, out, m_new,
