 ...
 * @breif Refactored version of the TreeNode class with the minimum data
 * required.
 * @remarks m_uiMaxDepth is removed from the arttributre list and listed as a
 * global variable.
 * */
...
class TreeNode {
   ...
   public:
    TreeNode(const int dummy, const unsigned int x, const unsigned int y,
             const unsigned int z, const unsigned int level,
             const unsigned int dim, const unsigned int maxDepth);

    TreeNode(const unsigned int x, const unsigned int y, const unsigned int z,
             const unsigned int level, const unsigned int dim,
   ...
   public:
    ...
    /**
      @author Milinda Fernando
      @brief set the m_uiFlag value. Which is used to store the level and other
      aditional info.
      */
    ...
    /**
      @author Milinda Fernando
      @brief get  the m_uiFlag value. Which is used to store the level and other
      aditional info.
      */
    ...
    /**
     * @author Milinda Fernando
     * @brief Can be used the increment octant level by 1. Used in SFC
     * treeSearch function.
     * */
    void incrementLevel();
    ...
    /**
    @author Rahul Sampath
    @brief Two octants are equal if their respective anchors are equal and their
    levels are equal.
    */
    ...
    /**
      @author Rahul Sampath
      @brief Two octants are equal if their respective anchors are equal and
    ...
    /**
     ...
     @brief The comparisons are based on the Morton/Hilbert
            ...
            @remarks Hilbert based comparision operator is O(nlog(n)). Which is
     costly than the standard Morton implmentation.
     */
    ...
    /**
     @author Milinda Fernando
     @brief returns true min corner(anchor) and the max corner coordinates of a
     octant.
     */

    inline unsigned int minX() const { return getX(); }  // end fn.
    ...
    /** Returns the deepest first decent  of the considering octant based on the
     * SFC being used. */
    TreeNode getDFD() const;

    /** Returns the deepest last decent of the considering octant based on the
     * SFC beging used*/
    ...
    */
     ...
     * @author Rahul Sampath
     * @author Milinda Fernando
     * @breif Following are the attributes and functionalities that needed by
     * mesh computation based on the 2:1 balanced octree. Most of the functions
     * taken from the old dendro implementation.
     * */
    ...
    */
    bool isBoundaryOctant(int type             = POSITIVE,
                          unsigned char *flags = NULL) const;
    ...
    /**
      ...
      smallest 3 are for -z,-y and -x axes.
      */
    bool isBoundaryOctant(const TreeNode &block, int type = POSITIVE,
                          unsigned char *flags = NULL) const;
    ...
    int getChildrenInMortonOrdering(std::vector<ot::TreeNode> &children) const {
        ...
        unsigned int len =
            (unsigned int)(1u << (maxDepth -
                                  ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1)));

        TreeNode tmpNode0(m_uiX, m_uiY, m_uiZ,
                          ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1), m_uiDim,
                          m_uiMaxDepth);
        children[0] = tmpNode0;

        TreeNode tmpNode1((m_uiX + len), m_uiY, m_uiZ,
                          ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1), m_uiDim,
                          m_uiMaxDepth);
        children[1] = tmpNode1;

        if (dim >= 2) {
            TreeNode tmpNode2(m_uiX, (m_uiY + len), m_uiZ,
                              ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1),
                              m_uiDim, m_uiMaxDepth);
            children[2] = tmpNode2;

            TreeNode tmpNode3((m_uiX + len), (m_uiY + len), m_uiZ,
                              ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1),
                              m_uiDim, m_uiMaxDepth);
            children[3] = tmpNode3;
        }
        ...
        if (dim == 3) {
            TreeNode tmpNode4(m_uiX, m_uiY, (m_uiZ + len),
                              ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1),
                              m_uiDim, m_uiMaxDepth);
            children[4] = tmpNode4;

            TreeNode tmpNode5((m_uiX + len), m_uiY, (m_uiZ + len),
                              ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1),
                              m_uiDim, m_uiMaxDepth);
            children[5] = tmpNode5;

            TreeNode tmpNode6(m_uiX, (m_uiY + len), (m_uiZ + len),
                              ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1),
                              m_uiDim, m_uiMaxDepth);
            children[6] = tmpNode6;

            TreeNode tmpNode7((m_uiX + len), (m_uiY + len), (m_uiZ + len),
                              ((m_uiLevel & ot::TreeNode::MAX_LEVEL) + 1),
                              m_uiDim, m_uiMaxDepth);
            children[7] = tmpNode7;
        }  // end if
...
inline unsigned int TreeNode::getY() const { return m_uiY; }

inline unsigned int TreeNode::getZ() const { return m_uiZ; }

inline unsigned int TreeNode::getDim() const { return m_uiDim; }

inline unsigned int TreeNode::getMaxDepth() const { return m_uiMaxDepth; }
...
inline bool TreeNode::isRoot() const {
    return ((this->getLevel() == 0) & (m_uiX == 0) & (m_uiY == 0) &
            (m_uiZ == 0));
}

inline void TreeNode::incrementLevel() {
    if (this->getLevel() < m_uiMaxDepth) m_uiLevel++;
}
...
inline bool TreeNode::isAncestor(const TreeNode &other) const {
    ...
    bool state1=( (this->getLevel() < other.getLevel()) && ( (min2[0] >=
    min1[0]) && (min2[1] >= min1[1]) && (min2[2] >= min1[2]) && (max2[0] <=
    max1[0]) && (max2[1] <= max1[1]) && (max2[2] <= max1[2]) ));

    return state1;*/

    return ((this->getLevel() < other.getLevel()) &&
            ((other.minX() >= this->minX()) && (other.minY() >= this->minY()) &&
             (other.minZ() >= this->minZ()) && (other.maxX() <= this->maxX()) &&
             (other.maxY() <= this->maxY()) && (other.maxZ() <= this->maxZ())));
}
...
inline bool TreeNode::operator<(TreeNode const &other) const {
#ifdef __DEBUG_TN__
    if (((this->m_uiDim) != (other.m_uiDim)) ||
        ((this->m_uiMaxDepth) != (other.m_uiMaxDepth))) {
        std::cout << "Me: " << (*this) << " Other: " << other << std::endl;
        std::cout << "My Dim: " << m_uiDim << " OthDim: " << other.m_uiDim
    ...
    if ((this->m_uiX == other.m_uiX) && (this->m_uiY == other.m_uiY) &&
        (this->m_uiZ == other.m_uiZ)) {
    ...
    // If you need to initilize the Hilbert table and the rotations for 2D you
    // need to define DENDRO_DIM2 Default initialization for 3D case. NOTE: To
    // work the Hilbert Ordering You need the Hilbert Table Initialized.
    ...
    //            std::cout<<"NCALEV:"<<ncaLev<<"\t
    //            this_lev:"<<this->getLevel()<<"\t
    //            other_lev:"<<other.getLevel()<<std::endl;
    //
    //            std::cout<<std::endl;
    //
    //        }
    ...
    unsigned int num_children =
        1u << dim;  // This is basically the hilbert table offset
    ...
    mid_bit--;
    index1 = ((((z1 & (1u << mid_bit)) >> mid_bit) << 2u) |
              (((y1 & (1u << mid_bit)) >> mid_bit) << 1u) |
              ((x1 & (1u << mid_bit)) >> mid_bit));
    index2 = ((((z2 & (1u << mid_bit)) >> mid_bit) << 2u) |
              (((y2 & (1u << mid_bit)) >> mid_bit) << 1u) |
              ((x2 & (1u << mid_bit)) >> mid_bit));

    return rotations[rot_offset * current_rot + num_children + index1] <
           rotations[rot_offset * current_rot + num_children + index2];
...
inline TreeNode TreeNode::getParent() const {
    ...
    unsigned int parLev =
        (((m_uiLevel & MAX_LEVEL) > 0) ? ((m_uiLevel & MAX_LEVEL) - 1) : 0);
...
inline int TreeNode::getAnchor(unsigned int &x, unsigned int &y,
...
inline std::vector<TreeNode> TreeNode::getAllNeighbours() const {
    /*
       0 = Left;  1 =  Right;  2 =  Front;  3 = Back;   4 = LeftBack;  5 =
       RightBack;  6 = LeftFront;  7 = RightFront;  8 = Top; 9 = TopRight;  10 =
       TopBack;  11 =  TopRightBack;  12 =  Bottom;  13 =  BottomBack;  14 =
       TopLeft;  15 =  BottomLeft; 16 =  BottomRight;  17 =  TopFront;  18 =
       BottomFront;  19 =  TopLeftFront;  20 =  TopRightFront;  21 =
       BottomLeftFront; 22 =  BottomRightFront;  23 =  TopLeftBack;  24 =
       BottomLeftBack;  25 = BottomRightBack;
       */
...
inline unsigned int ot::TreeNode::getMortonIndex() const {
    ...
    return ((((m_uiZ >> mid_bit) & 1u) << 2u) |
            (((m_uiY >> mid_bit) & 1u) << 1u) | ((m_uiX >> mid_bit) & 1u));
}
...
communicating messages of type "ot::TreeNode".
*/
template <>
class Mpi_datatype<ot::TreeNode> {
    static void Node_MAX_LEVEL(void *in, void *inout, int *len,
                               MPI_Datatype *dptr) {
        for (int i = 0; i < (*len); i++) {
    ...
    /* static void Node_NCA(void *in, void *inout, int* len, MPI_Datatype *
     ...
     (static_cast<ot::TreeNode*>(in))[i]; ot::TreeNode second =
     (static_cast<ot::TreeNode*>(inout))[i]; if( first != second ) {
                 (static_cast<ot::TreeNode*>(inout))[i] = ot::getNCA(first,
     second);
             }//end if
         }//end for
     }//end function*/
   ...
   public:
    /**
      @brief User defined MPI_Operation that sets second[i] to first[i] if
    first[i] is at a greater level than second[i].
    ...
    **/
    /*static MPI_Op MAX_LEVEL(){
        ...
        if (first) {
            first = false;
            MPI_Op_create(Mpi_datatype<ot::TreeNode>::Node_MAX_LEVEL ,true
    ,&maxLev);
        }
        return maxLev;
    }*/
    ...
    /**
      @brief User defined MPI_Operation that computes: second[i] = Max(first[i],
    second[i]),
      @remark first and second are 2 arrays of type TreeNode.
      "MAX" is a macro
    **/
    ...
    /**
      @brief User defined MPI_Operation that computes: second[i] = Min(first[i],
    second[i]),
      @remark first and second are 2 arrays of type TreeNode.
      "MIN" is a macro
    **/
    ...
    /**
      @brief User defined MPI_Operation that computes: second[i] = NCA(first[i],
    second[i]),
