...
@author Milinda Fernando ,milinda@cs.utah.edu
 ...
 @remarks Most of the functions used for the mesh generation. Most of the
implementations are based on the previous implementation of dendro version 4.0
...
void addBoundaryNodesType1(std::vector<ot::TreeNode>& in,
                           std::vector<ot::TreeNode>& bdy, unsigned int dim,
 ...
 * @brief   Generates an octree based on a function provided by the user based
 * on the Wavelet method to decide on adaptivity.
 * @param   fx        the function that taxes $x,y,z$ coordinates and returns
 * the the value at that point
 * @param   maxDepth  The maximum depth that the octree should be refined to.
 * @param   tol       user specified tolerance for the wavelet representation of
 * the function.
 * @param   elementOrder order of the element when defining the wavelet
 * representation of the function.
 * @param   comm      The MPI communicator to be use for parallel computation.
 *
 * Generates an octree based on a function provided by the user. The function is
 * expected to return the signed distance to the surface that needs to be
 * meshed. The coordinates are expected to be in [0,1]^3.
 */

int function2Octree(std::function<double(double, double, double)> fx,
                    std::vector<ot::TreeNode>& nodes, unsigned int maxDepth,
                    const double& tol, unsigned int elementOrder,
                    MPI_Comm comm);
 ...
 * @brief   Generates an octree based on a function provided by the user based
 * on the Wavelet method to decide on adaptivity.
 * @param[in] fx:        the function that taxes $x,y,z$ coordinates and returns
 * the the value at that point
 * @param[in] numVars: number of total variables computed from fx function.
 * @param[in] varIndex: variable index location that can be used to ascess
 * double * pointer in fx, to determine the refinement of octree.
 * @param[in] numInterpVars: Number of variables considered during the
 * refinement
 * @param[in] maxDepth:  The maximum depth that the octree should be refined to.
 * @param[in] tol       user specified tolerance for the wavelet representation
 * of the function.
 * @param[in] elementOrder order of the element when defining the wavelet
 * representation of the function.
...
int function2Octree(std::function<void(double, double, double, double*)> fx,
                    const unsigned int numVars, const unsigned int* varIndex,
...
/**
 * @author Milinda Fernando
 * @brief ensures that the children of the same parent not partioned arcross
 ...
 *
 * */
void enforceSiblingsAreNotPartitioned(std::vector<ot::TreeNode>& in,
                                      MPI_Comm comm);
 ...
 *
 * @param [in] pNodes 2:1 balance sorted octree. Note that pNodes should be a
 * sorted octree.
...
void octree2BlockDecomposition(std::vector<ot::TreeNode>& pNodes,
                               std::vector<ot::Block>& blockList,
                               unsigned int maxDepth, unsigned int& d_min,
                               unsigned int& d_max, DendroIntL localBegin,
...
void blockListToVtk(std::vector<ot::Block>& blkList,
                    const std::vector<ot::TreeNode>& pNodes, char* fNamePrefix,
                    MPI_Comm comm);
 ...
 * */
template <typename T>
void computeSFCBucketSplitters(const T* pNodes, int lev, unsigned int maxDepth,
                               unsigned char rot_id, DendroIntL& begin,
                               DendroIntL& end, DendroIntL* splitters);
...
template <typename T>
bool linearSearch(const T* pNodes, const T& key, unsigned int n,
                  unsigned int sWidth, unsigned int& result);

/**
 * @author Milinda Fernando
 * @brief Performs repartition of the pNodes array based on the provided
 * splitters.
 * @note assumption 1: Assumes that pNodes is sorted and unique, and pNodes
 * contains the splitters. (because of that this might not be a complete
 * octree.)
 ...
 * @param [in] numSplitters: number of splitters (should be equal to the comm
 * size. )
 * @param [in] comm: MPI communicator.
 *
 * */
template <typename T>
void partitionBasedOnSplitters(std::vector<T>& pNodes, const T* splitters,
                               unsigned int numSplitters, MPI_Comm comm);

/**
 * @brief shrink or expand the input octree while and enforce sorting
 * afterwards.
 * @param[in] in: input octree
 * @param[in] comm1: Number of partitions needed. (or after this method input
 * octree is going to end up in comm1)
 * @para[in] comm2: current communicator.
 * */
template <typename T>
void shrinkOrExpandOctree(
    std::vector<T>& in, const double ld_tol, const unsigned int sf_k,
...
void printBlockStats(const Blk* blkList, unsigned int n, unsigned int maxDepth,
                     MPI_Comm comm);
 ...
 */
unsigned int rankSelectRule(unsigned int size_global, unsigned int rank_global,
                            unsigned int size_local, unsigned int rank_i);

/**
 *@brief returns true if the rank_global is selected for size_local comm. based
 *on the rankSelectRule
 *@param[in] size_global : global comm. size
 *@param[in] rank_global : global rank
 *@param[in] size_local : local size
 * @return returns true if the rank_global is selected for size_local comm.
 *based on the rankSelectRule
 */
inline bool isRankSelected(unsigned int size_global, unsigned int rank_global,
                           unsigned int size_local) {
    bool isSelected = false;
    for (unsigned int p = 0; p < size_local; p++)
        if (rank_global ==
            rankSelectRule(size_global, rank_global, size_local, p)) {
            isSelected = true;
            break;
        }
    return isSelected;
...
void computeSFCBucketSplitters(const T* pNodes, int lev, unsigned int maxDepth,
    ...
    if ((lev >= maxDepth) || (begin == end)) {
        ...
        for (int ii = 0; ii < NUM_CHILDREN; ii++) {
            ...
            if (ii == (NUM_CHILDREN - 1))
                ...
                nextIndex =
                    (rotations[2 * NUM_CHILDREN * rot_id + ii + 1] - '0');
        ...
        // std::cout<<"End return "<<"maxDepth "<<maxDepth<<" Lev: "<<lev<< "
    ...
    for (DendroIntL i = begin; i < end; ++i) {
        /*cnum = (lev < pNodes[i].getLevel())? 1 +(((((pNodes[i].getZ() & (1u <<
           mid_bit)) >> mid_bit) << 2u) |
                                                  (((pNodes[i].getY() & (1u <<
           mid_bit)) >> mid_bit) << 1u) |
                                                  ((pNodes[i].getX() & (1u <<
        ...
        cnum = (lev < pNodes[i].getLevel())
                   ? 1 + ((((pNodes[i].getZ() >> mid_bit) & 1u) << 2u) |
                          (((pNodes[i].getY() >> mid_bit) & 1u) << 1u) |
                          ((pNodes[i].getX() >> mid_bit) & 1u))
                   : 0;
        count[cnum + 1]++;
    }
    ...
    // if(count[1]>0) std::cout<<"For rank: "<<rank<<" count [1]:
    // "<<count[1]<<std::endl;
    ...
    for (unsigned int ii = 0; ii < NUM_CHILDREN; ii++) {
        ...
        if (ii == 0) {
            splitters[index] = begin;
            splitters[nextIndex] =
                splitters[index] + count[1] +
                count[(index +
                       2)];  // number of elements which needs to come before
                             // the others due to level constraint.

        } else {
            splitters[nextIndex] = splitters[index] + count[(index + 2)];
        }
        //   if(count[1]>0 & !rank) std::cout<<" Spliter B:"<<index <<"
        //   "<<splitters[index]<<" Splitters E "<<nextIndex<<"
        //   "<<splitters[nextIndex]<<std::endl;
    }
}
...
void partitionBasedOnSplitters(std::vector<T>& pNodes, const T* splitters,
    ...
    SFC::seqSort::SFC_treeSort(&(*(pNodes.begin())), pNodes.size(), tmpVec,
                               tmpVec, tmpVec, m_uiMaxDepth, m_uiMaxDepth,
                               rootNode, ROOT_ROTATION, 1,
                               TS_REMOVE_DUPLICATES);
    ...
    SFC::seqSearch::SFC_treeSearch(
        &(*(splitterKeys.begin())), &(*(pNodes.begin())), 0, numSplitters, 0,
    ...
    /* if(!rank)
     {
         for(unsigned int p=0;p<npes;p++)
         {
             std::cout<<" p: "<<p<<" splitterKey: "<<splitterKeys[p]<<"
     searchResult: "<<splitterKeys[p].getSearchResult()<<std::endl;
         }
     }*/
    ...
    for (unsigned int p = 1; p < npes; p++) {
        ...
        if ((sResultPrev + 1) < pNodes.size() &&
            ((sResultPrev + 1) < sResult)) {
    ...
    // if(!rank) std::cout<<"rank: "<<rank<<" pNodes size:
    // "<<pNodes.size()<<std::endl;

    assert(sendBuffer.size() == (sendCount[npes - 1] + sendOffset[npes - 1]));
    par::Mpi_Alltoallv(&(*(sendBuffer.begin())), sendCount, sendOffset,
                       &(*(pNodes.begin())), recvCount, recvOffset, comm);

    for (unsigned int ele = 0; ele < pNodes.size(); ele++) {
        if (pNodes[ele].getLevel() == m_uiMaxDepth)
            std::cout << "rank: " << rank << " ele: " << ele
...
template <typename T>
void genEdgeSearchKeys(const T& elem, unsigned int blkId,
                       std::vector<ot::SearchKey>& sKeys) {
    ...
    if (myX > 0 && myY > 0) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX - 1), (myY - 1), (myZ),
    ...
    if (myX > 0 && (myY + mySz) < domain_max) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX - 1), (myY + mySz), (myZ),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
    ...
    if (myX > 0 && myZ > 0) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX - 1), (myY), (myZ - 1),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
    ...
    if (myX > 0 && (myZ + mySz) < domain_max) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX - 1), (myY), (myZ + mySz),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
    ...
    if ((myX + mySz) < domain_max && myY > 0) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX + mySz), (myY - 1), (myZ),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
    ...
    if ((myX + mySz) < domain_max && (myY + mySz) < domain_max) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX + mySz), (myY + mySz), (myZ),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
    ...
    if ((myX + mySz) < domain_max && myZ > 0) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX + mySz), (myY), (myZ - 1),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
    ...
    if ((myX + mySz) < domain_max && (myZ + mySz) < domain_max) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX + mySz), (myY), (myZ + mySz),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
    ...
    if (myY > 0 && myZ > 0) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX), (myY - 1), (myZ - 1),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
    ...
    if (myY > 0 && (myZ + mySz) < domain_max) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX), (myY - 1), (myZ + mySz),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
    ...
    if ((myY + mySz) < domain_max && myZ > 0) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX), (myY + mySz), (myZ - 1),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
    ...
    if ((myY + mySz) < domain_max && (myZ + mySz) < domain_max) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX), (myY + mySz), (myZ + mySz),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(blkId);
...
template <typename T>
void shrinkOrExpandOctree(std::vector<T>& in, const double ld_tol,
                          const unsigned int sf_k, bool isActive,
                          MPI_Comm activeComm, MPI_Comm globalComm,
    ...
    if (!rank_g) {
        if (!isActive) {
            std::cout << "[Shrink/Expand Error]: active communicator does not "
                         "include global rank=0. "
                      << std::endl;
            exit(0);
        }
    }
    ...
    int activeCommSz = 0;
    if (!rank_g) MPI_Comm_size(activeComm, &activeCommSz);

    par::Mpi_Bcast(&activeCommSz, 1, 0, globalComm);

    assert(activeCommSz <= npes_g);
    if (activeCommSz > npes_g) {
        std::cout << "[Shrink/Expand Error]: active communicator size is "
                     "larger than the global comm. "
                  << std::endl;
        exit(0);
    }
    ...
    for (unsigned int i = 0; i < npes_g; i++) sendCount[i] = 0;
    ...
    par::Mpi_Alltoallv_sparse(&(*(in.begin())), sendCount, sendOffset,
                              &(*(recvBuf.begin())), recvCount, recvOffset,
                              globalComm);
    std::swap(in, recvBuf);
    recvBuf.clear();
    ...
    if (isActive) {
        ...
        // @note: should not need a remove duplicates. but we ge duplicates if
        // we did not do it. Just find out why .
        SFC::parSort::SFC_treeSort(in, recvBuf, recvBuf, recvBuf, ld_tol,
                                   m_uiMaxDepth, rootTN, ROOT_ROTATION, 1,
                                   TS_REMOVE_DUPLICATES, sf_k, activeComm);
        std::swap(in, recvBuf);
        recvBuf.clear();
        if (getWeight != NULL) par::partitionW(in, getWeight, activeComm);
...
void printBlockStats(const Blk* blkList, unsigned int n, unsigned int maxDepth,
    ...
    for (unsigned int i = 0; i < maxDepth; i++) blk_counts[i] = 0;

    unsigned int ele1D, index;
    for (unsigned int blk = 0; blk < n; blk++) {
        ele1D = blkList[blk].get1DArraySize();
        index = (blkList[blk].get1DArraySize() -
                 2 * blkList[blk].get1DPadWidth() - 1) /
                blkList[blk].getElementOrder();
        blk_counts[index]++;
    }
    ...
    if (!rank) {
        for (unsigned int k = 0; k < maxDepth; k++) {
            std::cout << "blk_lev[" << k << "]: ";
            for (unsigned int w = 0; w < blk_counts_g[k]; w++) std::cout << "*";
            std::cout << std::endl;
        }
    }
 ...
 * @param[out] octsByLevLocal : array size of m_uiMaxDepth where octsByLev[i]
 * denotes the number of octants for level i for each rank.
 * @param[out] octsByLevGlobal: reduction of octsByLevLocal with comm, and
 * operator +
 * @param[out] regOcts: precentage of regular octants in the entrie octree.
 */
template <typename T>
void computeOctreeStats(const T* in, unsigned int n,
                        unsigned int* octsByLevLocal,
                        unsigned int* octsByLevGlobal, double& regOcts,
                        MPI_Comm comm) {
    ...
    for (unsigned int i = 0; i < n; i++) octsByLevLocal[in[i].getLevel()]++;

    par::Mpi_Allreduce(octsByLevLocal, octsByLevGlobal, m_uiMaxDepth, MPI_SUM,
                       comm);

    octsScanByLev[0] = 0;
    omp_par::scan(octsByLevGlobal, octsScanByLev, m_uiMaxDepth);

    DendroIntL totalOcts =
        octsScanByLev[m_uiMaxDepth - 1] + octsByLevGlobal[m_uiMaxDepth - 1];
