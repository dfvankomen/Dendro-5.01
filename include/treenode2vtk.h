...
// void treeNodesTovtk(std::vector<ot::TreeNode>& nodes,int mpi_rank,std::string
// vtk_file_name,bool hsorted=false);
template <typename T>
void treeNodesTovtk(std::vector<T>& nodes, int mpi_rank,
                    std::string vtk_file_name, bool hsorted = false);
...
void treeNodesTovtk(std::vector<T>& nodes, int mpi_rank,
    ...
    if (dim == 2) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            myfile << (xl + len) << " " << (yl + len) << " " << (zl + len)
                   << std::endl;
            myfile << xl << " " << (yl + len) << " " << (zl + len) << std::endl;
        }

        myfile << "CELLS " << nodes.size() << " " << num_cells_elements
               << std::endl;
        ...
        myfile << "LOOKUP_TABLE default" << std::endl;
        for (int i = 0; i < nodes.size(); i++) myfile << mpi_rank << std::endl;
