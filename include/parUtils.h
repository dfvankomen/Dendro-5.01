...
/**
  ...
  @author Hari Sundar
  @brief Collection of Generic Parallel Functions: Sorting, Partitioning,
  Searching,...
  */
namespace par {

template <typename T>
int Mpi_Isend(T *buf, int count, int dest, int tag, MPI_Comm comm,
              MPI_Request *request);

template <typename T>
int Mpi_Issend(T *buf, int count, int dest, int tag, MPI_Comm comm,
               MPI_Request *request);

template <typename T>
int Mpi_Recv(T *buf, int count, int source, int tag, MPI_Comm comm,
             MPI_Status *status);

template <typename T>
int Mpi_Irecv(T *buf, int count, int source, int tag, MPI_Comm comm,
              MPI_Request *request);

template <typename T>
int Mpi_Gather(T *sendBuffer, T *recvBuffer, int count, int root,
               MPI_Comm comm);

template <typename T>
int Mpi_Gatherv(T *sendBuffer, int sendcnts, T *recvBuffer, int *recvcnts,
                int *rdispls, int root, MPI_Comm comm);

template <typename T, typename S>
int Mpi_Sendrecv(T *sendBuf, int sendCount, int dest, int sendTag, S *recvBuf,
                 int recvCount, int source, int recvTag, MPI_Comm comm,
                 MPI_Status *status);
...
template <typename T>
int Mpi_Reduce(T *sendbuf, T *recvbuf, int count, MPI_Op op, int root,
               MPI_Comm comm);
...
int Mpi_Alltoallv_Kway(T *sbuff_, int *s_cnt_, int *sdisp_, T *rbuff_,
                       int *r_cnt_, int *rdisp_, MPI_Comm c);
 ...
 *@param out The output vector. Memory for this vector will be allocated within
 *the function.
 ...
 * @param searchList globally sorted unique list. No processor must call this
...
/**
 *@brief A parallel weighted partitioning function. In our implementation, we do
 *not pose any restriction on the input or the number of processors. This
 *function can be used with an odd number of processors as well. Some processors
 *can pass an empty vector as input. The relative ordering of the elements is
 *preserved.
 *@author Hari Sundar
 *@author Rahul Sampath
 *@param vec the input vector
 *@param getWeight function pointer to compute the weight of each element. If
 *you pass NULL, then every element will get a weight equal to 1.
 *@param comm the communicator
 */
template <typename T>
int partitionW(std::vector<T> &vec, unsigned int (*getWeight)(const T *),
               MPI_Comm comm);
 ...
 *listA must not be empty on any of the calling processors. listB can be empty
 *on some of the calling processors. listB will be cleared within the function.
 *@param comm the communicator
 *@author Rahul Sampath*/
template <typename T>
...
/**
 *@brief A parallel sample sort implementation. In our implementation, we do not
 *pose any restriction on the input or the number of processors. This function
 *can be used with an odd number of processors as well. Some processors can pass
 *an empty vector as input. If the total number of elements in the vector
 *(globally) is fewer than 10*p^2, where p is the number of processors, then we
 *will use bitonic sort instead of sample sort to sort the vector. We use a
 *paralle bitonic sort to sort the samples in the sample sort algorithm. Hence,
 *the complexity of the algorithm is O(n/p log n/p) + O(p log p). Here, n is the
 *global length of the vector and p is the number of processors.
 *@author Hari Sundar
 *@author Rahul Sampath
 ...
 */
template <typename T>
int sampleSort(std::vector<T> &in, std::vector<T> &out,
               std::vector<double> &stats, MPI_Comm comm);

/**
 *@brief Splits a communication group into two, one containing processors that
 *passed a value of 'false' for the parameter 'iAmEmpty' and the another
 *containing processors that passed a value of 'true' for the parameter. Both
 *the groups are sorted in the ascending order of their ranks in the old comm.
 *@author Rahul Sampath
 *@param iAmEmpty     Some flag to determine which group the calling processor
...
/**
 * @brief Splits a communication group into two depending on the values in
 * isEmptyList. Both the groups are sorted in the ascending order of their ranks
 * in the old comm. All processors must call this function with the same
 * 'isEmptyList' array.
 * @author Rahul Sampath
 * @param isEmptyList  flags (of length equal to the number of processors) to
 * determine whether each processor is active or not.
 * @param orig_comm    The comm group that needs to be split.
 * @param new_comm     The new comm group.*/
int splitComm2way(const bool *isEmptyList, MPI_Comm *new_comm,
                  MPI_Comm orig_comm);
 ...
 * is sorted in the ascending order of their ranks in the old comm and the
 * second group is sorted in the descending order of their ranks in the old comm
 * @author Hari Sundar
 * @param orig_comm    The comm group that needs to be split.
 * @param new_comm     The new comm group. */
...
/**
 * @author Hari Sundar
 * @brief The main operation in the parallel bitonic sort algorithm. This
 * implements the compare-split operation.
...
/**
 *@brief An implementation of parallel bitonic sort that expects the number of
 *processors to be a power of 2. However, unlike most implementations, we do not
 *expect the length of the vector (neither locally nor globally) to be a power
 *of 2 or even. Moreover, each processor can call this with a different number
 *of elements. However, we do expect that 'in' atleast has 1 element on each
 *processor.
...
/**
 *@brief An implementation of parallel bitonic sort that does not expect the
 *number of processors to be a power of 2. In fact, the number of processors can
 *even be odd. Moreover, we do not even expect the length of the vector (neither
 *locally nor globally) to be a power of 2 or even. Moreover, each processor can
 *call this with a different number of elements. However, we do expect that 'in'
 *atleast has 1 element on each processor. This recursively calls the function
 *bitonicSort_binary, followed by a special parallel merge.
 *@param in  the vector to be sorted
 *@author Hari Sundar
