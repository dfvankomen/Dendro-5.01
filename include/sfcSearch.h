 ...
 * @breif Contains SFC based searching functionality (finalized) for Morton and
 * Hilbert Curve. this searching functionality is mainly used the mesh
 * generation stage.
 *
 * */
...
namespace SFC {
/*@ breif compare function defined based on sfc < ordering operator defined in
 * ot::TreeSort class.
 *
 * */
template <typename T>
 ...
 *  Assumptions: pNodes can be sorted or unsorted, but pNodes must be unique. If
 * some keys were not found they need to be sent to the correst processor using
 * par::treeSort. Not found keys are marked
 *
 * */
template <typename TKey, typename TOctant>
void SFC_treeSearch(TKey* pKeys, TOctant* pNodes,
                    std::vector<unsigned int>& pSearchIndex,
                    DendroIntL nKeyBegin, DendroIntL nKeyEnd,
                    DendroIntL nNodeBegin, DendroIntL nNodeEnd,
                    unsigned int pMaxDepthBit, unsigned int pMaxDepth,
                    unsigned int rot_id);

template <typename TKey, typename TOctant>
void SFC_treeSearch(TKey* pKeys, TOctant* pNodes, DendroIntL nKeyBegin,
 ...
 *  Assumptions: pNodes can be sorted or unsorted, but pNodes must be unique. If
 * some keys were not found they need to be sent to the correst processor using
 * par::treeSort. Keys which are not found will be flags in the m_uiLevel. (max
 * level is 31 and rest of the bits are used to indicate the flags for mesh
 * generation part. )
 *
 * */
template <typename TKey, typename TOctant>
void SFC_treeSearch(TKey* pKeys, TOctant* pNodes,
    ...
    SFC::seqSort::SFC_bucketing(pKeys, lev, pMaxDepth, rot_id, nKeyBegin,
                                nKeyEnd, splitterKeys);
    ...
    if ((pMaxDepthBit) && (((nNodeEnd - nNodeBegin) >= 1) &&
                           (pNodes[splitterNodes[hindex]].getLevel() > lev))) {
        /*for (unsigned int k = splitterKeys[hindex]; k < splitterKeys[hindexN];
        k++) { if ((pKeys[k].getLevel() == lev) &
        (pNodes[splitterNodes[hindex]].getLevel() == lev)) {
        ...
        for (int i = 0; i < NUM_CHILDREN; i++) {
            ...
            SFC_treeSearch(pKeys, pNodes, pSearchIndex, splitterKeys[hindex],
                           splitterKeys[hindexN], splitterNodes[hindex],
                           splitterNodes[hindexN], (pMaxDepthBit - 1),
                           pMaxDepth, index);
        }
        ...
        if (((nNodeEnd - nNodeBegin) == 1)) {
            ...
            for (unsigned int k = nKeyBegin; k < nKeyEnd; k++) {
                assert(((pNodes[nNodeBegin].isAncestor(pKeys[k])) ||
        ...
        if (((nNodeEnd - nNodeBegin) == 1)) {
            ...
            for (unsigned int k = nKeyBegin; k < nKeyEnd; k++) {
                ...
                if ((pNodes[nNodeBegin].isAncestor(pKeys[k])) ||
                    ...
                    assert(
                        pKeys[k].getFlag() &
                        OCT_FOUND);  // Note: Since we are sarching for the
                                     // ghost elements we might not find a given
                                     // key. Hence this should be disabled
                    pKeys[k].setSearchResult(nNodeBegin);
                }
            }
            // std::cout<<"key update: "<<nKeyBegin<<" to :
            // "<<nKeyEnd<<std::endl;

        } else if (((nNodeEnd - nNodeBegin) > 1)) {
            // std::cout<<"Else call: "<<pMaxDepthBit<<": NBegin:
            // "<<nNodeBegin<<" NEnd: "<<nNodeEnd<<" KBegin: "<<nKeyBegin<<"
            // KEnd: "<<nKeyEnd<<" NodeLev: "<<pNodes[nNodeBegin].getLevel()<<"
            // lev: "<<lev<< std::endl;
            unsigned int nCnt = nNodeBegin;
            for (unsigned int k = nKeyBegin; k < nKeyEnd; k++) {
                while ((nCnt < nNodeEnd) &&
                       (!(pNodes[nCnt].isAncestor(pKeys[k])) &&
                        !(pNodes[nCnt] == pKeys[k])))
                    nCnt++;
                ...
                if (nCnt < nNodeEnd) {
                    ...
                    // we are sarching for the ghost elements we might not find
                    // a given key. Hence this should be disabled
                    assert(((pNodes[nCnt].isAncestor(pKeys[k])) ||
                            (pNodes[nCnt] == pKeys[k])));
