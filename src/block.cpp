...
double ot::Block::computeGridDx() const {
    return ((m_uiBlockNode.maxX() - m_uiBlockNode.minX())) /
           ((double)((1u << (m_uiRegGridLev - m_uiBlockNode.getLevel())) *
                     m_uiEleOrder));
}

double ot::Block::computeGridDy() const {
    return ((m_uiBlockNode.maxY() - m_uiBlockNode.minY())) /
           ((double)((1u << (m_uiRegGridLev - m_uiBlockNode.getLevel())) *
                     m_uiEleOrder));
}

double ot::Block::computeGridDz() const {
    return ((m_uiBlockNode.maxZ() - m_uiBlockNode.minZ())) /
           ((double)((1u << (m_uiRegGridLev - m_uiBlockNode.getLevel())) *
                     m_uiEleOrder));
}

double ot::Block::computeDx(const Point& d_min, const Point& d_max) const {
    return ((d_max.x() - d_min.x()) / ((double)(1u << m_uiMaxDepth))) *
           (computeGridDx());
}

double ot::Block::computeDy(const Point& d_min, const Point& d_max) const {
    return ((d_max.y() - d_min.y()) / ((double)(1u << m_uiMaxDepth))) *
           (computeGridDy());
}

double ot::Block::computeDz(const Point& d_min, const Point& d_max) const {
    return ((d_max.z() - d_min.z()) / ((double)(1u << m_uiMaxDepth))) *
           (computeGridDz());
}

void ot::Block::setOffset(DendroIntL offset) { m_uiOffset = offset; }
...
void ot::Block::computeEleIJK(ot::TreeNode pNode, unsigned int* eijk) const {
    eijk[0] = (pNode.getX() - m_uiBlockNode.getX()) >>
              (m_uiMaxDepth - m_uiRegGridLev);
    eijk[1] = (pNode.getY() - m_uiBlockNode.getY()) >>
              (m_uiMaxDepth - m_uiRegGridLev);
    eijk[2] = (pNode.getZ() - m_uiBlockNode.getZ()) >>
              (m_uiMaxDepth - m_uiRegGridLev);
}
...
bool ot::Block::isBlockInternalEle(ot::TreeNode pNode) const {
    ...
    // note that integer overflow if the element is out of the min bounds. hence
    // give MAX number which is larger than m_uiBlkElem_1D;
    bool s1 =
        ((eijk[0] < (m_uiBlkElem_1D - 1)) && (eijk[1] < (m_uiBlkElem_1D - 1)) &&
         (eijk[2] < (m_uiBlkElem_1D - 1)));
