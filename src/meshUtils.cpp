...
Mesh* createMesh(const ot::TreeNode* oct, unsigned int num,
    ...
    if (!isActive)
        if (tmpNodes.size() != 0) {
            std::cout << "Error: " << __LINE__ << " rank_g: " << rank
                      << " isActive: " << isActive
...
Mesh* createWAMRMesh(std::function<void(double, double, double, double*)> func,
                     ...
                     MPI_Comm comm, unsigned int verbose, ot::SM_TYPE sm_type,
                     unsigned int* refIds, unsigned int sz,
    ...
    for (unsigned int i = 0; i < refVars.size(); i++) refVars[i] = i;

    if (refIds == NULL || sz == 0)
        function2Octree(func, numVars, refVars.data(), refVars.size(), tmpNodes,
                        m_uiMaxDepth, wtol, eleOrder, comm);
    else
        function2Octree(func, numVars, refIds, sz, tmpNodes, m_uiMaxDepth, wtol,
                        eleOrder, comm);

    return createMesh(tmpNodes.data(), tmpNodes.size(), eleOrder, comm, verbose,
                      sm_type, grain_sz, ld_tol, sf_k);
}

void meshWAMRConvergence(
    ot::Mesh*& pMesh, std::function<void(double, double, double, double*)> func,
    double wtol, unsigned int numVars, unsigned int eleOrder,
    unsigned int* refIds, unsigned int sz, unsigned int maxiter) {
    ...
    std::function<double(double, double, double)> waveletTolFunc =
        [wtol](double x, double y, double z) { return wtol; };

    do {
        DVec vecCG;
        DVec vecUz;
        vecCG.create_vector(pMesh, DVEC_TYPE::OCT_SHARED_NODES, DVEC_LOC::HOST,
                            numVars, true);
        vecCG.create_vector(pMesh, DVEC_TYPE::OCT_LOCAL_WITH_PADDING,
                            DVEC_LOC::HOST, numVars, true);
        ...
        if (pMesh->isActive()) {
            ...
            for (unsigned int elem = pMesh->getElementLocalBegin();
                 elem < pMesh->getElementLocalEnd(); elem++) {
                            ...
                            if (nodeLookUp_CG >= nodeLocalBegin &&
                                ...
                                nodeLookUp_DG =
                                    e2n_dg[elem * nPe +
                                           k * (eleOrder + 1) * (eleOrder + 1) +
                                           j * (eleOrder + 1) + i];
                                pMesh->dg2eijk(nodeLookUp_DG, ownerID, ii_x,
                                               jj_y, kk_z);
                                len = (double)(1u
                                               << (m_uiMaxDepth -
        ...
        isRefine1 = wavelet::compute_wavelet_remesh_flags(
            pMesh, refine_flags, (const DendroScalar**)unzipIn, rid, nr,
            waveletTolFunc, 0.1, true);
        ...
        if (isRefine1) {
            ...
            if (!rank)
                std::cout << "[WAMR Grid Convergence]  old mesh size: "
                          << gSz_old << " new mesh size: " << gSz_new
                          << std::endl;
...
void computeBlockUnzipGhostNodes(const ot::Mesh* pMesh, unsigned int blk,
    ...
    if (pMesh->isActive()) {
        ...
        for (unsigned int elem = blkList[blk].getLocalElementBegin();
            ...
            if (((ei == emin) || (ej == emin) || (ek == emin) || (ei == emax) ||
                 (ej == emax) || (ek == emax))) {
                ...
                if (ei == emin) {
                    ...
                    if (lookup != LOOK_UP_TABLE_DEFAULT) {
                        if (pNodes[lookup].getLevel() > regLevel) {
                            ...
                            child[5] =
                                e2e[child[1] * NUM_FACES + OCT_DIR_FRONT];
                            assert(child[5] != LOOK_UP_TABLE_DEFAULT);
                            child[7] =
                                e2e[child[3] * NUM_FACES + OCT_DIR_FRONT];
                            ...
                            for (unsigned int c = 0; c < NUM_CHILDREN; c++) {
                                if (child[c] != LOOK_UP_TABLE_DEFAULT) {
                                    for (unsigned int node = 0; node < nPe;
                                         node++) {
                                        node_cg = e2n_cg[child[c] * nPe + node];
                                        if (!(node_cg >= nodeLocalBegin &&
                                              node_cg < nodeLocalEnd))
                                            gid.push_back(node_cg);
                                    }
                                }
                            }
                ...
                if (ei == emax) {  // OCT_DIR_RIGHT
                    ...
                    if (lookup != LOOK_UP_TABLE_DEFAULT) {
                        if (pNodes[lookup].getLevel() >
                            regLevel) {  // neighbour octant is smaller.
                            ...
                            child[4] =
                                e2e[child[0] * NUM_FACES + OCT_DIR_FRONT];
                            assert(child[4] != LOOK_UP_TABLE_DEFAULT);
                            child[6] =
                                e2e[child[2] * NUM_FACES + OCT_DIR_FRONT];
                ...
                if (ej == emin) {  // OCT_DIR_DOWN
                    ...
                    if (lookup != LOOK_UP_TABLE_DEFAULT) {
                        if (pNodes[lookup].getLevel() >
                            ...
                            child[2] = lookup;
                            child[3] =
                                e2e[child[2] * NUM_FACES + OCT_DIR_RIGHT];
                            assert(child[3] != LOOK_UP_TABLE_DEFAULT);
                            child[6] =
                                e2e[child[2] * NUM_FACES + OCT_DIR_FRONT];
                            assert(child[6] != LOOK_UP_TABLE_DEFAULT);
                        ...
                        } else {  // neighbour octant is same lev or coarser
                ...
                if (ej == emax) {  // OCT_DIR_UP
                    ...
                    if (lookup != LOOK_UP_TABLE_DEFAULT) {
                        if (pNodes[lookup].getLevel() >
                            ...
                            child[0] = lookup;
                            child[1] =
                                e2e[child[0] * NUM_FACES + OCT_DIR_RIGHT];
                ...
                if (ek == emin) {  // OCT_DIR_BACK
                    ...
                    if (lookup != LOOK_UP_TABLE_DEFAULT) {
                        if (pNodes[lookup].getLevel() >
                            ...
                            child[4] = lookup;
                            child[5] =
                                e2e[child[4] * NUM_FACES + OCT_DIR_RIGHT];
                            assert(child[5] != LOOK_UP_TABLE_DEFAULT);
...
unsigned int computeTLevel(const ot::Mesh* const pMesh, unsigned int blk) {
    ...
    for (unsigned int i = 0; i < blkEdgeMap.size(); i++) {
        if (blkEdgeMap[i] != LOOK_UP_TABLE_DEFAULT &&
            (b_lev < pNodes[blkEdgeMap[i]].getLevel())) {
            b_lev = pNodes[blkEdgeMap[i]].getLevel();
            return b_lev;  // return since 2:1 balance octree there for the
                           // smallest neighbour octant.
        }
    }

    for (unsigned int i = 0; i < blkVertMap.size(); i++) {
        if (blkVertMap[i] != LOOK_UP_TABLE_DEFAULT &&
    ...
    for (unsigned int ele = block.getLocalElementBegin();
         ele < block.getLocalElementEnd(); ele++) {
        if (block.isBlockInternalEle(pNodes[ele])) continue;
...
int slice_mesh(const ot::Mesh* pMesh, unsigned int s_val[3],
    ...
    for (unsigned int e = pMesh->getElementLocalBegin();
         e < pMesh->getElementLocalEnd(); e++) {
        pvec[0]         = pNodes[e].minX() - s_val[0];
        pvec[1]         = pNodes[e].minY() - s_val[1];
        pvec[2]         = pNodes[e].minZ() - s_val[2];

        const int dot_p = pvec[0] * s_normal[0] + pvec[1] * s_normal[1] +
                          pvec[2] * s_normal[2];
        if (dot_p == 0) sids.push_back(e);
    }
...
Mesh* createSplitMesh(unsigned int eleOrder, unsigned int lmin,
    ...
    for (unsigned int l = lmin; l < lmax; l++) {
        ...
        std::cout << "tmpOct: at lev " << l << " size : " << tmpOct.size()
                  << std::endl;
    }
