    ...
    for (unsigned int i = 0; i < in.size(); i++) {
        ...
        if (in[i].isBoundaryOctant(ot::TreeNode::POSITIVE, &bdyFlags)) {
            ...
            if (bdyFlags & ot::TreeNode::X_POS_BDY) {
                bdy.push_back(ot::TreeNode((1u << maxDepth), y, z,
                                           (d + 1) | ot::TreeNode::BOUNDARY,
                                           dim, maxDepth + 1));
            }
            ...
            if ((bdyFlags &
                ...
                (ot::TreeNode::X_POS_BDY + ot::TreeNode::Y_POS_BDY)) {
                bdy.push_back(ot::TreeNode((1u << maxDepth), (1u << maxDepth),
                                           z, (d + 1) | ot::TreeNode::BOUNDARY,
                                           dim, maxDepth + 1));
            }
            ...
            if ((bdyFlags &
                ...
                (ot::TreeNode::X_POS_BDY + ot::TreeNode::Z_POS_BDY)) {
                bdy.push_back(ot::TreeNode(
                    (1u << maxDepth), y, (1u << maxDepth),
            ...
            if ((bdyFlags & (ot::TreeNode::X_POS_BDY + ot::TreeNode::Y_POS_BDY +
                             ot::TreeNode::Z_POS_BDY)) ==
                (ot::TreeNode::X_POS_BDY + ot::TreeNode::Y_POS_BDY +
                 ot::TreeNode::Z_POS_BDY)) {
                bdy.push_back(ot::TreeNode(
                    (1u << maxDepth), (1u << maxDepth), (1u << maxDepth),
                    (d + 1) | ot::TreeNode::BOUNDARY, dim, maxDepth + 1));
            }
        }  // end if boundary
...
int function2Octree(std::function<double(double, double, double)> fx,
                    ...
                    unsigned int max_ref_level, const double& tol,
    ...
    if (!rank) {
        ...
        while ((num_intersected > 0) && (num_intersected < size /**size*/) &&
               (depth < max_ref_level)) {
            std::cout << "Depth: " << depth << " n = " << nodes.size()
                      << std::endl;
            num_intersected = 0;
                ...
                for (int k = 0; k < (elementOrder + 1); k++)
                    for (int j = 0; j < (elementOrder + 1); j++)
                        for (int i = 0; i < (elementOrder + 1); i++)
                            dist_parent[k * (elementOrder + 1) *
                                            (elementOrder + 1) +
                                        j * (elementOrder + 1) + i] =
                                fx(pt.x() + i * h1, pt.y() + j * h1,
                                   pt.z() + k * h1);

                for (unsigned int cnum = 0; cnum < NUM_CHILDREN; cnum++) {
                    refEl.I3D_Parent2Child(dist_parent, dist_child_ip, cnum);
                    pt_child = Point(
                        (pt.x() + (((int)((bool)(cnum & 1u)))
                                   << (m_uiMaxDepth - elem.getLevel() - 1))),
                        (pt.y() + (((int)((bool)(cnum & 2u)))
                                   << (m_uiMaxDepth - elem.getLevel() - 1))),
                        (pt.z() + (((int)((bool)(cnum & 4u)))
                                   << (m_uiMaxDepth - elem.getLevel() - 1))));
                    // std::cout<<"parent: "<<elem<<" child "<<cnum<<" value:
                    // "<<pt_child.x()<<" , "<<pt_child.y()<<" ,
                    // "<<pt_child.z()<<std::endl;
                    h2 = h1 / 2.0;
                    for (int k = 0; k < (elementOrder + 1); k++)
                        for (int j = 0; j < (elementOrder + 1); j++)
                            for (int i = 0; i < (elementOrder + 1); i++)
                                dist_child[k * (elementOrder + 1) *
                                               (elementOrder + 1) +
                                           j * (elementOrder + 1) + i] =
                                    fx(pt_child.x() + i * h2,
                                       pt_child.y() + j * h2,
                    ...
                    l2_norm = normLInfty(
                        dist_child, dist_child_ip,
                        nodesPerElement);  // normL2(dist_child,dist_child_ip,nodesPerElement)/normL2(dist_child_ip,nodesPerElement);
    ...
    while ((num_intersected > 0) && (depth < max_ref_level)) {
                    ...
                    for (int j = 0; j < (elementOrder + 1); j++)
                        for (int i = 0; i < (elementOrder + 1); i++)
                            dist_child[k * (elementOrder + 1) *
                                           (elementOrder + 1) +
                                       j * (elementOrder + 1) + i] =
                                fx(pt_child.x() + i * h2, pt_child.y() + j * h2,
                                   pt_child.z() + k * h2);
        ...
        SFC::parSort::SFC_treeSort(nodes, nodes_new, nodes_new, nodes_new, 0.1,
                                   m_uiMaxDepth, root, ROOT_ROTATION, 1,
                                   TS_REMOVE_DUPLICATES, 2, comm);
        std::swap(nodes, nodes_new);
        nodes_new.clear();

        par::Mpi_Allreduce(&num_intersected, &num_intersected_g, 1, MPI_MAX,
                           comm);
                ...
                for (int k = 0; k < (elementOrder + 1); k++)
                    for (int j = 0; j < (elementOrder + 1); j++)
                        for (int i = 0; i < (elementOrder + 1); i++) {
                            fx(pt.x() + i * h1, pt.y() + j * h1,
                               pt.z() + k * h1, varVal);
                            for (unsigned int var = 0; var < numInterpVars;
                                 var++)
                                dist_parent[varIndex[var] * nodesPerElement +
                                            k * (elementOrder + 1) *
                                                (elementOrder + 1) +
                                            j * (elementOrder + 1) + i] =
                                    varVal[varIndex[var]];
                ...
                for (unsigned int cnum = 0; cnum < NUM_CHILDREN; cnum++) {
                    ...
                    for (int k = 0; k < (elementOrder + 1); k++)
                        for (int j = 0; j < (elementOrder + 1); j++)
                            for (int i = 0; i < (elementOrder + 1); i++) {
                                fx(pt_child.x() + i * h2, pt_child.y() + j * h2,
                                   pt_child.z() + k * h2, varVal);
                                for (unsigned int var = 0; var < numInterpVars;
                                     var++)
                                    dist_child[varIndex[var] * nodesPerElement +
                                               k * (elementOrder + 1) *
                                                   (elementOrder + 1) +
                                               j * (elementOrder + 1) + i] =
                                        varVal[varIndex[var]];
                    ...
                    for (unsigned int var = 0; var < numInterpVars; var++) {
                        refEl.I3D_Parent2Child(
                            dist_parent + varIndex[var] * nodesPerElement,
                            dist_child_ip + varIndex[var] * nodesPerElement,
                            cnum);
                        l2_norm = normLInfty(
                            dist_child + varIndex[var] * nodesPerElement,
                            dist_child_ip + varIndex[var] * nodesPerElement,
    ...
    while ((num_intersected > 0) && (depth < max_ref_level)) {
        ...
        for (auto elem : nodes) {
            ...
            for (int k = 0; k < (elementOrder + 1); k++)
                for (int j = 0; j < (elementOrder + 1); j++)
                    for (int i = 0; i < (elementOrder + 1); i++) {
                        fx(pt.x() + i * h1, pt.y() + j * h1, pt.z() + k * h1,
                           varVal);
                        for (unsigned int var = 0; var < numInterpVars; var++)
                            dist_parent[varIndex[var] * nodesPerElement +
                                        k * (elementOrder + 1) *
            ...
            for (unsigned int cnum = 0; cnum < NUM_CHILDREN; cnum++) {
                ...
                for (unsigned int var = 0; var < numInterpVars; var++) {
                    refEl.I3D_Parent2Child(
                        dist_parent + varIndex[var] * nodesPerElement,
                        dist_child_ip + varIndex[var] * nodesPerElement, cnum);
                    l2_norm = normLInfty(
                        dist_child + varIndex[var] * nodesPerElement,
                        dist_child_ip + varIndex[var] * nodesPerElement,
                        nodesPerElement);
                    // std::cout<<"rank: "<<rank<<" node: "<<elem<<" l2 norm :
                    // "<<l2_norm<<" var: "<<varIndex[var]<<std::endl;
                    if (l2_norm > tol) {
                        splitOctant = true;
                        break;
                    }
...
void octree2BlockDecomposition(std::vector<ot::TreeNode>& pNodes,
                               ...
                               unsigned int coarsetLev, unsigned int* tag,
                               unsigned int tsz) {
...
// Note: Commented out code is for debugging purposes.
    ...
    ot::Block rootBlock(rootNode, ROOT_ROTATION, d_min, localBegin, localEnd,
                        eleOrder);
    ...
    while (!initialBLocks.empty()) {
        ...
        if (tag != NULL) {
            ...
            for (unsigned int elem = nBegin; elem < nEnd; elem++) {
                if (pNodes[elem].getLevel() == currRegGridLev)
                    numRegGridOcts++;
                else if (abs((int)pNodes[elem].getLevel() -
                             (int)currRegGridLev) > OCT2BLK_DECOMP_LEV_GAP) {
                    octLevelGap = false;
                    break;
                }
        ...
        // if(rank==debug_rank) std::cout<<"current parent: "<<parent<<" rot id:
        // "<<(int)rot_id<<" curRegGridLev: "<<currRegGridLev<<" begin:
        // "<<nBegin<<" end: "<<nEnd<<" numRegGridOcts:
        // "<<numRegGridOcts<<std::endl;
        ...
        if ((parent.getLevel() >= coarsetLev) && (isTagValid) &&
        ...
        } else {  // implies that we need to split the tmpBlock.
            assert(parent.getLevel() < maxDepth);
            pMaxDepthBit = maxDepth - parent.getLevel() - 1;
            SFC::seqSort::SFC_bucketing(&(*(pNodes.begin())), parent.getLevel(),
                                        maxDepth, rot_id, nBegin, nEnd,
                                        splitters);
            ...
            for (int i = 0; i < NUM_CHILDREN; i++) {
                ...
                if (i == (NUM_CHILDREN - 1))
                    ...
                    hindexN =
                        (rotations[2 * NUM_CHILDREN * rot_id + i + 1] - '0');
                assert(splitters[hindex] <= splitters[hindexN]);

                for (unsigned int elem = splitters[hindex];
                     elem < splitters[hindexN]; elem++) {
                    if (pNodes[elem].getLevel() == currRegGridLev) {
                        childHasRegLev[i] = 1;
                        break;
                    }
                ...
                if (childHasRegLev[i] == 1) {
                    if ((parent.getLevel() + 1) <= currRegGridLev) {
                        tmpBlock = ot::Block(
                            ot::TreeNode(x, y, z, parent.getLevel() + 1,
                                         m_uiDim, maxDepth),
                            index, currRegGridLev, splitters[hindex],
                            splitters[hindexN], eleOrder);
                    ...
                    if (currRegGridLev <= d_max &&
                        ...
                        tmpBlock = ot::Block(
                            ot::TreeNode(x, y, z, parent.getLevel() + 1,
                                         m_uiDim, maxDepth),
                            index, (currRegGridLev + 1), splitters[hindex],
                            splitters[hindexN], eleOrder);
                        // if(rank==debug_rank) std::cout<<"block node pushed
                        // (initialBlocks):
                        // "<<tmpBlock.getBlockNode()<<std::endl;
                        initialBLocks.push_back(tmpBlock);
                    }
                }
            }
    ...
    for (unsigned int i = 0; i < blockList.size(); i++) {
        ...
        if (numActualRegOcts == 1) singleOctBlockCount++;
...
bool isBlockConnected(const std::vector<ot::TreeNode>& nodes,
                      const std::vector<DendroIntL>& blockNodeIndices,
                      const std::vector<unsigned int> e2e_map) {
    if (blockNodeIndices.empty()) {
        return false;
    }

    // create an unordered set of the indices really quick
    std::unordered_set<DendroIntL> blocksFlat(blockNodeIndices.begin(),
                                              blockNodeIndices.end());

    DendroIntL rootNodeIdx = blockNodeIndices[0];
    std::unordered_set<DendroIntL> visited;
    std::queue<DendroIntL> queue;
    queue.push(rootNodeIdx);
    visited.insert(rootNodeIdx);

    while (!queue.empty()) {
        DendroIntL current = queue.front();
        queue.pop();

        for (int face = 0; face < 6; face++) {
            DendroIntL neighbor = e2e_map[current * 6 + face];
            // if there's no connection or we've already visited that face,
            // continue
            if (neighbor == LOOK_UP_TABLE_DEFAULT ||
                visited.count(neighbor) > 0)
                continue;

            // check if the neighbor is part of the current block
            if (blocksFlat.count(neighbor) > 0) {
    ...
    return visited.size() == blockNodeIndices.size();
}

inline unsigned int computeChildRotationID(unsigned int parentRotID,
                                           unsigned int childMortonIndex) {
    assert(parentRotID < 24 && childMortonIndex < 8);
    return HILBERT_TABLE[parentRotID * 8 + childMortonIndex];
}

std::string toHexString(DendroUInt_128 value) {
    const uint64_t lower = static_cast<uint64_t>(value);
    const uint64_t upper = static_cast<uint64_t>(value >> 64);
    char buf[34];
    snprintf(buf, sizeof(buf), "0x%016lx%016lx", upper, lower);
    ...
    while (!initialBlocks.empty()) {
        ...
        for (auto idx : blockElementIndices) {
            ...
            if (nodeLevel == currRegGridLev)
                numRegGridOcts++;
            else if (abs((int)nodeLevel - (int)currRegGridLev) >
        ...
        if (parent.getLevel() < coarsetLev)
            std::cout << rank << ": Rejected: Parent level "
                      << parent.getLevel() << " < coarsetLev " << coarsetLev
                      << std::endl;
        if (!isTagValid)
            std::cout << rank << ": Rejected: Tags are inconsistent"
                      << std::endl;
        if (!octLevelGap)
            std::cout << rank << ": Rejected: Level gap too large" << std::endl;
        if (blockFillRatio < OCT2BLK_DECOMP_BLK_FILL_RATIO)
            std::cout << rank << ": Rejected: Fill ratio " << blockFillRatio
                      << " < " << OCT2BLK_DECOMP_BLK_FILL_RATIO << std::endl;
        if (octVolume != blockVolume)
            std::cout << rank << ": Rejected: Volume mismatch (oct="
...
void blockListToVtk(std::vector<ot::Block>& blkList,
    ...
    for (unsigned int k = 0; k < blkList.size(); k++) {
        ...
        for (unsigned int j = blkList[k].getLocalElementBegin();
             j < blkList[k].getLocalElementEnd(); j++)
...
void enforceSiblingsAreNotPartitioned(std::vector<ot::TreeNode>& in,
    ...
    if (!in.empty()) {
        ...
        par::Mpi_Sendrecv(&maxNode, sendCount, next, 0, &prev_max, recvCount,
                          prev, 0, comm, &status);
        ...
        for (unsigned int elem = 0; elem < NUM_CHILDREN; elem++) {
            if ((elem < in.size()) &&
                (in.front().getParent() == in[elem].getParent()))
                sendCount++;
            else
                break;
        }

        if ((rank > 0) && (sendCount == 1) &&
            (in.front().getParent() != prev_max.getParent()))
            sendCount = 0;
        if (sendCount == NUM_CHILDREN || rank == 0) sendCount = 0;

        // std::cout<<"m_uiActiveRank: "<<m_uiActiveRank<<" sendCount:
        // "<<sendCount<<std::endl;
        par::Mpi_Sendrecv(&sendCount, 1, prev, 1, &recvCount, 1, next, 1, comm,
                          &status);
        /*
        MPI_Isend(&sendCount,1,MPI_INT,prev,0,m_uiCommActive,&req);
        MPI_Recv(&recvCount,1,MPI_INT,next,0,m_uiCommActive,&status);*/

        // std::cout<<"m_uiActiveRank: "<<m_uiActiveRank<<" recvCount:
        // "<<recvCount<<std::endl;
...
void generateBlkEdgeSKeys(const ot::Block& blk,
    ...
    if (myX > 0 && myY > 0) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX - 1), (myY - 1), (myZ + k * hsz),
    ...
    if (myX > 0 && (myY + mySz) < domain_max) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX - 1), (myY + mySz), (myZ + k * hsz),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
    ...
    if (myX > 0 && myZ > 0) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX - 1), (myY + k * hsz), (myZ - 1),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
    ...
    if (myX > 0 && (myZ + mySz) < domain_max) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX - 1), (myY + k * hsz), (myZ + mySz),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
    ...
    if ((myX + mySz) < domain_max && myY > 0) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX + mySz), (myY - 1), (myZ + k * hsz),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
    ...
    if ((myX + mySz) < domain_max && (myY + mySz) < domain_max) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX + mySz), (myY + mySz), (myZ + k * hsz),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
    ...
    if ((myX + mySz) < domain_max && myZ > 0) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX + mySz), (myY + k * hsz), (myZ - 1),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
    ...
    if ((myX + mySz) < domain_max && (myZ + mySz) < domain_max) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX + mySz), (myY + k * hsz), (myZ + mySz),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
    ...
    if (myY > 0 && myZ > 0) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX + k * hsz), (myY - 1), (myZ - 1),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
    ...
    if (myY > 0 && (myZ + mySz) < domain_max) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX + k * hsz), (myY - 1), (myZ + mySz),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
    ...
    if ((myY + mySz) < domain_max && myZ > 0) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX + k * hsz), (myY + mySz), (myZ - 1),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
    ...
    if ((myY + mySz) < domain_max && (myZ + mySz) < domain_max) {
        for (unsigned int k = 0; k < blkElem_1D; k++) {
            hint = sKeys.emplace(
                sKeys.end(),
                ot::SearchKey((myX + k * hsz), (myY + mySz), (myZ + mySz),
                              m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
            hint->addOwner(k);
...
void generateBlkVertexSKeys(const ot::Block& blk,
    ...
    if ((myX > 0) && (myY > 0) && (myZ > 0)) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX - 1), (myY - 1), (myZ - 1),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(0);
    ...
    if (((myX + mySz) < domain_max) && (myY > 0) && (myZ > 0)) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX + mySz), (myY - 1), (myZ - 1),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(1);
    ...
    if ((myX > 0) && ((myY + mySz) < domain_max) && (myZ > 0)) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX - 1), (myY + mySz), (myZ - 1),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(2);
    ...
    if (((myX + mySz) < domain_max) && ((myY + mySz) < domain_max) &&
        (myZ > 0)) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX + mySz), (myY + mySz), (myZ - 1),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(3);
    ...
    if ((myX > 0) && (myY > 0) && ((myZ + mySz) < domain_max)) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX - 1), (myY - 1), (myZ + mySz),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(4);
    ...
    if (((myX + mySz) < domain_max) && (myY > 0) &&
        ((myZ + mySz) < domain_max)) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX + mySz), (myY - 1), (myZ + mySz),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(5);
    ...
    if ((myX > 0) && ((myY + mySz) < domain_max) &&
        ((myZ + mySz) < domain_max)) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX - 1), (myY + mySz), (myZ + mySz),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(6);
    ...
    if (((myX + mySz) < domain_max) && ((myY + mySz) < domain_max) &&
        ((myZ + mySz) < domain_max)) {
        hint = sKeys.emplace(
            sKeys.end(), ot::SearchKey((myX + mySz), (myY + mySz), (myZ + mySz),
                                       m_uiMaxDepth, m_uiDim, m_uiMaxDepth));
        hint->addOwner(7);
...
unsigned int rankSelectRule(unsigned int size_global, unsigned int rank_global,
    ...
    if (size_local > size_global) {
        std::cout << "[Error] : " << __func__ << " rank: " << rank_global
                  << " size_local > size_global " << std::endl;
        exit(0);
    }

// Rule 1 [enable this code to choose consecative ranks (deafult) works with any
// size_global and size_local]
