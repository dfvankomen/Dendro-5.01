    ...
    for (std::size_t k = begin[2]; k < end[2]; ++k)
        for (std::size_t j = begin[1]; j < end[1]; ++j)
            for (std::size_t i = begin[0]; i < end[0]; ++i) {
                ...
                double term             = 0;
                if (i >= valid_begin[0] && i < valid_end[0])
                    term = compact_ko_term(u, du_x, index, 1, hx, sigma);
                if (j >= valid_begin[1] && j < valid_end[1])
                    term += compact_ko_term(u, du_y, index, nx, hy, sigma);
                if (k >= valid_begin[2] && k < valid_end[2])
                    term += compact_ko_term(u, du_z, index, plane, hz, sigma);
                rhs[index] += term;
            }
}
}  // namespace dendroderivs
