 ...
 * @brief minimal mesh class, for CUDA only, specialization of the mesh_gpu.h
 * class.
 * @version 0.1
 * @date 2022-01-21
...
DEVICE_FUNC
inline void dg2eijk(DEVICE_UINT p, DEVICE_UINT dg_index, DEVICE_UINT& e,
                    DEVICE_UINT& i, DEVICE_UINT& j, DEVICE_UINT& k) {
    ...
    if (dg_index > e * nPe) k = (dg_index - e * nPe) / ((p + 1) * (p + 1));
...
DEVICE_FUNC inline DEVICE_BOOL is_node_hanging(const MeshGPU* const dptr_mesh,
                                               DEVICE_UINT eleID,
                                               DEVICE_UINT ix, DEVICE_UINT jy,
                                               DEVICE_UINT kz) {
    const DEVICE_UINT nPe = (p + 1) * (p + 1) * (p + 1);
    const DEVICE_UINT nx  = p + 1;
    const DEVICE_UINT owner_id =
        (dptr_mesh->m_e2n_dg[eleID * (nPe) + IDX_DG(ix, jy, kz)] / nPe);
    return dptr_mesh->m_all_elements[owner_id].m_level <
           dptr_mesh->m_all_elements[eleID].m_level;
}

template <DEVICE_UINT p>
DEVICE_FUNC DEVICE_BOOL is_edge_hanging(DEVICE_UINT sx, DEVICE_UINT sy,
                                        DEVICE_UINT sz,
                                        const MeshGPU* const dptr_mesh,
                                        DEVICE_UINT elementId,
                                        DEVICE_UINT& cnum) {
    assert(p > 1);
    const DEVICE_UINT nPe     = (p + 1) * (p + 1) * (p + 1);
    ...
    switch (edge_id) {
        case OCT_DIR_LEFT_DOWN:
            isVertexHanging[0] =
                is_node_hanging<p>(dptr_mesh, elementId, 0, 0, 0);
            isVertexHanging[1] =
                is_node_hanging<p>(dptr_mesh, elementId, 0, 0, p);

            isHanging = (isVertexHanging[0] && isVertexHanging[1]);
            if (!isHanging) return false;

            nodeLookUp_DG =
                dptr_mesh->m_e2n_dg[elementId * m_uiNpE +
                                    ((p >> 1u)) * (p + 1) * (p + 1) +
                                    (0) * (p + 1) + (0)];

            if (dptr_mesh->m_all_elements[nodeLookUp_DG / m_uiNpE].m_z ==
                dptr_mesh->m_all_elements[elementId].m_z)
                cnum = 0;
            else
                cnum = 1;
        ...
        case OCT_DIR_LEFT_UP:
            isVertexHanging[0] =
                is_node_hanging<p>(dptr_mesh, elementId, 0, p, 0);
            isVertexHanging[1] =
                is_node_hanging<p>(dptr_mesh, elementId, 0, p, p);
        ...
        case OCT_DIR_LEFT_BACK:
            ...
            nodeLookUp_DG =
                dptr_mesh
                    ->m_e2n_dg[elementId * m_uiNpE + (0) * (p + 1) * (p + 1) +
                               ((p >> 1u)) * (p + 1) + (0)];
            if (dptr_mesh->m_all_elements[nodeLookUp_DG / m_uiNpE].m_y ==
        ...
        case OCT_DIR_LEFT_FRONT:
            ...
            nodeLookUp_DG =
                dptr_mesh
                    ->m_e2n_dg[elementId * m_uiNpE + (p) * (p + 1) * (p + 1) +
                               ((p >> 1u)) * (p + 1) + (0)];
            if (dptr_mesh->m_all_elements[nodeLookUp_DG / m_uiNpE].m_y ==
                dptr_mesh->m_all_elements[elementId].m_y)
                cnum = 0;
        ...
        case OCT_DIR_RIGHT_DOWN:
            isVertexHanging[0] =
                is_node_hanging<p>(dptr_mesh, elementId, p, 0, 0);
            isVertexHanging[1] =
                is_node_hanging<p>(dptr_mesh, elementId, p, 0, p);
        ...
        case OCT_DIR_RIGHT_UP:
            isVertexHanging[0] =
                is_node_hanging<p>(dptr_mesh, elementId, p, p, 0);
            isVertexHanging[1] =
                is_node_hanging<p>(dptr_mesh, elementId, p, p, p);
        ...
        case OCT_DIR_DOWN_BACK:
            ...
            nodeLookUp_DG =
                dptr_mesh
                    ->m_e2n_dg[elementId * m_uiNpE + (0) * (p + 1) * (p + 1) +
                               (0) * (p + 1) + (p >> 1u)];
            if (dptr_mesh->m_all_elements[nodeLookUp_DG / m_uiNpE].m_x ==
...
template <DEVICE_UINT p>
DEVICE_FUNC DEVICE_BOOL is_face_hanging(DEVICE_UINT sx, DEVICE_UINT sy,
                                        DEVICE_UINT sz,
                                        const MeshGPU* const dptr_mesh,
                                        DEVICE_UINT elementId,
                                        DEVICE_UINT& cnum) {
    ...
    const DEVICE_UINT lookup =
        dptr_mesh->m_e2e[elementId * NUM_FACES + face_id];
    if (lookup == LOOK_UP_TABLE_DEFAULT)
        isHanging = false;
    else
        isHanging = dptr_mesh->m_all_elements[lookup].m_level <
                    dptr_mesh->m_all_elements[elementId].m_level;
...
DEVICE_FUNC
inline DEVICE_BOOL is_hinfo_edge_hanging(DEVICE_UINT sx, DEVICE_UINT sy,
                                         DEVICE_UINT sz,
                                         const HangingInfo& hinfo_ele,
                                         DEVICE_UINT& cnum) {
    DEVICE_UINT edge_id;
...
DEVICE_FUNC
inline DEVICE_BOOL is_hinfo_face_hanging(DEVICE_UINT sx, DEVICE_UINT sy,
                                         DEVICE_UINT sz,
                                         const HangingInfo& hinfo_ele,
                                         DEVICE_UINT& cnum) {
    DEVICE_UINT face_id;
...
GLOBAL_FUNC __launch_bounds__(512) void __unzip_dg__3(
    const MeshGPU* const __restrict__ dptr_mesh,
    const T* const __restrict__ dptr_in, T* const __restrict__ dptr_out) {
    ...
    if (blk_id < dptr_mesh->m_e2b_unzip_map_count[ele]) {
        ...
        const DEVICE_UINT blk =
            dptr_mesh->m_e2b_unzip_map[dptr_mesh->m_e2b_unzip_map_offset[ele] +
                                       blk_id];
        ...
        const DEVICE_UINT patch_level =
            dptr_mesh->m_blk_list[blk].m_patch_level;
        const DEVICE_UINT bLev =
            dptr_mesh->m_all_elements[dptr_mesh->m_blk_list[blk].m_elem_begin]
                .m_level;  // or blkNode.m_level + (patch_level
                           // -blkNode.m_level);

        const DEVICE_UINT max_depth = dptr_mesh->m_max_depth;
        const TNode3d elem_node     = dptr_mesh->m_all_elements[ele];
        if (ele >= dptr_mesh->m_blk_list[blk].m_elem_begin &&
            ele < dptr_mesh->m_blk_list[blk].m_elem_end) {
            if (!(tx < nx && ty < nx && tz < nx)) return;

            const DEVICE_UINT ei =
                (elem_node.m_x - blkNode.m_x) >> (max_depth - patch_level);
            const DEVICE_UINT ej =
                (elem_node.m_y - blkNode.m_y) >> (max_depth - patch_level);
            const DEVICE_UINT ek =
                (elem_node.m_z - blkNode.m_z) >> (max_depth - patch_level);

            dptr_out[var * unzip_sz + offset + (ek * p + tz + PW) * (ly * lx) +
                     (ej * p + ty + PW) * (lx) + (ei * p + tx + PW)] =
                dptr_in[var * dg_sz + ele * nPe + IDX_DG(tx, ty, tz)];

            return;
        }

        const DEVICE_FLOAT32 hx =
            (1u << (max_depth - bLev)) / (DEVICE_FLOAT32)p;
        ...
        const DEVICE_FLOAT32 xmax =
            blkNode.m_x + (1u << (max_depth - blkNode.m_level)) + PW * hx;
        const DEVICE_FLOAT32 ymax =
            blkNode.m_y + (1u << (max_depth - blkNode.m_level)) + PW * hx;
        const DEVICE_FLOAT32 zmax =
            blkNode.m_z + (1u << (max_depth - blkNode.m_level)) + PW * hx;
        ...
        if (elem_node.m_level == bLev) {
            ...
            const DEVICE_FLOAT32 hh =
                (1u << (max_depth - elem_node.m_level)) / (DEVICE_FLOAT32)p;
            ...
            if ((kkz >= 0 && kkz < lz) && (jjy >= 0 && jjy < ly) &&
                (iix >= 0 && iix < lx))
                dptr_out[var * unzip_sz + offset + kkz * lx * ly + jjy * lx +
                         iix] =
                    dptr_in[var * dg_sz + ele * nPe + IDX_DG(tx, ty, tz)];
            ...
            if (tx % 2 == cb && ty % 2 == cb && tz % 2 == cb) {
                ...
                if ((kkz >= 0 && kkz < lz) && (jjy >= 0 && jjy < ly) &&
                    (iix >= 0 && iix < lx))
                    dptr_out[var * unzip_sz + offset + kkz * lx * ly +
                             jjy * lx + iix] =
                        dptr_in[var * dg_sz + ele * nPe + IDX_DG(tx, ty, tz)];
            }
            ...
            TNode3d child_node;
            const DEVICE_UINT child_sz =
                (1u << (max_depth - elem_node.m_level - 1));

            if (tx < nx && ty < nx && tz < nx) {
                dgVec[tz][ty][tx] =
                    dptr_in[var * dg_sz + ele * nPe + IDX_DG(tx, ty, tz)];
                refEL[0][ty][tx] =
                    refel_1d[0 * (p + 1) * (p + 1) + ty * (p + 1) + tx];
                refEL[1][ty][tx] =
                    refel_1d[1 * (p + 1) * (p + 1) + ty * (p + 1) + tx];
            }

            GPUDevice::sync_threads();

            for (DEVICE_UINT child = 0; child < NUM_CHILDREN; child++) {
                const DEVICE_UINT cbit[3] = {(child & 1u), ((child & 2u) >> 1u),
                                             ((child & 4u) >> 2u)};
                child_node.m_x =
                    elem_node.m_x +
                    cbit[0] * (1u << (max_depth - elem_node.m_level - 1));
                child_node.m_y =
                    elem_node.m_y +
                    cbit[1] * (1u << (max_depth - elem_node.m_level - 1));
                child_node.m_z =
                    elem_node.m_z +
                    cbit[2] * (1u << (max_depth - elem_node.m_level - 1));
                child_node.m_level = elem_node.m_level + 1;

                if ((child_node.m_x + child_sz < xmin ||
                     child_node.m_x >= xmax) ||
                    (child_node.m_y + child_sz < ymin ||
                     child_node.m_y >= ymax) ||
                    (child_node.m_z + child_sz < zmin ||
                     child_node.m_z >= zmax))
                    continue;

                const DEVICE_FLOAT32 hh =
                    (1u << (max_depth - child_node.m_level)) /
                    (DEVICE_FLOAT32)p;
                const DEVICE_FLOAT32 invhh = 1.0 / hh;
                ...
                if ((kkz >= 0 && kkz < lz) && (jjy >= 0 && jjy < ly) &&
                    ...
                    //             interp_out += refEL[cbit[0]][tx][mm_x] *
                    //             refEL[cbit[1]][ty][mm_y] *
                    //             refEL[cbit[2]][tz][mm_z] *
                    //             dgVec[mm_z][mm_y][mm_x];
                    // printf("interp out : %.8E  val = %.8E\n", interp_out,
                    // v1[tz][ty][tx]);
                    dptr_out[var * unzip_sz + offset + kkz * lx * ly +
                             jjy * lx + iix] = v1[tz][ty][tx];
                }
            }
...
template <typename T, DEVICE_UINT p, DEVICE_UINT PW>
GLOBAL_FUNC __launch_bounds__(64) void __block_internal_unzip_dg__2(
    ...
    assert(ele >= dptr_mesh->m_blk_list[blk].m_elem_begin &&
           ele < dptr_mesh->m_blk_list[blk].m_elem_end);
...
GLOBAL_FUNC __launch_bounds__(64) void __unzip_dg__2(
    ...
    if (blk_id < dptr_mesh->m_e2b_unzip_map_count[ele]) {
        ...
        const DEVICE_UINT patch_level =
            dptr_mesh->m_blk_list[blk].m_patch_level;
        const DEVICE_UINT bLev =
            patch_level;  // dptr_mesh->m_all_elements[dptr_mesh->m_blk_list[blk].m_elem_begin].m_level;
                          // // or blkNode.m_level + (patch_level
                          // -blkNode.m_level);
        const DEVICE_UINT max_depth = dptr_mesh->m_max_depth;
            ...
            for (DEVICE_UINT child = 0; child < NUM_CHILDREN; child++) {
                ...
                if ((tx < nx && ty < nx)) {
                    ...
                    if (((jjy >= 0 && jjy < ly) && (iix >= 0 && iix < lx))) {
                        for (DEVICE_UINT tz = 0; tz < nx; tz++) {
                            const DEVICE_FLOAT32 zz = child_node.m_z + tz * hh;
                            const DEVICE_INT kkz =
                                std::round((zz - zmin) * invhh);

                            if ((kkz >= 0 && kkz < lz))
                                dptr_out[var * unzip_sz + offset +
                                         kkz * lx * ly + jjy * lx + iix] =
                                    v1[tz][ty][tx];
                        }
                    }
                }
...
DEVICE_FUNC inline void __p2c_3d__(const T refel_1d[][p + 1][p + 1],
                                   DEVICE_UINT cnum, const T* const dptr_in,
                                   T* const dptr_out, T* v1, T* v2) {
    const DEVICE_UINT tx      = GPUDevice::thread_id_x();
    const DEVICE_UINT ty      = GPUDevice::thread_id_y();
    const DEVICE_UINT cbit[3] = {(cnum & 1u), ((cnum & 2u) >> 1u),
                                 ((cnum & 4u) >> 2u)};
    const DEVICE_UINT nx      = p + 1;
    ...
    if ((tx < nx && ty < nx)) {
        for (DEVICE_UINT tz = 0; tz < (p + 1); tz++) {
            DEVICE_REAL val = 0;
            for (DEVICE_UINT m = 0; m < (p + 1); m++)
                val += refel_1d[cbit[0]][tx][m] *
                       dptr_in[IDX_DG(m, ty, tz)];  //[tz][ty][m];
    ...
    if ((tx < nx && ty < nx)) {
        for (DEVICE_UINT tz = 0; tz < (p + 1); tz++) {
            DEVICE_REAL val = 0;
            for (DEVICE_UINT m = 0; m < (p + 1); m++)
                val += refel_1d[cbit[1]][ty][m] *
                       v1[IDX_DG(tx, m, tz)];  //[tz][m][tx];
    ...
    if ((tx < nx && ty < nx)) {
        for (DEVICE_UINT tz = 0; tz < (p + 1); tz++) {
            DEVICE_REAL val = 0;
            for (DEVICE_UINT m = 0; m < (p + 1); m++)
                val += refel_1d[cbit[2]][tz][m] *
                       v2[IDX_DG(tx, ty, m)];  //[m][ty][tx];
...
template <typename T, DEVICE_UINT p>
DEVICE_FUNC inline void __p2c_2d__(const T refel_1d[][p + 1][p + 1],
                                   DEVICE_UINT cnum, DEVICE_UINT idx_base,
                                   DEVICE_UINT inc1, DEVICE_UINT inc2,
    ...
    if ((tx < nx && ty < nx)) {
        ...
        for (DEVICE_UINT m = 0; m < (p + 1); m++)
            val += refel_1d[cbit[0]][tx][m] *
                   dptr_in[idx_base + ty * inc2 + m * inc1];
        v1[ty * nx + tx] = val;
    }
...
template <typename T, DEVICE_UINT p>
DEVICE_FUNC inline void __p2c_1d__(const T refel_1d[][p + 1][p + 1],
                                   DEVICE_UINT cnum, DEVICE_UINT idx_base,
                                   DEVICE_UINT inc1, const T* const dptr_in,
                                   T* const dptr_out) {
    // const DEVICE_UINT cbit[3]  = {(cnum & 1u), ((cnum & 2u)>>1u), ((cnum &
    // 4u)>>2u)};
    //  const DEVICE_UINT tx     = GPUDevice::thread_id_x();
    //  const DEVICE_UINT ty     = GPUDevice::thread_id_y();
    const DEVICE_UINT nx = p + 1;
...
template <typename T, DEVICE_UINT p, DEVICE_UINT PW>
GLOBAL_FUNC __launch_bounds__(64) void __unzip_cg__2(
    const MeshGPU* const dptr_mesh, const T* const dptr_in, T* const dptr_out) {
    const DEVICE_UINT ele    = GPUDevice::block_id_x();
    const DEVICE_UINT blk_id = GPUDevice::block_id_y();
    const DEVICE_UINT var    = GPUDevice::block_id_z();
    ...
    if (blk_id < dptr_mesh->m_e2b_unzip_map_count[ele]) {
        ...
        if (tx < nx && ty < nx) {
            for (DEVICE_UINT tz = 0; tz < nx; tz++)
                cgVec[IDX_DG(tx, ty, tz)] =
                    dptr_in[var * cg_sz +
                            e2n_cg[ele * nPe + IDX_DG(tx, ty, tz)]];

            for (DEVICE_UINT tz = 0; tz < nx; tz++)
                dgVec[IDX_DG(tx, ty, tz)] = cgVec[IDX_DG(
                    tx, ty, tz)];  // dptr_in[var* cg_sz + e2n_cg[ele * nPe +
                                   // IDX_DG(tx,ty,tz)]];
        ...
        for (DEVICE_UINT s1 = 0; s1 < 3; s1 += 2) {
            ...
            if (is_hanging) {
                ...
                __p2c_2d__<DEVICE_REAL, p>(refEL, cnum, idx_base, inc1, inc2,
                                           cgVec, dgVec, v1);
        ...
        if (tx < 2 && ty < 2) {
            ...
            if ((!node_valid[1][s1][s2]) && is_hanging) {
                const DEVICE_UINT inc1 = 1;
                const DEVICE_UINT idx_base =
                    IDX_DG(0, ((s1 >> 1u) * p), ((s2 >> 1u) * p));

                __p2c_1d__<DEVICE_REAL, p>(refEL, cnum, idx_base, inc1, cgVec,
                                           dgVec);
            }
        ...
        if (ele >= dptr_mesh->m_blk_list[blk].m_elem_begin &&
            ...
            if (((jjy >= 0 && jjy < ly) && (iix >= 0 && iix < lx))) {
                for (DEVICE_UINT tz = 0; tz < nx; tz++) {
                    ...
                    if ((kkz >= 0 && kkz < lz))
                        dptr_out[var * unzip_sz + offset + kkz * lx * ly +
                                 jjy * lx + iix] = dgVec[IDX_DG(tx, ty, tz)];
                }
            }
...
template <typename T, DEVICE_UINT p, DEVICE_UINT PW>
GLOBAL_FUNC void __unzip_cg1__2(const MeshGPU* const dptr_mesh,
                                const T* const dptr_in, T* const dptr_out) {
    const DEVICE_UINT ele      = GPUDevice::block_id_x();
    // const DEVICE_UINT blk_id = GPUDevice::block_id_y();
    const DEVICE_UINT var      = GPUDevice::block_id_z();
    ...
    // bool is_refine[8]={false, false, false, false, false, false, false,
    // false};
    ...
    if (hflag != 0) {
        ...
        if (tx < nx && ty < nx) {
            ...
            DEVICE_INT owner_id =
                (dptr_mesh->m_e2n_dg[ele * (nPe) + idx] / nPe);
            DEVICE_INT l =
                dptr_mesh->m_all_elements[owner_id].m_level < ele_level;
            dgVec[idx] = v3[l][idx];
    ...
    // if(blk_id < dptr_mesh->m_e2b_unzip_map_count[ele])
    for (DEVICE_INT blk_id = 0; blk_id < dptr_mesh->m_e2b_unzip_map_count[ele];
         blk_id++) {
        const DEVICE_UINT blk =
            dptr_mesh->m_e2b_unzip_map[dptr_mesh->m_e2b_unzip_map_offset[ele] +
                                       blk_id];
        ...
        /*if(ele>= dptr_mesh->m_blk_list[blk].m_elem_begin && ele <
        dptr_mesh->m_blk_list[blk].m_elem_end)
        {
            if(!(tx < nx && ty < nx))
                return;

            const DEVICE_UINT
        ei=(elem_node.m_x-blkNode.m_x)>>(max_depth-patch_level); const
        DEVICE_UINT ej=(elem_node.m_y-blkNode.m_y)>>(max_depth-patch_level);
            const DEVICE_UINT
        ek=(elem_node.m_z-blkNode.m_z)>>(max_depth-patch_level);

            for(DEVICE_UINT tz=0; tz<nx; tz++)
                dptr_out[var * unzip_sz + offset + (ek*p+tz+PW) * (ly * lx) +(ej
        * p + ty + PW) * (lx) + (ei*p + tx + PW)] = dgVec[IDX_DG(tx,ty,tz)];
                ...
                if ((tx < nx && ty < nx)) {
                    ...
                    if (((jjy >= 0 && jjy < ly) && (iix >= 0 && iix < lx))) {
                        for (DEVICE_INT tz = 0; tz < nx; tz++) {
                            ...
                            //     dptr_out[var*unzip_sz + offset + kkz*lx*ly +
                            //     jjy*lx + iix] =
                            //     u_refine[child][IDX_DG(tx,ty,tz)];
                            if ((kkz >= 0 && kkz < lz))
                                dptr_out[var * unzip_sz + offset +
                                         kkz * lx * ly + jjy * lx + iix] =
                                    v3[1][IDX_DG(tx, ty, tz)];
                        }
                    }
                }
            }
...
template <typename T, DEVICE_UINT p, DEVICE_UINT PW>
GLOBAL_FUNC __launch_bounds__(8) void __unzip_dg__1(
    const MeshGPU* const dptr_mesh, const T* const dptr_in, T* const dptr_out) {
    const DEVICE_UINT ele    = GPUDevice::block_id_x();
    const DEVICE_UINT blk_id = GPUDevice::block_id_y();
    const DEVICE_UINT var    = GPUDevice::block_id_z();
    ...
    if (blk_id < dptr_mesh->m_e2b_unzip_map_count[ele]) {
        ...
        if (ele >= dptr_mesh->m_blk_list[blk].m_elem_begin &&
            ...
            if (tx < nx) {
                for (DEVICE_UINT tz = 0; tz < nx; tz++)
                    for (DEVICE_UINT ty = 0; ty < nx; ty++)
                        dgVec[tz][ty][tx] = dptr_in[var * dg_sz + ele * nPe +
                                                    IDX_DG(tx, ty, tz)];
            ...
            for (DEVICE_UINT child = 0; child < NUM_CHILDREN; child++) {
                ...
                if ((tx < nx)) {
                    ...
                    if (((iix >= 0 && iix < lx))) {
                        for (DEVICE_UINT tz = 0; tz < nx; tz++)
                            for (DEVICE_UINT ty = 0; ty < nx; ty++) {
                                const DEVICE_FLOAT32 yy =
                                    child_node.m_y + ty * hh;
                                const DEVICE_FLOAT32 zz =
                                    child_node.m_z + tz * hh;
...
template <typename T, DEVICE_UINT p, DEVICE_UINT PW>
GLOBAL_FUNC __launch_bounds__(64) void __zip_dg_enforce_c0__(
    const MeshGPU* const dptr_mesh, const T* const dptr_in, T* const dptr_out) {
    ...
    // implies no contribution for the unzip.
    const DEVICE_UINT num_elems_blk = (dptr_mesh->m_blk_list[blk].m_elem_end -
                                       dptr_mesh->m_blk_list[blk].m_elem_begin);
    if (num_elems_blk <= ele_id) return;
    ...
    if (tx < nx && ty < nx) {
        const DEVICE_UINT ele =
            dptr_mesh->m_blk_list[blk].m_elem_begin + ele_id;
        const DEVICE_UINT porder   = dptr_mesh->m_ele_order;
        const DEVICE_UINT unzip_sz = dptr_mesh->m_oct_unzip_sz;
        const DEVICE_UINT dg_sz    = dptr_mesh->m_oct_local_sz;
        const DEVICE_UINT nPe      = nx * nx * nx;
        ...
        for (DEVICE_UINT tz = 0; tz < (p + 1); tz += p) {
            const DEVICE_UINT dg_index =
                dptr_mesh->m_e2n_dg[ele * nPe + IDX_DG(tx, ty, tz)];
            ...
            if (dg_index != dg_ele) {
                ...
                if (dptr_mesh->m_all_elements[owner].m_level ==
                    dptr_mesh->m_all_elements[ele].m_level) {
                    DEVICE_UINT ee, k_ee, j_ee, i_ee;
                    dg2eijk(p, dg_index, ee, i_ee, j_ee, k_ee);
                    dptr_out[var * dg_sz + ele * nPe + IDX_DG(tx, ty, tz)] =
                        dptr_out[var * dg_sz + ee * nPe +
                                 IDX_DG(i_ee, j_ee, k_ee)];
                }
            }
        }
    }
...
GLOBAL_FUNC __launch_bounds__(64) void __zip_cg__(
    ...
    if (tx < nx && ty < nx) {
        ...
        for (DEVICE_UINT tz = 0; tz < (p + 1); tz++)
            if ((dptr_mesh->m_e2n_dg[ele * nPe + IDX_DG(tx, ty, tz)] / nPe) ==
                ele)
                dptr_out[var * cg_sz +
                         dptr_mesh->m_e2n_cg[ele * nPe + IDX_DG(tx, ty, tz)]] =
                    dptr_in[var * unzip_sz + offset +
                            (ek * p + tz + PW) * (ly * lx) +
                            (ej * p + ty + PW) * (lx) + (ei * p + tx + PW)];
    }
...
template <typename T>
GLOBAL_FUNC __launch_bounds__(64) void __write_sb_dg__(
    const MeshGPU* const dptr_mesh, const T* const dptr_in, T* const dptr_out) {
    ...
    if (dptr_mesh->m_elem_send_c[proc_id] <= ele_id) return;
...
template <typename T>
GLOBAL_FUNC __launch_bounds__(64) void __read_rb_dg__(
    const MeshGPU* const dptr_mesh, const T* const dptr_in, T* const dptr_out) {
    const DEVICE_UINT proc_id = GPUDevice::block_id_x();
    const DEVICE_UINT ele_id  = GPUDevice::block_id_y();
    const DEVICE_UINT var     = GPUDevice::block_id_z();
    ...
    if (dptr_mesh->m_elem_recv_c[proc_id] <= ele_id) return;
...
template <typename T>
GLOBAL_FUNC __launch_bounds__(GPU_MAX_THREADS_PER_BLOCK) void __write_sb_cg__(
    const MeshGPU* const dptr_mesh, const T* const dptr_in, T* const dptr_out) {
    ...
    const DEVICE_UINT t_id =
        GPUDevice::thread_id_x() + GPUDevice::block_id_x() * stride;
...
template <typename T>
GLOBAL_FUNC __launch_bounds__(GPU_MAX_THREADS_PER_BLOCK) void __read_rb_cg__(
    const MeshGPU* const dptr_mesh, const T* const dptr_in, T* const dptr_out) {
    const DEVICE_UINT proc_id = GPUDevice::block_id_y();
    const DEVICE_UINT var     = GPUDevice::block_id_z();
...
T* MeshGPU::createVector(DEVICE_UINT dof, bool is_local) {
    ...
    if (vtype == vec_type::device)
        (!is_local)
            ? vec = GPUDevice::device_malloc<T>(dof * m_oct_shared_sz)
            : vec = GPUDevice::device_malloc<T>(dof * m_num_local_nodes);
    else if (vtype == vec_type::host)
        (!is_local) ? vec = new T[dof * m_oct_shared_sz]
                    : vec = new T[dof * m_num_local_nodes];
    else if (vtype == vec_type::host_pinned)
        (!is_local) ? vec = GPUDevice::host_malloc<T>(dof * m_oct_shared_sz)
                    : vec = GPUDevice::host_malloc<T>(dof * m_num_local_nodes);
...
T* MeshGPU::createUnZippedVector(DEVICE_UINT dof, bool is_local) {
    T* vec = nullptr;
    if (vtype == vec_type::device)
        (!is_local) ? vec = GPUDevice::device_malloc<T>(dof * m_oct_unzip_sz)
                    : vec = GPUDevice::device_malloc<T>(dof * m_oct_unzip_sz);
    else if (vtype == vec_type::host)
        (!is_local) ? vec = new T[dof * m_oct_unzip_sz]
                    : vec = new T[dof * m_oct_unzip_sz];
    else if (vtype == vec_type::host_pinned)
        (!is_local) ? vec = GPUDevice::host_malloc<T>(dof * m_oct_unzip_sz)
                    : vec = GPUDevice::host_malloc<T>(dof * m_oct_unzip_sz);
...
T* MeshGPU::createDGVector(DEVICE_UINT dof, bool is_local) {
    ...
    const unsigned int nPe =
        (m_ele_order + 1) * (m_ele_order + 1) * (m_ele_order + 1);
    if (vtype == vec_type::device)
        (!is_local)
            ? vec = GPUDevice::device_malloc<T>(dof * m_oct_local_sz)
            : vec =
                  GPUDevice::device_malloc<T>(dof * m_num_local_elements * nPe);
    else if (vtype == vec_type::host)
        (!is_local) ? vec = new T[dof * m_oct_local_sz]
                    : vec = new T[dof * m_num_local_elements * nPe];
    else if (vtype == vec_type::host_pinned)
        (!is_local)
            ? vec = GPUDevice::host_malloc<T>(dof * m_oct_local_sz)
            : vec = GPUDevice::host_malloc<T>(dof * m_num_local_elements * nPe);
...
T* MeshGPU::createElementVector(DEVICE_UINT dof, bool is_local) {
    T* vec                              = nullptr;
    const unsigned int num_all_elements = m_num_pre_ghost_elements +
                                          m_num_local_elements +
                                          m_num_post_ghost_elements;
    if (vtype == vec_type::device)
        (!is_local)
            ? vec = GPUDevice::device_malloc<T>(dof * num_all_elements)
            : vec = GPUDevice::device_malloc<T>(dof * m_num_local_elements);
    else if (vtype == vec_type::host)
        (!is_local) ? vec = new T[dof * num_all_elements]
                    : vec = new T[dof * m_num_local_elements];
    else if (vtype == vec_type::host_pinned)
        (!is_local)
            ? vec = GPUDevice::host_malloc<T>(dof * num_all_elements)
            : vec = GPUDevice::host_malloc<T>(dof * m_num_local_elements);
...
void MeshGPU::unzip_dg(const ot::Mesh* const pMesh,
    ...
    assert(pMesh->getNumLocalMeshElements() ==
...
template <typename T, typename stream>
void MeshGPU::unzip_cg(const ot::Mesh* const pMesh,
                       const MeshGPU* const dptr_mesh, const T* const dptr_in,
                       T* const dptr_out, unsigned int dof, stream s) {
    if (!pMesh->isActive()) return;
...
template <typename T, typename stream>
void MeshGPU::zip_dg(const ot::Mesh* const pMesh,
                     const MeshGPU* const dptr_mesh, const T* const dptr_in,
                     T* const dptr_out, unsigned int dof, stream s) {
...
template <typename T, typename stream>
void MeshGPU::zip_cg(const ot::Mesh* const pMesh,
                     const MeshGPU* const dptr_mesh, const T* const dptr_in,
                     T* const dptr_out, unsigned int dof, stream s) {
    dim3 gb = dim3(m_zip_grid[0], m_zip_grid[1], dof);
    dim3 tb = dim3(8, 8, 1);
...
template <typename T, typename stream>
void MeshGPU::read_from_ghost_dg_begin(ot::AsyncExchangeContex& ctx_host,
                                       ot::AsyncExchangeContex& ctx_device,
                                       const ot::Mesh* const pMesh,
                                       const MeshGPU* const dptr_mesh,
                                       T* dptr_vec, unsigned int dof,
                                       stream s) {
    ...
    const unsigned int recvBSz =
        (re_f[active_p - 1] + re_c[active_p - 1]) * nPe;
    ...
    if (recvBSz) {
        ...
        for (unsigned int recv_p = 0; recv_p < recv_plist.size(); recv_p++) {
            ...
            par::Mpi_Irecv((recvB + dof * nPe * re_f[proc_id]),
                           dof * nPe * re_c[proc_id], proc_id, m_ctag,
                           commActive, &ctx_host.m_recv_req[recv_p]);
        }
    }
    ...
    if (sendBSz) {
        for (unsigned int send_p = 0; send_p < send_plist.size(); send_p++) {
            ...
            par::Mpi_Isend(sendB + dof * nPe * se_f[proc_id],
                           dof * nPe * se_c[proc_id], proc_id, m_ctag,
                           commActive, &ctx_host.m_send_req[send_p]);
        }
    }
#else
    T* const sendB = (T*)ctx_host.getSendBuffer();
    GPUDevice::device_to_host_async<DEVICE_REAL, cudaStream_t>(
        sendB, s_buff, dof * sendBSz, s);
    if (recvBSz) {
...
template <typename T, typename stream>
void MeshGPU::read_from_ghost_dg_end(ot::AsyncExchangeContex& ctx_host,
                                     ot::AsyncExchangeContex& ctx_device,
                                     const ot::Mesh* const pMesh,
                                     const MeshGPU* const dptr_mesh,
    ...
    if (recvBSz) {
        ...
        MPI_Waitall(send_plist.size(), ctx_host.m_send_req.data(),
                    MPI_STATUSES_IGNORE);
        MPI_Waitall(recv_plist.size(), ctx_host.m_recv_req.data(),
                    MPI_STATUSES_IGNORE);
...
template <typename T, typename stream>
void MeshGPU::read_from_ghost_cg_begin(ot::AsyncExchangeContex& ctx_host,
                                       ot::AsyncExchangeContex& ctx_device,
                                       const ot::Mesh* const pMesh,
                                       const MeshGPU* const dptr_mesh,
                                       T* dptr_vec, unsigned int dof,
    ...
    dim3 gb   = dim3(m_max_send_cg / GPU_MAX_THREADS_PER_BLOCK + 1,
                     pMesh->getMPICommSize(), dof);
    ...
    if (recvBSz) {
        ...
        for (unsigned int recv_p = 0; recv_p < recv_plist.size(); recv_p++) {
            const unsigned int proc_id = recv_plist[recv_p];
            par::Mpi_Irecv((recvB + dof * re_f[proc_id]), dof * re_c[proc_id],
                           proc_id, m_ctag, commActive,
                           &ctx_host.m_recv_req[recv_p]);
        }
    }
    GPUDevice::stream_synchronize(s);
    if (sendBSz) {
        for (unsigned int send_p = 0; send_p < send_plist.size(); send_p++) {
            const unsigned int proc_id = send_plist[send_p];
            par::Mpi_Isend(sendB + dof * se_f[proc_id], dof * se_c[proc_id],
                           proc_id, m_ctag, commActive,
                           &ctx_host.m_send_req[send_p]);
        }
    }
#else
    T* const sendB = (T*)ctx_host.getSendBuffer();
...
template <typename T, typename stream>
void MeshGPU::read_from_ghost_cg_end(ot::AsyncExchangeContex& ctx_host,
                                     ot::AsyncExchangeContex& ctx_device,
                                     const ot::Mesh* const pMesh,
                                     const MeshGPU* const dptr_mesh,
                                     T* dptr_vec, unsigned int dof, stream s) {
    ...
    if (recvBSz) {
        ...
        MPI_Waitall(recv_plist.size(), ctx_host.m_recv_req.data(),
                    MPI_STATUSES_IGNORE);
        dim3 gb = dim3(m_max_recv_cg / GPU_MAX_THREADS_PER_BLOCK + 1,
                       pMesh->getMPICommSize(), dof);
        dim3 tb = dim3(GPU_MAX_THREADS_PER_BLOCK, 1, 1);
