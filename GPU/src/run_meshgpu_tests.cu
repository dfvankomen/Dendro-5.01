...
const char* event_names[] = {"unzip_cg (gather)",
                             "unzip_cg (scatter)",
                             "unzip_dg (gather)",
                             "unzip_dg (scatter)",
                             "zip_cg",
...
int main(int argc, char** argv) {
    ...
    if (argc < 5) {
        if (!rank)
            std::cout << "Usage: " << argv[0]
                      << " maxDepth wavelet_tol partition_tol eleOrder run_gpu"
                      << std::endl;
        MPI_Abort(comm, 0);
    }
    ...
    std::function<void(double, double, double, double*)> func =
        [d_min, d_max, dof](double x, double y, double z, double* var) {
            double ca[] = {-2, 0, 0};
            double cb[] = {2, 0, 0};
            double rra =
                ((x - ca[0]) * (x - ca[0]) + (y - ca[1]) * (y - ca[1]) +
                 (z - ca[2]) * (z - ca[2])) /
                1.0;
            double rrb =
                ((x - cb[0]) * (x - cb[0]) + (y - cb[1]) * (y - cb[1]) +
                 (z - cb[2]) * (z - cb[2])) /
                1.0;
    ...
    std::function<double(double, double, double)> fr =
        [func, d_min, d_max, dof](double x, double y, double z) {
    ...
    if (run_gpu == 1) {
        ...
        GPUDevice::host_to_device<DEVICE_REAL>(u, dptr_vcg,
                                               mesh->getDegOfFreedom() * dof);
        GPUDevice::host_to_device<DEVICE_REAL>(v_dg, dptr_vdg,
                                               mesh->getDegOfFreedomDG() * dof);

        double* dptr_u_unzip =
            mesh_gpu.createUnZippedVector<double, device::vec_type::device>(
                dof);  // cg unzip
        double* dptr_v_unzip =
            mesh_gpu.createUnZippedVector<double, device::vec_type::device>(
                dof);  // dg unzip
        ...
        for (unsigned int w = 0; w < 1; w++) {
            ...
            mesh_gpu.unzip_dg(mesh, dptr_mesh, dptr_vdg, dptr_v_unzip, dof,
                              (cudaStream_t)0);
            GPUDevice::device_synchronize();
            cuda_check_last_error();

            GPUDevice::device_to_host<DEVICE_REAL>(
                u_unzip, dptr_u_unzip, mesh->getDegOfFreedomUnZip() * dof);
            GPUDevice::device_to_host<DEVICE_REAL>(
                v_unzip, dptr_v_unzip, mesh->getDegOfFreedomUnZip() * dof);
            for (unsigned int var = 0; var < std::min(3u, dof); var++) {
                valid = ot::test::isUnzipValid(
                    mesh, u_unzip + var * mesh->getDegOfFreedomUnZip(), fr,
                    unzip_check_tol);

                if (!rank)
                    std::cout << "GPU unzip valid cg scatter: " << valid
                              << "for dof: " << var << std::endl;
            }

            for (unsigned int var = 0; var < std::min(3u, dof); var++) {
                valid = ot::test::isUnzipValid(
                    mesh, v_unzip + var * mesh->getDegOfFreedomUnZip(), fr,
                    unzip_check_tol);
                if (!rank)
                    std::cout << "GPU unzip valid dg scatter: " << valid
                              << "for dof: " << var << std::endl;
            }

            mesh_gpu.zip_cg(mesh, dptr_mesh, dptr_u_unzip, dptr_vcg, dof,
                            (cudaStream_t)0);
            mesh_gpu.zip_dg(mesh, dptr_mesh, dptr_v_unzip, dptr_vdg, dof,
                            (cudaStream_t)0);
            GPUDevice::device_to_host<DEVICE_REAL>(
                u, dptr_vcg, mesh->getDegOfFreedom() * dof);
            GPUDevice::device_to_host<DEVICE_REAL>(
                v_dg, dptr_vdg, mesh->getDegOfFreedomDG() * dof);
            cuda_check_last_error();
        }
        ...
        for (unsigned int i = 0; i < blkIDs.size(); i++) blkIDs[i] = i;
        ...
        for (unsigned int var = 0; var < std::min(3u, dof); var++) {
            valid = ot::test::isUnzipValid(
                ...
                unzip_check_tol);
            if (!rank)
                std::cout << "unzip valid scatter: " << valid
                          << "for dof: " << var << std::endl;
        }
        ...
        for (unsigned int var = 0; var < std::min(3u, dof); var++) {
            valid = ot::test::isUnzipValid(
                ...
                unzip_check_tol);
            if (!rank)
                std::cout << "unzip valid dg scatter: " << valid
                          << "for dof: " << var << std::endl;
        }
        ...
        for (unsigned int var = 0; var < std::min(3u, dof); var++) {
            valid = ot::test::isUnzipValid(
                ...
                unzip_check_tol);
            if (!rank)
                std::cout << "unzip valid gather: " << valid
                          << "for dof: " << var << std::endl;
        }
        ...
        for (unsigned int var = 0; var < std::min(3u, dof); var++) {
            valid = ot::test::isUnzipValid(
                ...
                unzip_check_tol);
            if (!rank)
                std::cout << "unzip valid dg gather: " << valid
                          << "for dof: " << var << std::endl;
        }
        ...
        for (unsigned int i = 0; i < iter; i++)
            mesh->unzip(u, u_unzip, blkIDs.data(), blkIDs.size(),
                        dof);  // mesh->unzip(u,u_unzip,dof);
        ...
        for (unsigned int i = 0; i < iter; i++)
            for (unsigned int var = 0; var < dof; var++)
                mesh->zip(u_unzip + var * mesh->getDegOfFreedomUnZip(),
                          u + var * mesh->getDegOfFreedom());
