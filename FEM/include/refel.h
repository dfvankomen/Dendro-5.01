...
class RefElement {
   protected:
    ...
    /** 1D reference coordinates of the interpolation nodes (uniform nodal
     * points) */
    std::vector<double> u;
    ...
    // Todo: remove this once the all the children to parent computations are
    // finalized.(i.e. write this in the tensor form. )
    /** p2c transpose */
   ...
   public:
    ...
    /**@brief: derivative of the basis functions evaluated at the quadrature
     * points. */
    inline const double *getDg1d() const { return &(*(Dg.begin())); }
    /**@brief: derivative of the basis functions evaluated at the quadrature
     * points (Transpose) */
    inline const double *getDgT1d() const { return &(*(DgT.begin())); }
    /**@brief: derivative of the basis functions evaluated at the nodal
     * locations points  */
     ...
     * @param[in] childNum: Morton ID of the child number where the
     * interpolation is needed.
     * @param[out] out: interpolated values.
     *
     * @brief This is computed in way that 3d coordinates changes in the order
     * of z, y, x Which means first we need to fill all the z values in
     * plane(x=0,y=0) then all the z values in plane (x=0,y=0+h) and so forth.
     *
     */
    ...
    /**
     * @param[in] in: input function values.
     * @param[in] childNum: Morton ID of the child number where the contribution
     * needed to be computed.
     * @param[out] out: child to parent contribution values. (used in FEM
     * integral ivaluation)
     *
     * @brief This is computed in way that 3d coordinates changes in the order
     * of z, y, x Which means first we need to fill all the z values in
    ...
    inline void I3D_Child2Parent(const double *in, double *out,
        ...
        switch (childNum) {
            case 0:
                DENDRO_TENSOR_IIAX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_0.begin())),
                                              in, im1);  // along x
                DENDRO_TENSOR_IAIX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_0.begin())),
                                              im1, im2);  // along y
                DENDRO_TENSOR_AIIX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_0.begin())),
                                              im2, out);  // along z
                break;
            case 1:
                DENDRO_TENSOR_IIAX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_1.begin())),
                                              in, im1);  // along x
                DENDRO_TENSOR_IAIX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_0.begin())),
                                              im1, im2);  // along y
                DENDRO_TENSOR_AIIX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_0.begin())),
            ...
            case 2:
                DENDRO_TENSOR_IIAX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_0.begin())),
                                              in, im1);  // along x
                DENDRO_TENSOR_IAIX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_1.begin())),
                                              im1, im2);  // along y
                DENDRO_TENSOR_AIIX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_0.begin())),
                                              im2, out);  // along z
                break;
            ...
            case 4:
                ...
                DENDRO_TENSOR_IAIX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_0.begin())),
                                              im1, im2);  // along y
                DENDRO_TENSOR_AIIX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_1.begin())),
                                              im2, out);  // along z
                break;
            case 5:
                DENDRO_TENSOR_IIAX_APPLY_ELEM(m_uiNrp, &(*(ipT_1D_1.begin())),
        ...
        for (unsigned int node = 0; node < (m_uiNrp * m_uiNrp * m_uiNrp);
             node++)
            out[node] = 1.0;
#endif
    }
    ...
    inline void I2D_Child2Parent(const double *in, double *out,
        ...
        switch (childNum) {
            case 0:
                DENDRO_TENSOR_IAX_APPLY_ELEM_2D(m_uiNrp, &(*(ipT_1D_0.begin())),
                                                in, im1);  // along x
                DENDRO_TENSOR_AIX_APPLY_ELEM_2D(m_uiNrp, &(*(ipT_1D_0.begin())),
                                                im1, out);  // along y (in 3d z)
                break;
            case 1:
                DENDRO_TENSOR_IAX_APPLY_ELEM_2D(m_uiNrp, &(*(ipT_1D_1.begin())),
                                                in, im1);  // along x
                DENDRO_TENSOR_AIX_APPLY_ELEM_2D(m_uiNrp, &(*(ipT_1D_0.begin())),
                                                im1, out);  // along y (in 3d z)
                break;
            ...
            case 2:
                DENDRO_TENSOR_IAX_APPLY_ELEM_2D(m_uiNrp, &(*(ipT_1D_0.begin())),
                                                in, im1);  // along x
                DENDRO_TENSOR_AIX_APPLY_ELEM_2D(m_uiNrp, &(*(ipT_1D_1.begin())),
                                                im1, out);  // along y (in 3d z)
                break;
    ...
    inline void I1D_Parent2Child(const double *in, double *out,
                                 unsigned int childNUm) const {
        switch (childNUm) {
            case 0:
     ...
     * @param [in] in input function values
     * @param [in] childNum Morton ID of the child number
     * @param [out] child to parent contribution values. (used in FEM integral
     * ivaluation)
     * */

    inline void I1D_Child2Parent(const double *in, double *out,
                                 unsigned int childNUm) const {
        switch (childNUm) {
            case 0:
                for (unsigned int i = 0; i < m_uiNrp; i++) {
...
#ifdef FEM_ACCUMILATE_ONES_TEST
        for (unsigned int node = 0; node < (m_uiNrp); node++) out[node] = 1.0;
#endif
    }
     ...
     * @param in : input vector (children values, (2N+1)^3) where N is the
     * element order.
