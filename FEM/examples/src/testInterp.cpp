 ...
 *@brief Creates a unit cube (element) and perform the interpolation by refining
 *it hierarchically.
 */
//
...
int main(int argc, char** argv) {
    ...
    if (argc < 5) {
        if (!rank)
            std::cout << "Usage: " << argv[0]
                      << " maxDepth wavelet_tol partition_tol eleOrder cnum"
                      << std::endl;
        return 0;
    }
    ...
    std::function<double(double)> func1D = [](const double x) {
        return (sin(2 * M_PI * x));
    };
    // std::function<double(double,double,double)> func =[](const double
    // &x,const double & y,const double &z){ return (x*x+y*y+z*z);};
    // std::function<double(double,double,double)> func =[](const double
    // &x,const double & y,const double &z){ return (x+y+z);};

    std::vector<ot::TreeNode> tmpNodes;
    if (npes > 1)
        if (!rank)
            std::cout << "This needs to be run sequentially. " << std::endl;
    ...
    if (npes == 1) {
        ...
        do
        {
            ...
            //std::cout<<"parent_pt: "<<ptParent.x()<<" ,"<<ptParent.y()<<"
        ,"<<ptParent.z()<<std::endl; for(unsigned int i=0;i<(eOrder+1);i++)
            ...
            //std::cout<<"child_pt: "<<ptChild.x()<<" ,"<<ptChild.y()<<"
        ,"<<ptChild.z()<<std::endl;
        ...
        do {
            ...
            for (unsigned int k = 0; k < (eOrder + 1); k++)
                for (unsigned int j = 0; j < (eOrder + 1); j++)
                    for (unsigned int i = 0; i < (eOrder + 1); i++)
                        pValue[k * (eOrder + 1) * (eOrder + 1) +
                               j * (eOrder + 1) + i] =
                            func(ptParent.x() + i * hp, ptParent.y() + j * hp,
                                 ptParent.z() + k * hp);
            ...
            if (cnum == 0)
                ...
                ptChild =
                    Point(ptParent.x() + mySz, ptParent.y(), ptParent.z());
            else if (cnum == 2)
                ptChild =
                    Point(ptParent.x(), ptParent.y() + mySz, ptParent.z());
            else if (cnum == 3)
                ptChild = Point(ptParent.x() + mySz, ptParent.y() + mySz,
                                ptParent.z());
            else if (cnum == 4)
                ptChild =
                    Point(ptParent.x(), ptParent.y(), ptParent.z() + mySz);
            else if (cnum == 5)
                ptChild = Point(ptParent.x() + mySz, ptParent.y(),
                                ptParent.z() + mySz);
            else if (cnum == 6)
                ptChild = Point(ptParent.x(), ptParent.y() + mySz,
                                ptParent.z() + mySz);
            else if (cnum == 7)
                ptChild = Point(ptParent.x() + mySz, ptParent.y() + mySz,
                                ptParent.z() + mySz);
            ...
            for (unsigned int k = 0; k < (eOrder + 1); k++)
                for (unsigned int j = 0; j < (eOrder + 1); j++)
                    for (unsigned int i = 0; i < (eOrder + 1); i++)
                        cValue[k * (eOrder + 1) * (eOrder + 1) +
                               j * (eOrder + 1) + i] =
                            func(ptChild.x() + i * hp, ptChild.y() + j * hp,
                                 ptChild.z() + k * hp);

            /* std::cout<<" child_ip: ";printArray_1D(cValue_ip,nodesPerElem);
             std::cout<<" child: ";printArray_1D(cValue,nodesPerElem);*/

            l2 = normL2(cValue, cValue_ip, nodesPerElem) /
                 normL2(cValue, nodesPerElem);
            depth++;
            std::cout << "depth: " << depth << " relErr: " << l2 << std::endl;
