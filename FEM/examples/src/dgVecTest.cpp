 ...
 * @brief : Simple code to perform some examples in DG functionalities, in
 * Dendro.
 * @version 0.1
 * @date 2020-06-28
 *
...
int main(int argc, char** argv) {
            ...
            for (unsigned int k = 0; k < eOrder + 1; k++)
                for (unsigned int j = 0; j < eOrder + 1; j++)
                    for (unsigned int i = 0; i < eOrder + 1; i++) {
                        double x =
                            pNodes[ele].minX() +
                            i * ((1u << m_uiMaxDepth - pNodes[ele].getLevel()) /
                                 eOrder);
                        double y =
                            pNodes[ele].minY() +
                            j * ((1u << m_uiMaxDepth - pNodes[ele].getLevel()) /
                                 eOrder);
                        double z =
                            pNodes[ele].minZ() +
                            k * ((1u << m_uiMaxDepth - pNodes[ele].getLevel()) /
                                 eOrder);

                        (dg_vec + v * dg_sz)[ele * nPe +
                                             k * (eOrder + 1) * (eOrder + 1) +
                                             j * (eOrder + 1) + i] =
                            func(x, y, z);
                    }
    ...
    const double* pData[2] = {dg_vecNew,
                              dg_vecNew + newMesh->getDegOfFreedomDG()};

    newMesh->readFromGhostBeginEleDGVec(dg_vecNew, dof);
    newMesh->readFromGhostEndEleDGVec(dg_vecNew, dof);

    io::vtk::mesh2vtuFine(newMesh, "dgVec", 0, NULL, NULL, dof, vNames, pData,
                          0, NULL, NULL, true);
