 ...
 *@brief This file contains a basic laplace or poisson equation and solve that
 *using FEM based on SFC traversal based matvec
 *
 * \Delta u = f
...
// NOTE: the includes here must be in this order or else it doesn't compile.
// clang-format off
#include "TreeNode.h"
#include "mpi.h"
...
int main(int argc, char** argv) {
    ...
    fem::domain::Rd_x =
        (fem::domain::domain_max.x() - fem::domain::domain_min.x());
    fem::domain::Rd_y =
        (fem::domain::domain_max.y() - fem::domain::domain_min.y());
    fem::domain::Rd_z =
        (fem::domain::domain_max.z() - fem::domain::domain_min.z());
    ...
    std::function<double(double, double, double)> f_rhs = [d_min, d_max, g_min,
                                                           g_max, Rg_x, Rg_y,
                                                           Rg_z, Rd_x, Rd_y,
                                                           Rd_z](
                                                              const double x,
        ...
        return (
            -12 * M_PI * M_PI *
            sin(2 * M_PI * (((x - g_min.x()) / (Rg_x)) * (Rd_x) + d_min.x())) *
            sin(2 * M_PI * (((y - g_min.y()) / (Rg_y)) * (Rd_y) + d_min.y())) *
            sin(2 * M_PI * (((z - g_min.z()) / (Rg_z)) * (Rd_z) + d_min.z())));
    };
    ...
    if (argc > 5) {
        if (!rank)
            std::cout
                << "using randomly generated mesh initialized to the f_rhs"
                << std::endl;
        numPts                    = atoi(argv[5]);
        ...
        pts2Octants(tmpNodes, pts, totPts, m_uiDim,
                    m_uiMaxDepth);  // generate octants from the points.

        delete[] pts;
    } else {
        function2Octree(f_rhs, tmpNodes, m_uiMaxDepth, wavelet_tol, eOrder,
                        comm);
    }
    ...
    shrinkOrExpandOctree(tmpNodes, partition_tol, DENDRO_DEFAULT_SF_K, isActive,
                         commActive, comm);
    ...
    if (isActive) {
        ...
        SFC::parSort::SFC_treeSort(tmpNodes, tmpVec, tmpVec, tmpVec,
                                   partition_tol, m_uiMaxDepth, root,
    ...
    t1             = MPI_Wtime();
    ot::Mesh* mesh = new ot::Mesh(balOct, 1, eOrder, comm, true,
                                  ot::SM_TYPE::FEM_CG, grainSz, partition_tol);
    t2             = MPI_Wtime();
    ...
    if (mesh->isActive()) {
        ...
        unsigned int nodeLookUp_CG, nodeLookUp_DG, ownerID, ii_x, jj_y, kk_z,
            len;
        unsigned int x, y, z;
        ...
        /*for(unsigned int
        elem=mesh->getElementLocalBegin();elem<mesh->getElementLocalEnd();elem++)
        {
        ...
        nodeLookUp_CG<nodeLocalEnd)
                        {
                            ...
                            mesh->dg2eijk(nodeLookUp_DG,ownerID,ii_x,jj_y,kk_z);
                            len=1u<<(m_uiMaxDepth-allElements[ownerID].getLevel());
                            x=allElements[ownerID].getX()+
        ii_x*(len/(eleOrder)); y=allElements[ownerID].getY()+
        jj_y*(len/(eleOrder)); z=allElements[ownerID].getZ()+
        kk_z*(len/(eleOrder)); assert(len%eleOrder==0); if(ownerID==elem)
                                pt_list.push_back(Point(x,y,z));
        ...
        /*for(unsigned int i=0;i<pt_list.size();i++)
            std::cout<<"pt_list  "<<i<<" x : "<<pt_list[i].xint()<<" y:
           "<<pt_list[i].yint()<<" z: "<<pt_list[i].zint()<<std::endl;*/
        ...
        if (!rank)
            std::cout << " mesh matvec time (s): " << (t2 - t1) << std::endl;
        ...
        for (unsigned int iter = 0; iter < NUM_ITER; iter++)
            fem::seq::matvec(&(*(pt_list.begin())), rhs, 0, pt_list.size(), 0,
                             m_uiMaxDepth, eleOrder, rootNode, &refEl, Mrhs_sfc,
                             0);
        ...
        if (!rank)
            std::cout << " sfc matvec time (s): " << (t2 - t1) << std::endl;
        ...
        for (unsigned int node = nodeLocalBegin; node < nodeLocalEnd; node++) {
            ...
            /*if(fabs(Mrhs_sfc[node]-Mrhs[node])>1e-6)
            {
                nodeLookUp_DG=cg2dg[node];
                mesh->dg2eijk(nodeLookUp_DG,ownerID,ii_x,jj_y,kk_z);
                std::cout<<"node : "<<node<<" Mrhs: "<<Mrhs[node]<<" Mrhs_sfc:
            "<<Mrhs_sfc[node]<<" diff "<<fabs(Mrhs_sfc[node]-Mrhs[node])<<" ptX:
            "<<pt_list[node].xint()<<" ptY: "<<pt_list[node].yint()<<" ptZ:
            "<<pt_list[node].zint()<<" owner: "<<allElements[ownerID]<<"
            (i,j,k): ( "<<ii_x<<", "<<jj_y<<", "<<kk_z<<" )"<<std::endl;
            }*/
        }
