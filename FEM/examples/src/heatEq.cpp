...
int main(int argc, char** argv) {
    ...
    Point grid_max((1u << m_uiMaxDepth), (1u << m_uiMaxDepth),
                   (1u << m_uiMaxDepth));
    ...
    std::function<void(double, double, double, double*)> f_rhs =
        [d_min, d_max, g_min, g_max, Rg_x, Rg_y, Rg_z, Rd_x, Rd_y, Rd_z](
            const double x, const double y, const double z, double* var) {
            var[0] = (-12 * M_PI * M_PI *
                      sin(2 * M_PI *
                          (((x - g_min.x()) / (Rg_x)) * (Rd_x) + d_min.x())) *
                      sin(2 * M_PI *
                          (((y - g_min.y()) / (Rg_y)) * (Rd_y) + d_min.y())) *
                      sin(2 * M_PI *
                          (((z - g_min.z()) / (Rg_z)) * (Rd_z) + d_min.z())));
    ...
    std::function<void(double, double, double, double*)> f_init =
        [d_min, d_max, g_min, g_max, Rg_x, Rg_y, Rg_z, Rd_x, Rd_y, Rd_z](
            const double x, const double y, const double z, double* var) {
            var[0] =
                0;  //(-12*M_PI*M_PI*sin(2*M_PI*(((x-g_min.x())/(Rg_x))*(Rd_x)+d_min.x()))*sin(2*M_PI*(((y-g_min.y())/(Rg_y))*(Rd_y)+d_min.y()))*sin(2*M_PI*(((z-g_min.z())/(Rg_z))*(Rd_z)+d_min.z())));
            // var[1]=0;
            // var[2]=0;
        };
    ...
    ot::DA* octDA = new ot::DA(f_rhs, 1, comm, eOrder, wavelet_tol, 100,
                               partition_tol, ot::FEM_CG);
    ...
    // ot::DA* octDA=new ot::DA(pts, 100, Point(0.0,0.0,0.0),
    // Point(1.0,1.0,1.0), comm, eOrder, 100,
    // partition_tol,ot::SM_TYPE::FEM_CG);
    ...
    double* ux =
        octDA->getVecPointerToDof(uSolVecPtr, VAR::M_UI_U, false, false);
    double* frhs =
        octDA->getVecPointerToDof(uSolVecPtr, VAR::M_UI_F, false, false);
    double* Mfrhs =
        octDA->getVecPointerToDof(uSolVecPtr, VAR::M_UI_MF, false, false);
    ...
    if (!rank) std::cout << " end of heatEq: " << std::endl;
