 ...
 * @brief This an example of using the interpolation on a octree using tensor
 * contraction.
 *
 * */
...
int main(int argc, char** argv) {
    ...
    if (argc < 3) {
        if (!rank)
            std::cout
                ...
                << " maxDepth "
                << " tol  "
                << " distribution (0- Normal 1- Uniform 2- LogarithmicNormal) "
                   "options( 1-Remove duplicates 2- constructOctree "
                   "6-balancedOctree)  splitterFixK  interpOrder"
                << std::endl;
    }
    ...
    if (argc > 7) sf_k = atoi(argv[7]);

    if (argc > 8) interpOrder = atoi(argv[8]);
    ...
    if (!rank)
        std::cout << "Octree Generated: " << activeOctree.size() << std::endl;
    ...
    if (activeOctree.size() != 0) {
        ...
        for (unsigned int w = 0; w < activeOctree.size(); w++) {
            for (unsigned int k = 0; k < (interpOrder + 1); k++) {
                z = activeOctree[w].minZ() +
                    k * (activeOctree[w].maxZ() - activeOctree[w].minZ()) /
                        (double)interpOrder;

                for (unsigned int j = 0; j < (interpOrder + 1); j++) {
                    y = activeOctree[w].minY() +
                        j * (activeOctree[w].maxY() - activeOctree[w].minY()) /
                            (double)interpOrder;
                    for (unsigned int i = 0; i < (interpOrder + 1); i++) {
                        x = activeOctree[w].minX() +
                            i *
                                (activeOctree[w].maxX() -
                                 activeOctree[w].minX()) /
                                (double)interpOrder;
                        f_val[w * Np +
                              k * (interpOrder + 1) * (interpOrder + 1) +
                              j * (interpOrder + 1) + i] = evalOct(x, y, z);
                    }
                }
            }
        }
        ...
        for (unsigned int w = 0; w < activeOctree.size(); w++) {
            ...
            if (li < (m_uiMaxDepth - 1)) {
                ...
                for (unsigned int mChildNum = 0; mChildNum < NUM_CHILDREN;
                     mChildNum++) {
                    switch (mChildNum) {
                        case 0:
                            tmp = ot::TreeNode(xi, yi, zi, li + 1, m_uiDim,
                                               m_uiMaxDepth);
                            refEl.I3D_Parent2Child(f_val + w * Np, f_intp, 0);
                            for (unsigned int k = 0; k < (interpOrder + 1);
                                 k++) {
                                z = tmp.minZ() + k * (tmp.maxZ() - tmp.minZ()) /
                                                     (double)interpOrder;
                                for (unsigned int j = 0; j < (interpOrder + 1);
                                     j++) {
                                    y = tmp.minY() +
                                        j * (tmp.maxY() - tmp.minY()) /
                                            (double)interpOrder;
                            ...
                            if (!rank) std::cout << " f_val for " << w << " :";
                            printArray_1D(f_val + w * Np, Np);
                            if (!rank) std::cout << " f_cal for " << w << " :";
                            printArray_1D(f_calc, Np);
                            if (!rank) std::cout << " f_inp for " << w << " :";
                            printArray_1D(f_intp, Np);
#endif
                            break;

                        case 1:
                            tmp = ot::TreeNode(xi + sz, yi, zi, li + 1, m_uiDim,
                                               m_uiMaxDepth);
                            ...
                            break;
                        case 2:
                            tmp = ot::TreeNode(xi, yi + sz, zi, li + 1, m_uiDim,
                                               m_uiMaxDepth);
                        ...
                        case 3:
                            tmp = ot::TreeNode(xi + sz, yi + sz, zi, li + 1,
                            ...
                            break;
                        case 4:
                            tmp = ot::TreeNode(xi, yi, zi + sz, li + 1, m_uiDim,
                                               m_uiMaxDepth);
                            ...
                            break;
                        case 5:
                            tmp = ot::TreeNode(xi + sz, yi, zi + sz, li + 1,
                                               m_uiDim, m_uiMaxDepth);
                            ...
                            break;
                        case 6:
                            tmp = ot::TreeNode(xi, yi + sz, zi + sz, li + 1,
                                               m_uiDim, m_uiMaxDepth);
                            ...
                            break;
                        case 7:
                            tmp = ot::TreeNode(xi + sz, yi + sz, zi + sz,
                                               li + 1, m_uiDim, m_uiMaxDepth);
                    ...
                    if (l2Norm(f_calc, f_intp, Np) > 1)
                        std::cout << "Rank " << rank << " Octant: " << w
                                  << " to  Child Num : " << mChildNum
                                  << " normed difference: "
                                  << l2Norm(f_calc, f_intp, Np) << std::endl;
                }
            }
        }
