...
namespace io {
namespace vtk {
void mesh2vtk(const ot::Mesh *pMesh, const char *fPrefix,
              unsigned int numFieldData, const char **filedDataNames,
              double *filedData, unsigned int numPointdata,
              const char **pointDataNames, double **pointData) {
    ...
    if (fp == NULL) {
        std::cout << "rank: " << rank
                  << "[IO Error]: Could not open the vtk file. " << std::endl;
        return;
    }
    ...
    unsigned int num_vertices =
        pMesh->getNumLocalMeshElements() * pMesh->getNumNodesPerElement();
        ...
        for (unsigned int k = 0; k < (eleOrder + 1); k++)
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    fprintf(
                        fp, "%f %f %f \n",
                        (float)(pElements[ele].getX() + i * (sz / eleOrder)),
                        (float)(pElements[ele].getY() + j * (sz / eleOrder)),
                        (float)(pElements[ele].getZ() + k * (sz / eleOrder)));
                }
    }
    ...
    for (unsigned int i = 0; i < num_cells; i++) {
        fprintf(
            ...
            (DendroIntL)(i * nPe + 0 * (eleOrder + 1) * (eleOrder + 1) +
                         0 * (eleOrder + 1) + 0),
            (DendroIntL)(i * nPe + 0 * (eleOrder + 1) * (eleOrder + 1) +
                         0 * (eleOrder + 1) + eleOrder),
            (DendroIntL)(i * nPe + 0 * (eleOrder + 1) * (eleOrder + 1) +
                         eleOrder * (eleOrder + 1) + eleOrder),
            (DendroIntL)(i * nPe + 0 * (eleOrder + 1) * (eleOrder + 1) +
                         eleOrder * (eleOrder + 1) + 0),
            (DendroIntL)(i * nPe + eleOrder * (eleOrder + 1) * (eleOrder + 1) +
                         0 * (eleOrder + 1) + 0),
            (DendroIntL)(i * nPe + eleOrder * (eleOrder + 1) * (eleOrder + 1) +
                         0 * (eleOrder + 1) + eleOrder),
            (DendroIntL)(i * nPe + eleOrder * (eleOrder + 1) * (eleOrder + 1) +
...
void mesh2vtu(const ot::Mesh *pMesh, const char *fPrefix,
              unsigned int numFieldData, const char **filedDataNames,
              const double *filedData, unsigned int numPointdata,
    ...
    if (numFieldData > 0 && filedData != NULL) {
        ...
        for (unsigned int fdata = 0; fdata < numFieldData; fdata++) {
            fprintf(
                fp,
                "        <DataArray type=\"%s\" Name=\"%s\" format=\"%s\">\n",
                DENDRO_NODE_VAR_FLOAT, filedDataNames[fdata],
                DENDRO_FORMAT_ASCII);
            fprintf(fp, "%f\n", filedData[fdata]);
    ...
    for (unsigned int ele = pMesh->getElementLocalBegin();
        ...
        for (unsigned int k = 0; k < (eleOrder + 1); k++)
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    fprintf(fp, "          %d %d %d\n",
                            VTU_OCT_X_GRID_X(pElements[ele].getX() +
                                             i * (sz / eleOrder)),
                            VTU_OCT_Y_GRID_Y(pElements[ele].getY() +
                                             j * (sz / eleOrder)),
                            VTU_OCT_Z_GRID_Z(pElements[ele].getZ() +
                                             k * (sz / eleOrder)));
                }
    }
#else

    DENDRO_NODE_COORD_DTYPE *coord_data =
        new DENDRO_NODE_COORD_DTYPE[num_vertices * m_uiDim];
    ...
    for (unsigned int ele = pMesh->getElementLocalBegin();
        ...
        for (unsigned int k = 0; k < (eleOrder + 1); k++)
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    coord_data[((ele - pMesh->getElementLocalBegin()) * nPe +
                                k * (eleOrder + 1) * (eleOrder + 1) +
                                j * (eleOrder + 1) + i) *
                                   m_uiDim +
                               0] = VTU_OCT_X_GRID_X(pElements[ele].getX() +
                                                     i * (sz / eleOrder));
                    coord_data[((ele - pMesh->getElementLocalBegin()) * nPe +
                                k * (eleOrder + 1) * (eleOrder + 1) +
                                j * (eleOrder + 1) + i) *
                                   m_uiDim +
                               1] = VTU_OCT_Y_GRID_Y(pElements[ele].getY() +
                                                     j * (sz / eleOrder));
                    coord_data[((ele - pMesh->getElementLocalBegin()) * nPe +
                                k * (eleOrder + 1) * (eleOrder + 1) +
                                j * (eleOrder + 1) + i) *
                                   m_uiDim +
                               2] = VTU_OCT_Z_GRID_Z(pElements[ele].getZ() +
                                                     k * (sz / eleOrder));
                }
    }
    ...
    for (unsigned int i = 0; i < num_cells; i++) {
        fprintf(
            fp, "          %lld %lld %lld %lld %lld %lld %lld %lld\n",
            (DendroIntL)(i * nPe + 0 * (eleOrder + 1) * (eleOrder + 1) +
                         0 * (eleOrder + 1) + 0),
            (DendroIntL)(i * nPe + 0 * (eleOrder + 1) * (eleOrder + 1) +
                         0 * (eleOrder + 1) + eleOrder),
    ...
    for (unsigned int i = 0; i < num_cells; i++) {
        locidx_data[i * NUM_CHILDREN + 0] =
            (DendroIntL)(i * nPe + 0 * (eleOrder + 1) * (eleOrder + 1) +
                         0 * (eleOrder + 1) + 0);
        locidx_data[i * NUM_CHILDREN + 1] =
            (DendroIntL)(i * nPe + 0 * (eleOrder + 1) * (eleOrder + 1) +
                         0 * (eleOrder + 1) + eleOrder);
        locidx_data[i * NUM_CHILDREN + 2] =
            (DendroIntL)(i * nPe + 0 * (eleOrder + 1) * (eleOrder + 1) +
                         eleOrder * (eleOrder + 1) + eleOrder);
        locidx_data[i * NUM_CHILDREN + 3] =
            (DendroIntL)(i * nPe + 0 * (eleOrder + 1) * (eleOrder + 1) +
                         eleOrder * (eleOrder + 1) + 0);
        locidx_data[i * NUM_CHILDREN + 4] =
            (DendroIntL)(i * nPe + eleOrder * (eleOrder + 1) * (eleOrder + 1) +
                         0 * (eleOrder + 1) + 0);
        locidx_data[i * NUM_CHILDREN + 5] =
            (DendroIntL)(i * nPe + eleOrder * (eleOrder + 1) * (eleOrder + 1) +
                         0 * (eleOrder + 1) + eleOrder);
        locidx_data[i * NUM_CHILDREN + 6] =
            (DendroIntL)(i * nPe + eleOrder * (eleOrder + 1) * (eleOrder + 1) +
                         eleOrder * (eleOrder + 1) + eleOrder);
        locidx_data[i * NUM_CHILDREN + 7] =
            (DendroIntL)(i * nPe + eleOrder * (eleOrder + 1) * (eleOrder + 1) +
                         eleOrder * (eleOrder + 1) + 0);
    }
    retval = vtk_write_binary(fp, (char *)locidx_data,
                              sizeof(*locidx_data) * NUM_CHILDREN * num_cells);
    if (retval) {
        std::cout << rank << ": [VTU Error]: "
                  << "base64 encode connectivity data failed" << std::endl;
        fclose(fp);
    }
    delete[] locidx_data;
    ...
    for (unsigned int il = 1, sk = 1; il <= num_cells; ++il, ++sk) {
        ...
        if (!(sk % 8) && il != num_cells) fprintf(fp, "\n         ");
    }
    fprintf(fp, "\n");
#else
    fprintf(fp,
    ...
    for (unsigned int il = 0, sk = 1; il < num_cells; ++il, ++sk) {
        ...
        if (!(sk % 20) && il != (num_cells - 1)) fprintf(fp, "\n         ");
    }
    fprintf(fp, "\n");
#else
    fprintf(fp,
    ...
    retval =
        vtk_write_binary(fp, (char *)loc_type, sizeof(*loc_type) * num_cells);
    if (retval) {
        std::cout << rank << ": [VTU Error]: "
                  << "base64 encode element type data failed" << std::endl;
        fclose(fp);
    }
    delete[] loc_type;
...
#ifdef DENDRO_VTU_ASCII
    fprintf(fp,
            "        <DataArray type=\"%s\" Name=\"mpi_rank\""
            " format=\"%s\">\n",
    ...
    for (unsigned int il = 0; il < num_cells; ++il) loc_rank[il] = rank;

    retval =
        vtk_write_binary(fp, (char *)loc_rank, sizeof(*loc_rank) * num_cells);
    if (retval) {
        std::cout << rank << ": [VTU Error]: "
                  << "base64 encode element rank data failed" << std::endl;
        fclose(fp);
    }
    delete[] loc_rank;
...
#ifdef DENDRO_VTU_ASCII
    fprintf(fp,
            "        <DataArray type=\"%s\" Name=\"cell_level\""
            " format=\"%s\">\n",
            DENDRO_NODE_VAR_INT, DENDRO_FORMAT_ASCII);
    fprintf(fp, "         ");
    for (unsigned int il = 0; il < num_cells; ++il) {
        fprintf(fp, "%d ",
                pElements[il + pMesh->getElementLocalBegin()].getLevel());
    }
    fprintf(fp, "\n");
#else
    fprintf(fp,
    ...
    retval =
        vtk_write_binary(fp, (char *)loc_level, sizeof(*loc_level) * num_cells);
    if (retval) {
        std::cout << rank << ": [VTU Error]: "
                  << "base64 encode element level data failed" << std::endl;
        fclose(fp);
    }
    delete[] loc_level;
    ...
    if (numPointdata > 0 && pointData != NULL) {
        ...
        for (unsigned int pdata = 0; pdata < numPointdata; pdata++) {
            ...
            fprintf(fp,
                    ...
                    " format=\"%s\">\n",
                    DENDRO_NODE_VAR_DOUBLE, pointDataNames[pdata],
                    DENDRO_FORMAT_ASCII);
#else
            fprintf(fp,
                    "        <DataArray type=\"%s\" Name=\"%s\""
...
// std::cout<<"tmp_var: pdata: "<<pdata<<" var: "<<tmp_var[0]<<std::endl;
#ifdef DENDRO_VTU_ASCII
            fprintf(fp, "         ");
            for (unsigned int il = pMesh->getElementLocalBegin();
                 il < pMesh->getElementLocalEnd(); ++il) {
                pMesh->getElementNodalValues(tmp_var, &(*(nodalVal.begin())),
                                             il, isDGPData);
            ...
            for (unsigned int il = pMesh->getElementLocalBegin();
                ...
                pMesh->getElementNodalValues(tmp_var, &(*(nodalVal.begin())),
                                             il, isDGPData);
                for (unsigned int node = 0; node < nPe; node++)
                    nodalVal_binary[(il - pMesh->getElementLocalBegin()) * nPe +
                                    node] = nodalVal[node];
            }

            retval =
                vtk_write_binary(fp, (char *)nodalVal_binary,
                                 sizeof(*nodalVal_binary) * num_cells * nPe);

            if (retval) {
                std::cout << rank << ": [VTU Error]: "
                          << "base64 encode point vars data failed"
                          << std::endl;
                fclose(fp);
            }
    ...
    if (!rank) {
        ...
        if (fp == NULL) {
            std::cout << "rank: " << rank
                      << "[IO Error]: Could not open the pvtu file. "
                      << std::endl;
            return;
        }
...
#ifdef DENDRO_VTU_ASCII
        fprintf(fp,
                "        <PDataArray type=\"%s\" Name=\"mpi_rank\""
                " format=\"%s\"/>\n",
                DENDRO_NODE_VAR_INT, DENDRO_FORMAT_ASCII);
#else
        fprintf(fp,
...
#ifdef DENDRO_VTU_ASCII
        fprintf(fp,
                "        <PDataArray type=\"%s\" Name=\"cell_level\""
                " format=\"%s\"/>\n",
                DENDRO_NODE_VAR_INT, DENDRO_FORMAT_ASCII);
#else
        fprintf(fp,
...
void mesh2vtuCoarse(const ot::Mesh *pMesh, const char *fPrefix,
                    unsigned int numFieldData, const char **filedDataNames,
                    const double *filedData, unsigned int numPointdata,
                    const char **pointDataNames, const double **pointData,
                    bool isDGPData) {
    ...
    for (unsigned int ele = pMesh->getElementLocalBegin();
        ...
        for (unsigned int k = 0; k < (eleOrder + 1); k += eleOrder)
            for (unsigned int j = 0; j < (eleOrder + 1); j += eleOrder)
                for (unsigned int i = 0; i < (eleOrder + 1); i += eleOrder) {
                    coord_data[((ele - pMesh->getElementLocalBegin()) *
                                    NUM_CHILDREN +
                                (k / eleOrder) * ny * nx + (j / eleOrder) * nx +
                                (i / eleOrder)) *
                                   m_uiDim +
                               0] = VTU_OCT_X_GRID_X(pElements[ele].getX() +
                                                     i * (sz / eleOrder));
    ...
    for (unsigned int i = 0; i < num_cells; i++) {
        ...
        retval =
            vtk_write_binary(fp, (char *)locidx_data,
                             sizeof(*locidx_data) * NUM_CHILDREN * num_cells);
        if (retval) {
            std::cout << rank << ": [VTU Error]: "
                      << "base64 encode connectivity data failed" << std::endl;
...
// std::cout<<"tmp_var: pdata: "<<pdata<<" var: "<<tmp_var[0]<<std::endl;
            ...
            for (unsigned int il = pMesh->getElementLocalBegin();
                ...
                for (unsigned int k = 0; k < (eleOrder + 1); k += eleOrder)
                    for (unsigned int j = 0; j < (eleOrder + 1); j += eleOrder)
                        for (unsigned int i = 0; i < (eleOrder + 1);
                             i += eleOrder)
                            fprintf(
                                fp, "%f ",
                                nodalVal[k * (eleOrder + 1) * (eleOrder + 1) +
                                         j * (eleOrder + 1) + i]);
            }
            fprintf(fp, "\n");
#else
            ...
            retval = vtk_write_binary(fp, (char *)nodalVal_binary,
                                      sizeof(*nodalVal_binary) * num_vertices);
...
void mesh2vtuFine(const ot::Mesh *pMesh, const char *fPrefix,
                  unsigned int numFieldData, const char **filedDataNames,
                  const double *filedData, unsigned int numPointdata,
                  const char **pointDataNames, const double **pointData,
    ...
    for (unsigned int ele = 0; ele < pMesh->getNumLocalMeshElements(); ele++) {
        for (unsigned int ek = 0; ek < (eleOrder); ek++)
            for (unsigned int ej = 0; ej < (eleOrder); ej++)
                for (unsigned int ei = 0; ei < (eleOrder); ei++) {
                    locidx_data[(ele * ePe + ek * (eleOrder) * (eleOrder) +
                                 ej * (eleOrder) + ei) *
                                    NUM_CHILDREN +
                                0] =
                        (DendroIntL)(ele * nPe +
    ...
    if (nCellData > 0 && cellData != NULL) cell_dtmp = new double[num_cells];
#endif
    ...
    for (unsigned int il = 0; il < num_cells; ++il) cell_tmp[il] = rank;

    retval =
        vtk_write_binary(fp, (char *)cell_tmp, sizeof(*cell_tmp) * num_cells);
    if (retval) {
        std::cout << rank << ": [VTU Error]: "
                  << "base64 encode element rank data failed" << std::endl;
        fclose(fp);
    ...
    for (unsigned int il = pMesh->getElementLocalBegin();
         il < pMesh->getElementLocalEnd(); ++il) {
        for (unsigned int w = 0; w < ePe; w++)
            cell_tmp[(il - pMesh->getElementLocalBegin()) * ePe + w] =
                pElements[il].getLevel();
    }
    ...
    if (nCellData > 0 && cellData != NULL) {
        for (unsigned int v = 0; v < nCellData; v++) {
            ...
            fprintf(fp,
                    "        <DataArray type=\"%s\" Name=\"%s\""
                    " format=\"%s\">\n",
                    DENDRO_NODE_VAR_DOUBLE, cellDNames[v],
                    DENDRO_FORMAT_BINARY);

            for (unsigned int il = pMesh->getElementLocalBegin();
                 il < pMesh->getElementLocalEnd(); ++il) {
                for (unsigned int w = 0; w < ePe; w++)
                    cell_dtmp[(il - pMesh->getElementLocalBegin()) * ePe + w] =
                        cellData[v][il];
            }

            retval = vtk_write_binary(fp, (char *)(cell_dtmp),
                                      sizeof(*cell_dtmp) * num_cells);
            if (retval) {
                std::cout << rank << ": [VTU Error]: "
                          << "base64 encode element level data failed"
                          << std::endl;
                fclose(fp);
            }
    ...
    if (nCellData > 0 && cellData != NULL) delete[] cell_dtmp;
#endif
...
void oct2vtu(const ot::TreeNode *pNodes, const unsigned int nSize,
        ...
        for (unsigned int k = 0; k < nz; k++)
            for (unsigned int j = 0; j < ny; j++)
                for (unsigned int i = 0; i < nx; i++) {
                    fprintf(fp, "          %d %d %d\n",
                            (pNodes[ele].getX() + i * sz),
                            (pNodes[ele].getY() + j * sz),
                            (pNodes[ele].getZ() + k * sz));
                }
    }
#else
    ...
    for (unsigned int ele = 0; ele < nSize; ele++) {
        ...
        for (unsigned int k = 0; k < nz; k++)
            for (unsigned int j = 0; j < ny; j++)
                for (unsigned int i = 0; i < nx; i++) {
                    coord_data[((ele)*NUM_CHILDREN + k * ny * nx + j * nx + i) *
                                   m_uiDim +
                               0] = (pNodes[ele].getX() + i * sz);
                    coord_data[((ele)*NUM_CHILDREN + k * ny * nx + j * nx + i) *
                                   m_uiDim +
                               1] = (pNodes[ele].getY() + j * sz);
                    coord_data[((ele)*NUM_CHILDREN + k * ny * nx + j * nx + i) *
                                   m_uiDim +
                               2] = (pNodes[ele].getZ() + k * sz);
                }
    }
...
void mesh2vtu_slice(const ot::Mesh *pMesh, unsigned int s_val[],
                    unsigned int s_normal[], const char *fPrefix,
                    unsigned int numFieldData, const char **filedDataNames,
                    const double *filedData, unsigned int numPointdata,
                    const char **pointDataNames, const double **pointData,
                    bool isDGPData) {
    ...
    if (!sid_empty) {
        ...
        if (fp == NULL) {
            std::cout << "rank: " << rank
                      << "[IO Error]: Could not open the vtk file. "
                      << std::endl;
            return;
        }
        ...
        fprintf(fp,
                "    <Piece NumberOfPoints=\"%lld\" NumberOfCells=\"%d\">\n",
                (long long)num_vertices, num_cells);
        ...
        retval = vtk_write_binary(fp, (char *)loc_type,
                                  sizeof(*loc_type) * num_cells);
        if (retval) {
            std::cout << rank << ": [VTU Error]: "
                      << "base64 encode element type data failed" << std::endl;
            fclose(fp);
        ...
        retval = vtk_write_binary(fp, (char *)loc_rank,
                                  sizeof(*loc_rank) * num_cells);
        if (retval) {
            std::cout << rank << ": [VTU Error]: "
                      << "base64 encode element rank data failed" << std::endl;
            fclose(fp);
        ...
        for (unsigned int s = 0; s < sids.size(); s++) {
            ...
            for (unsigned int w = 0; w < ePe; w++)
                fprintf(fp, "%d ",
                        (pElements[il].getLevel() + (eleOrder >> 1u)));
        }
        ...
        retval = vtk_write_binary(fp, (char *)loc_level,
                                  sizeof(*loc_level) * num_cells);
        if (retval) {
            std::cout << rank << ": [VTU Error]: "
                      << "base64 encode element level data failed" << std::endl;
            fclose(fp);
...
// std::cout<<"tmp_var: pdata: "<<pdata<<" var: "<<tmp_var[0]<<std::endl;
                ...
                for (unsigned int s = 0; s < sids.size(); s++) {
                    const unsigned int il = sids[s];
                    pMesh->getElementNodalValues(
                        tmp_var, &(*(nodalVal.begin())), il, isDGPData);
                    for (unsigned int node = 0; node < nPe; node++)
                        fprintf(fp, "%f ", nodalVal[node]);
                }
                fprintf(fp, "\n");
