 ...
 */
enum ETSType { RK3 = 0, RK4, RK5, RK4_RALSTON, RK45_CASH_KARP, RKF45 };
...
struct BLK_STMP {
    ...
    inline bool operator==(const BLK_STMP& other) const {
        return (_time == other._time && _stage == other._stage &&
                _lev == other._lev);
    }
...
//         return (os << other._time << " " << other._stage << " " <<
//         other._lev);
// } //end fn
...
struct SyncBLK {
    ...
    void append_from_prev(const SyncBLK<T>& lprev) {
        for (unsigned int i = 0; i < lprev.blks.size(); i++)
            blks.push_back(
                BlockTSInfo<T>(lprev.blks[i].blkID, TSFlag::FULL_STEP));

        if (ll > lprev.ll) ll = lprev.ll;
    }
