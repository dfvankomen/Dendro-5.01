...
void sync_blk_padding(const ot::Mesh* pMesh, const T* const dgWVec,
                      ...
                      unsigned int bVecIndex, unsigned int dof) {
    ...
    for (unsigned int elem = blkList[blk].getLocalElementBegin();
        ...
        const unsigned int ei =
            (pNodes[elem].getX() - blkNode.getX()) >> (m_uiMaxDepth - regLevel);
        const unsigned int ej =
            (pNodes[elem].getY() - blkNode.getY()) >> (m_uiMaxDepth - regLevel);
        const unsigned int ek =
            (pNodes[elem].getZ() - blkNode.getZ()) >> (m_uiMaxDepth - regLevel);
