 ...
 * @brief Fully asynchronous storage to ot::Block data structure for non-uniform
 * time stepping.
 * @version 0.1
 * @date 2020-03-17
 *
...
class BlockAsyncVector {
   ...
   public:
     ...
     */
    void createVec(unsigned int blk, const unsigned int* sz,
                   bool synced             = false,
                   BLK_ASYNC_VEC_MODE mode = BLK_ASYNC_VEC_MODE::BLK_UNZIP,
                   unsigned int dof        = 1) {
        m_uiBlkID             = blk;
        m_uiSz[0]             = sz[0];
        m_uiSz[1]             = sz[1];
     ...
     * @param isWithPadding : if true the copy will happen with padding.
     * otherwise padding region is not coppied.
     * @param dof : number of dofs
     */
    void copyFromUnzip(const ot::Mesh* pMesh, const T* const vUnzip,
                       bool isWithPadding, unsigned int dof = 1) {
        ...
        if (m_uiMode == BLK_ASYNC_VEC_MODE::BLK_UNZIP) {
            ...
            if (isWithPadding) {
                for (unsigned int v = 0; v < dof; v++) {
                    ...
                    std::memcpy(m_uiVec + v * nx * ny * nz, &v1[offset],
                                sizeof(T) * nx * ny * nz);
                }
                    ...
                    for (unsigned int k = pw; k < nz - pw; k++)
                        for (unsigned int j = pw; j < ny - pw; j++)
                            for (unsigned int i = pw; i < nx - pw; i++)
                                m_uiVec[(v * nx * ny * nz) + k * ny * nx +
                                        j * nx + i] =
                                    v1[offset + k * ny * nx + j * nx + i];
                }
            }
        }
     ...
     * @param dof   : number of DOF
     */
    void copyFromVecDG(const ot::Mesh* pMesh, const T* const vecDG,
                       unsigned int dof = 1) {
        if (!(pMesh->isActive())) return;
                ...
                for (unsigned int k = 0; k < (eOrder + 1); k++)
                    for (unsigned int j = 0; j < (eOrder + 1); j++)
                        for (unsigned int i = 0; i < (eOrder + 1); i++)
                            m_uiVec[(v * lx * ly * lz) +
                                    (ek * eOrder + k + paddWidth) * (ly * lx) +
                                    (ej * eOrder + j + paddWidth) * (lx) +
                                    (ei * eOrder + i + paddWidth)] =
                                vecDG[(v * dgSz + elem * nPe) +
                                      k * (eOrder + 1) * (eOrder + 1) +
                                      j * (eOrder + 1) + i];
            }
        }
        ...
        if (m_uiMode == BLK_ASYNC_VEC_MODE::BLK_UNZIP) {
            for (unsigned int v = 0; v < dof; v++) {
                for (unsigned int elem =
                         blkList[m_uiBlkID].getLocalElementBegin();
                     elem < blkList[m_uiBlkID].getLocalElementEnd(); elem++) {
                    const unsigned int ei =
                        (pNodes[elem].getX() - blkNode.getX()) >>
                        (m_uiMaxDepth - regLev);
                    const unsigned int ej =
                                ...
                                if ((e2n_dg[elem * nPe +
                                    ...
                                    zipVec[(v * vsz_cg) +
                                           e2n[elem * nPe +
                                               ...
                                               j * (eOrder + 1) + i]] =
                                        m_uiVec[(v * lx * ly * lz) +
                                                (ek * eOrder + k + paddWidth) *
                                                    (ly * lx) +
                                                (ej * eOrder + j + paddWidth) *
                                                    (lx) +
                                                (ei * eOrder + i + paddWidth)];

                                // Note 10/28/2020 : The below zip is wrong you
                                // copy the nodal values in the wrong place for
                                // non hanging case, node index mapping is not
                                // that simple. const unsigned int node_dg =
                                // e2n_dg[elem*nPe +
                                // k*(eOrder+1)*(eOrder+1)+j*(eOrder+1)+i ];
                                // pMesh->dg2eijk(node_dg,oEid,oXi,oYj,oZk);
                                // const bool isHanging =
                                // pMesh->isNodeHanging(elem,i,j,k);
                                // if(!isHanging)
                                //     zipVec[ (v*vsz_cg) + e2n[elem*nPe +
                                //     k*(eOrder+1)*(eOrder+1) + j* (eOrder+1)+
                                //     i]] = m_uiVec[ (v*lx*ly*lz) +
                                //     (ek*eOrder+k+paddWidth)*(ly*lx)+(ej*eOrder+j+paddWidth)*(lx)+(ei*eOrder+i+paddWidth)];
                                // else
                                // {
                                //     const unsigned int
                                //     cnum=pNodes[(elem)].getMortonIndex();
                                //     const unsigned int iix = eOrder * (int)
                                //     (cnum & 1u)  +  i; const unsigned int jjy
                                //     = eOrder * (int) ((cnum & 2u)>>1u)  +  j;
                                //     const unsigned int kkz = eOrder * (int)
                                //     ((cnum & 4u)>>2u)  +  k;

                                //     if( (iix %2 ==0) && (jjy%2 ==0) &&
                                //     (kkz%2==0))
                                //         zipVec[(v*vsz_cg) + e2n[elem*nPe +
                                //         (kkz>>1u)*(eOrder+1)*(eOrder+1) +
                                //         (jjy>>1u) * (eOrder+1)+ (iix>>1u)]] =
                                //         m_uiVec[ (v*lx*ly*lz)  +
                                //         (ek*eOrder+k+paddWidth)*(ly*lx)+(ej*eOrder+j+paddWidth)*(lx)+(ei*eOrder+i+paddWidth)];
    ...
    void zipDG(ot::Mesh* pMesh, T* dgVec, unsigned int dof = 1) const {
        ...
        for (unsigned int v = 0; v < dof; v++) {
            for (unsigned int elem = blkList[m_uiBlkID].getLocalElementBegin();
                 elem < blkList[m_uiBlkID].getLocalElementEnd(); elem++) {
                const unsigned int ei =
                    (pNodes[elem].getX() - blkNode.getX()) >>
                    (m_uiMaxDepth - regLev);
    ...
    inline unsigned int getSz() const {
        return (m_uiSz[0] * m_uiSz[1] * m_uiSz[2]);
    }
    ...
    void getUnzipElementalValues(const ot::Mesh* pMesh, unsigned int ele,
                                 T* eleVec) {
        if (pMesh->isActive()) {
            const std::vector<ot::Block>& blkList = pMesh->getLocalBlockList();
            const ot::TreeNode* pNodes = pMesh->getAllElements().data();

            if (!(ele >= blkList[m_uiBlkID].getLocalElementBegin() &&
                  ele < blkList[m_uiBlkID].getLocalElementEnd())) {
                std::cout << "Error : " << __LINE__
                          << " block async. get unzip elemental values called "
                             "on the invalid BVec"
                          << std::endl;
                return;
            }
            ...
            const unsigned int en[3] = {(eOrder + 1) + 2 * paddWidth,
                                        (eOrder + 1) + 2 * paddWidth,
                                        (eOrder + 1) + 2 * paddWidth};
            ...
            for (unsigned int v = 0; v < m_uiDof; v++) {
                ...
                //         out[(k-kb) * en[1]*en[0] + (j-jb)*en[1] + (i-ib)] =
                //         uzipVec[k*ly*lx + j*lx + paddWidth];
                ...
                //     for(unsigned int i=ib; i< ie; i++)
                //         out[(k-kb) * en[1]*en[0] + (j-jb)*en[1] + (i-ib)] =
                //         uzipVec[k*ly*lx + paddWidth*lx + i];
                ...
                //     for(unsigned int i=ib; i< ie; i++)
                //         out[(k-kb) * en[1]*en[0] + (j-jb)*en[1] + (i-ib)] =
                //         uzipVec[paddWidth*ly*lx + j*lx + i];
                ...
                //         out[(k-kb) * en[1]*en[0] + (j-jb)*en[1] + (i-ib)] =
                //         uzipVec[k*ly*lx + j*lx + (ie-paddWidth-1)];
                ...
                //     for(unsigned int i=ib; i< ie; i++)
                //         out[(k-kb) * en[1]*en[0] + (j-jb)*en[1] + (i-ib)] =
                //         uzipVec[k*ly*lx + (je-paddWidth-1)*lx + i];
                ...
                //     for(unsigned int i=ib; i< ie; i++)
                //         out[(k-kb) * en[1]*en[0] + (j-jb)*en[1] + (i-ib)] =
                //         uzipVec[(ke-paddWidth-1)*ly*lx + j*lx + i];
    ...
    void getElementalValues(const ot::Mesh* pMesh, unsigned int ele,
                            T* eleVec) {
        if (pMesh->isActive()) {
            const std::vector<ot::Block>& blkList = pMesh->getLocalBlockList();
            const ot::TreeNode* pNodes = pMesh->getAllElements().data();
            ...
            const unsigned int en[3]    = {(eOrder + 1), (eOrder + 1),
                                           (eOrder + 1)};
