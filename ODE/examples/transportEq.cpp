...
int main(int argc, char** argv) {
        ...
        if (!rank)
            std::cout << "Usage: " << argv[0]
                      << " maxDepth wavelet_tol partition_tol eleOrder "
                         "fPrefix=[sol_step] begin=[0] end=[10] th=[0.01] "
                         "restore_step[=0] restore_rk[=0]"
                      << std::endl;
        return 0;
    }
    ...
    std::function<double(double, double, double)> g =
        [domain_min, domain_max, grid_min, grid_max](
            const double x, const double y, const double z) {
            return (sin((((x - grid_min.x()) / (grid_max.x() - grid_min.x())) *
                             (domain_max.x() - domain_min.x()) +
                         domain_min.x())) *
                    sin((((y - grid_min.y()) / (grid_max.y() - grid_min.y())) *
                             (domain_max.y() - domain_min.y()) +
                         domain_min.y())) *
                    sin((((z - grid_min.z()) / (grid_max.z() - grid_min.z())) *
                             (domain_max.z() - domain_min.z()) +
                         domain_min.z())));
        };
    std::function<double(double, double, double, double)> f =
        [domain_min, domain_max](const double x, const double y, const double z,
                                 const double t) { return 0; };

    // std::function<double(double,double,double)> dx_func=[d_min,d_max](const
    ...
    // std::function<double(double,double,double)> dy_func=[d_min,d_max](const
    // double x,const double y,const double z){ return
    ...
    // std::function<double(double,double,double)> dz_func=[d_min,d_max](const
    // double x,const double y,const double z){ return
    ...
    SFC::parSort::SFC_treeSort(tmpNodes, balOctree, balOctree, balOctree,
                               partition_tol, m_uiMaxDepth, rootNode,
                               ROOT_ROTATION, 1, TS_REMOVE_DUPLICATES, 2, comm);
    std::swap(tmpNodes, balOctree);
    balOctree.clear();

    SFC::parSort::SFC_treeSort(tmpNodes, balOctree, balOctree, balOctree,
                               partition_tol, m_uiMaxDepth, rootNode,
                               ROOT_ROTATION, 1, TS_CONSTRUCT_OCTREE, 2, comm);
    std::swap(tmpNodes, balOctree);
    balOctree.clear();

    SFC::parSort::SFC_treeSort(tmpNodes, balOctree, balOctree, balOctree,
                               partition_tol, m_uiMaxDepth, rootNode,
                               ROOT_ROTATION, 1, TS_BALANCE_OCTREE, 2, comm);
    tmpNodes.clear();
