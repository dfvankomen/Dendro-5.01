 ...
 * @brief File to test LTS block vector routines, unzip form DG vector zipDG and
 * zipCG when all the blocks are synced.
 * @version 0.1
 * @date 2021-03-19
...
int main(int argc, char** argv) {
    ...
    std::function<void(double, double, double, double*)> func_1 =
        [](const double xx, const double yy, const double zz, double* var) {
            var[0] =
                sin(2 * M_PI * xx) * sin(2 * M_PI * yy) * sin(2 * M_PI * zz);
            var[1] = 1 * sin(2 * M_PI * xx) * sin(2 * M_PI * yy) *
                     sin(2 * M_PI *
                         zz);  // 1/(1+
                               // sin(2*M_PI*xx)*sin(2*M_PI*yy)*sin(2*M_PI*zz));
            return;
    ...
    DendroScalar* funcValCGUnzip =
        mesh->createUnZippedVector<DendroScalar>(DOF);
    DendroScalar* funcValDGUnzip =
        mesh->createUnZippedVector<DendroScalar>(DOF);
...

    {
        if (mesh->isActive()) {
            ...
            for (unsigned int blk = 0; blk < blkList.size(); blk++) {
                ...
                m_uiBVec[blk]._rks  = 0;
                m_uiBVec[blk]._vec[0].copyFromUnzip(mesh, funcValDGUnzip, true,
                                                    DOF);
                m_uiBVec[blk]._vec[0].mark_synced();

                // second vector copy without the padding region.
                m_uiBVec[blk]._vec[INDEX_PT_SYNC].copyFromUnzip(
                    mesh, funcValDGUnzip, false, DOF);
                m_uiBVec[blk]._vec[INDEX_PT_SYNC].mark_unsynced();
            }

            // sync m_uiBVec[blk]._vec[1] using partial block sync
            for (unsigned int blk = 0; blk < blkList.size(); blk++)
                ts::sync_blk_padding(mesh, funcValDG, m_uiBVec, blk,
                                     INDEX_PT_SYNC, DOF);
                ...
                if (!bflag) {
                    ...
                    for (unsigned int node = 0; node < (lx * ly * lz); node++) {
                        for (unsigned int v = 0; v < DOF; v++) {
                            const DendroScalar v1 =
                                bdata[(v * lx * ly * lz) + node];
                            const DendroScalar v2 = funcValDGUnzip
                                [v * mesh->getDegOfFreedomUnZip() + offset +
                                 node];
                            const DendroScalar diff_abs = fabs(v1 - v2);
                            if (diff_abs > 1e-6) {
                                std::cout << RED
                                          << "Failed: [Error in parital block "
                                             "sync]: rank: "
                                          << active_rank << " blk: " << blk
                                          << " v1: " << v1 << " v2: " << v2
                                          << NRM << std::endl;
            ...
            for (unsigned int v = 0; v < DOF; v++)
                linf[v] = normLInfty(
                    fVec1_DG + v * dg_sz + mesh->getElementLocalBegin() * nPe,
                    funcValDG + v * dg_sz + mesh->getElementLocalBegin() * nPe,
                    mesh->getNumLocalMeshElements() * nPe, aComm);

            if (!active_rank) {
                for (unsigned int v = 0; v < DOF; v++)
                    std::cout << " var dg : " << v << " linf : " << linf[v]
                              << std::endl;
            }

            for (unsigned int v = 0; v < DOF; v++)
                linf[v] = normLInfty(
                    fVec1_CG + v * cg_sz + mesh->getNodeLocalBegin(),
                    funcValCG + v * cg_sz + mesh->getNodeLocalBegin(),
                    mesh->getNumLocalMeshNodes(), aComm);

            if (!active_rank) {
                for (unsigned int v = 0; v < DOF; v++)
                    std::cout << " var cg : " << v << " linf : " << linf[v]
                              << std::endl;
            }
        }
    }
