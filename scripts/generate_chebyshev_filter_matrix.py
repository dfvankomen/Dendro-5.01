...
print("Now generating hard-coded Chebyshev filter matrices!")
...
def create_string(
    ...
    outstr += f"void set_chebyshev_filt_ele{eleorder}_out{reduced}_dim{dim}(){{\n"
    ...
    outstr += f"    {dimvarname}_dim{dim}_nx = {u.shape[0]};\n"
    outstr += f"    {dimvarname}_dim{dim}_ny = {u.shape[1]};\n"
...
if __name__ == "__main__":
    ...
    with open(output_folder / f"cheb_filter_ele{ELE_ORDER}.inc.h", "w") as f:
        f.write(to_write)
